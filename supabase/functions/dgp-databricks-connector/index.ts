...
function fullTable(catalog: string, schema: string, table: string) {
  return `${catalog}.${schema}.${table}`;
}

function quotedTable(catalog: string, schema: string, table: string) {
  return `${quoteIdentifier(catalog)}.${quoteIdentifier(schema)}.${quoteIdentifier(table)}`;
}

function statementError(payload: StatementPayload) {
  return payload.status?.error?.message ?? `Databricks SQL statement ended in ${payload.status?.state ?? "UNKNOWN"} state.`;
}
...
async function catalog(body: ConnectorRequest) {
  ...
  const schemaNames = schemas
    .map((row) => typeof row.name === "string" ? row.name : typeof row.full_name === "string" ? row.full_name.split(".").at(-1) ?? "" : "")
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b));
  let tables: Record<string, unknown>[] = [];
  if (schema) {
    tables = await pagedUnityCatalog(
      target,
      token,
...
async function validate(body: ConnectorRequest) {
  ...
  return {
    ...
    details: {
      ...
      schema,
      table,
      table_type: typeof info.table_type === "string" ? info.table_type : null,
      warehouse_id: target.warehouseId,
    },
  };
}
...
async function query(body: ConnectorRequest) {
  ...
  return {
    ...
    columns: result.columns.map((column) => ({ name: column.name ?? "", type: column.type_name ?? column.type_text ?? null })),
...
async function lineage(body: ConnectorRequest) {
  ...
  try {
    const result = await executeSql(
      target,
      token,
      `SELECT source_table_full_name, target_table_full_name, source_column_name, target_column_name, entity_type, entity_id, created_by, event_time FROM system.access.column_lineage WHERE source_table_full_name = '${requestedLiteral}' OR target_table_full_name = '${requestedLiteral}' ORDER BY event_time DESC LIMIT 2000`,
      catalog,
      schema,
    );
    columnRows = rowsFromStatement(result) as LineageRow[];
  } catch (error) {
    warnings.push(`Databricks column lineage is unavailable for ${requestedFullName}: ${error instanceof Error ? error.message : "unknown error"}`);
  }
  ...
  try {
    const result = await executeSql(
      target,
      token,
      `SELECT source_table_full_name, target_table_full_name, entity_type, entity_id, created_by, event_time FROM system.access.table_lineage WHERE source_table_full_name = '${requestedLiteral}' OR target_table_full_name = '${requestedLiteral}' ORDER BY event_time DESC LIMIT 2000`,
      catalog,
      schema,
    );
    tableRows = rowsFromStatement(result) as LineageRow[];
  } catch (error) {
    warnings.push(`Databricks table lineage is unavailable for ${requestedFullName}: ${error instanceof Error ? error.message : "unknown error"}`);
  }
  ...
  for (const row of columnRows) {
    ...
    current.mappings.push({
      sourceAsset: source,
      sourceColumn,
      targetAsset: targetName,
      targetColumn,
      operation: "DATABRICKS_COLUMN_LINEAGE",
      expression: null,
      metadata: { authoritative_source: "system.access.column_lineage", entity_type: entityType, entity_id: entityId, created_by: createdBy, event_time: eventTime },
    });
  ...
  for (const [key, event] of grouped) {
    ...
    transformations.push({
      ...
      metadata: {
        authoritative_source: event.sourceTable,
        hash_basis: "DATABRICKS_SYSTEM_LINEAGE_RECORD",
        source_table_full_name: event.source,
        target_table_full_name: event.target,
        entity_type: event.entityType,
        entity_id: event.entityId,
        created_by: event.createdBy,
        event_time: event.eventTime,
        source_event_key_hash: await sha256(key),
      },
    });
  }
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (body.action === "health") return reply(200, { ok: true, drivers: ["databricks"], credential_store: "supabase-vault", metadata_provider: "unity-catalog-rest", query_provider: "statement-execution-api", lineage_provider: ["system.access.table_lineage", "system.access.column_lineage"], technical_max_rows: TECHNICAL_MAX_ROWS });
