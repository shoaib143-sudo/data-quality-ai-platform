...
-- The affected legacy run stored each dataset metric once per profile column.
-- Values were identical, so one canonical row per dataset metric can be retained.
with legacy_rows as (
  select
    pm.id,
    pm.profile_run_id,
    pm.metric_definition_id,
    pm.metric_key,
    pm.profile_column_id,
    row_number() over (
      partition by pm.profile_run_id, pm.metric_definition_id
...
audit_rows as (
  insert into profiling.metric_repair_audit (
    profile_run_id,
    profile_metric_id,
    metric_definition_id,
    metric_key,
    original_profile_column_id,
    action,
    details
  )
  select
...
delete from profiling.profile_metrics pm
using legacy_rows lr
where pm.id = lr.id and lr.rn > 1;
...
update profiling.profile_metrics pm
set profile_column_id = null
from legacy_rows lr
where pm.id = lr.id and lr.rn = 1;

-- The known historical partial run is retained, but no longer represented as a
-- successful completion. This makes downstream consumers respect the contract
...
set
  ...
  summary = coalesce(summary, '{}'::jsonb) || jsonb_build_object(
    'historical_repair', jsonb_build_object(
      'reclassified', true,
      'reason', 'incomplete metric execution set under current persistence contract'
    )
  )
where id = 'bf38ba8d-d9b2-424e-a010-c4df05556081'
  and status = 'COMPLETED';

-- Enforce one execution result per run + metric definition + column identity.
-- Dataset metrics use a null column identity, so coalesce is required for a
...
-- Cover the foreign key used by profile anomaly lookups and joins.
create index if not exists profile_anomalies_metric_idx
on profiling.profile_anomalies (metric_definition_id);
