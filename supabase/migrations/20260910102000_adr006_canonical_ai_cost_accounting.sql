...
-- Reuses governance.ai_model_pricing_versions, the existing governed pricing authority.
-- No prices are seeded or inferred by this migration.
...
drop trigger if exists ai_model_cost_events_immutable on governance.ai_model_cost_events;
...
begin
  ...
  if p_provider_id is null or length(btrim(p_provider_id)) = 0
     or p_model_name is null or length(btrim(p_model_name)) = 0 then
  ...
  if found then
    if existing_event.project_id <> p_project_id
       or existing_event.provider_id <> lower(btrim(p_provider_id))
  ...
  if p_input_tokens is null or p_output_tokens is null then
    ...
    where p.project_id = p_project_id
      and p.provider = lower(btrim(p_provider_id))
      and p.model_id = btrim(p_model_name)
      and p.effective_from <= p_observed_at
      and (p.effective_to is null or p.effective_to > p_observed_at);

    if pricing_matches > 1 then
      raise exception 'Ambiguous effective AI model pricing for project %, provider %, model % at %',
        p_project_id, p_provider_id, p_model_name, p_observed_at
        using errcode = '23000';
      ...
      status := 'PRICED';
      calculated_input_cost := round((p_input_tokens::numeric / 1000000::numeric) * pricing.input_price_per_million_tokens, 10);
      calculated_output_cost := round((p_output_tokens::numeric / 1000000::numeric) * pricing.output_price_per_million_tokens, 10);
  ...
  insert into governance.ai_model_cost_events (
    ...
    lower(btrim(p_provider_id)), btrim(p_model_name), p_input_tokens, p_output_tokens, p_total_tokens,
...
revoke all on table governance.ai_model_cost_events from public, anon, authenticated;
grant select, insert on table governance.ai_model_cost_events to service_role;

revoke all on function governance.record_ai_model_cost_event(uuid,uuid,uuid,text,text,text,bigint,bigint,bigint,timestamptz)
  from public, anon, authenticated;
grant execute on function governance.record_ai_model_cost_event(uuid,uuid,uuid,text,text,text,bigint,bigint,bigint,timestamptz)
  to service_role;

comment on table governance.ai_model_cost_events is
  'Immutable canonical AI invocation cost evidence derived only from provider-observed usage and the existing governed project pricing authority.';
comment on function governance.record_ai_model_cost_event(uuid,uuid,uuid,text,text,text,bigint,bigint,bigint,timestamptz) is
  'Records idempotent AI invocation cost evidence. Missing observed usage or governed project pricing is explicit and never estimated.';
