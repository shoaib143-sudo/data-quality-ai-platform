...
declare
  ...
  v_contract jsonb;
  v_component_overall_score numeric;
  v_requested_overall_score numeric;
...
begin
  ...
  delete from profiling.profile_distributions where profile_run_id = p_profile_run_id;
  ...
  select
    ...
    nullif(m->'json_value', 'null'::jsonb)
  from jsonb_array_elements(coalesce(p_metrics, '[]'::jsonb)) m;

  insert into profiling.profile_distributions (
    profile_run_id, profile_column_id, distribution_type, distribution
  )
  select
    p_profile_run_id,
    nullif(m->>'profile_column_id','')::uuid,
    ...
    nullif(m->'json_value','null'::jsonb)
  from jsonb_array_elements(coalesce(p_metrics, '[]'::jsonb)) m
  where m->>'metric_key' in ('top_values','quantiles','histogram')
    and nullif(m->>'profile_column_id','') is not null
    and nullif(m->'json_value','null'::jsonb) is not null;
  ...
  v_requested_overall_score := nullif(p_score->>'overall_score','')::numeric;
  if v_requested_overall_score is not null then
    if v_requested_overall_score < 0 or v_requested_overall_score > 1 then
      raise exception 'Profiling overall score % is outside the valid range [0,1]', v_requested_overall_score;
    end if;
    if v_component_overall_score is not null
       and abs(v_requested_overall_score - v_component_overall_score) > 0.0002 then
      raise exception 'Profiling overall score % is inconsistent with component aggregate %', v_requested_overall_score, v_component_overall_score;
    end if;
  end if;

  v_overall_score := coalesce(v_requested_overall_score, v_component_overall_score);
  v_summary := coalesce(p_summary, '{}'::jsonb)
    || jsonb_build_object(
      'score',
      coalesce(p_summary->'score', '{}'::jsonb)
      || coalesce(p_score, '{}'::jsonb)
      || jsonb_build_object('overall_score', v_overall_score)
    );
