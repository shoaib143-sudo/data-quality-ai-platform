...
language plpgsql
security definer
set search_path to 'pg_catalog', 'governance', 'profiling', 'catalog', 'orchestration', 'app'
as $function$
declare
  v_run_id uuid;
  v_org_id uuid;
...
begin
  ...
  begin
    ...
    join app.projects p on p.organization_id = m.organization_id
    where governance.has_project_capability(p.id, m.user_id, 'policy.approve')
    order by m.user_id
    limit 1;

    if v_reviewer_id is null then
      raise exception 'Synthetic governance integration suite requires an existing policy.approve reviewer fixture';
    end if;
    ...
    values(v_org_id,v_reviewer_id,'OWNER');
    ...
    values(v_project_id,v_dataset_id,'Synthetic Contract','DRAFT',0)
    returning id into v_contract_id;

    v_contract_version_id := governance.propose_data_contract_version(
      v_contract_id,
      v_reviewer_id,
      'schema-v1',
      'BACKWARD',
      ...
      'Synthetic governed contract version for integration validation'
    );

    perform governance.review_data_contract_version(
      v_contract_version_id,
      v_reviewer_id,
      'APPROVE',
      'Synthetic governed approval for integration validation'
    );

    select count(*) into v_count
    from governance.data_contract_versions
    where id=v_contract_version_id
      and contract_id=v_contract_id
      and status='ACTIVE'
      and authority_status='APPROVED'
      and approved_by=v_reviewer_id
      and effective_at is not null;
    v_checks := v_checks || jsonb_build_object('governed_contract_activation',v_count=1);
    if v_count<>1 then raise exception 'Synthetic data contract did not complete governed activation'; end if;
    ...
    from profiling.metric_definitions md where md.enabled=true and md.scope='DATASET';
    ...
    if not (v_checks->>'profiling_metric_contract_dataset_metrics')::boolean then raise exception 'Synthetic profiling dataset metrics are incomplete'; end if;
    ...
    from governance.data_contract_evaluations
    where contract_version_id=v_contract_version_id and profile_run_id=v_profile_id;
    ...
    values(v_project_id,v_dataset_id,v_version_id,'synthetic.required','Synthetic Required Rule','COMPLETENESS','HIGH','null_rate','LTE',0,true,'SYSTEM','METRIC_THRESHOLD','{}'::jsonb)
