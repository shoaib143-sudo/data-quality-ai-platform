...
begin
  ...
  if new.state = 'READY' then
    ...
    if source_row.checksum_algorithm = 'sha256'
       and source_row.checksum is not null
       and source_row.checksum <> new.checksum then
      raise exception 'READY MIGRATION_COPY checksum must match source checksum when present.'
        using errcode = '23514';
    end if;
  end if;
...
drop trigger if exists trg_storage_objects_r2_migration_copy_invariants on catalog.storage_objects;
create trigger trg_storage_objects_r2_migration_copy_invariants
before insert or update of project_id, provider, owner_type, metadata, state, size_bytes, checksum_algorithm, checksum
on catalog.storage_objects
for each row
execute function catalog.enforce_r2_migration_copy_invariants();

comment on function catalog.enforce_r2_migration_copy_invariants() is
  'Fail-closed database boundary for R2 migration copies: same-project READY Supabase source, matching size, and SHA-256 integrity before READY.';
