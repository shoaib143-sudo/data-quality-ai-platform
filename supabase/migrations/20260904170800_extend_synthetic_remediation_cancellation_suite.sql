...
begin
  ...
  begin
    ...
    insert into governance.profiling_remediation_outcomes(
      ...
      production_mutation_performed,remediation_issue_ids,outcome
    )
    values(
      v_project_id,
      v_instance_id,
      ...
      false,
      '{}'::uuid[],
      jsonb_build_object('verification_generation',1)
    );
    ...
    values(
      ...
      'synthetic:remediation-verification:'||v_instance_id::text||':1'
    )
    ...
    update governance.profiling_remediation_outcomes
    set status='VERIFICATION_CANCELLED',
    ...
    from governance.profiling_remediation_outcomes
    where workflow_instance_id=v_instance_id;
    v_checks := v_checks || jsonb_build_object('cancelled_state_persisted',v_status='VERIFICATION_CANCELLED');
    if v_status is distinct from 'VERIFICATION_CANCELLED' then raise exception 'Cancelled verification state should persist'; end if;

    select governance.claim_profiling_remediation_verification(v_instance_id,null) into v_claim;
    v_checks := v_checks || jsonb_build_object('cancelled_state_not_claimable',v_claim is false);
    if v_claim is distinct from false then raise exception 'Cancelled verification must not be reclaimed without explicit restart'; end if;

    update governance.profiling_remediation_outcomes
    set status='ACTION_TRACKED',
        ...
        outcome=(coalesce(outcome,'{}'::jsonb)-'verification_cancelled')||jsonb_build_object(
    ...
    select governance.claim_profiling_remediation_verification(v_instance_id,null) into v_claim;
    v_checks := v_checks || jsonb_build_object('fresh_generation_reclaim_succeeds',v_claim is true);
    if v_claim is distinct from true then raise exception 'Fresh verification generation should become claimable after explicit reset'; end if;
