create unique index if not exists profile_distributions_identity
on profiling.profile_distributions(profile_run_id,profile_column_id,distribution_type);

delete from profiling.profile_distributions pd
where exists (
  select 1 from profiling.profile_metrics pm
  where pm.profile_run_id=pd.profile_run_id
    and pm.profile_column_id=pd.profile_column_id
    and upper(pm.metric_key)=pd.distribution_type
    and pm.metric_key in ('top_values','quantiles','histogram')
);

insert into profiling.profile_distributions(profile_run_id,profile_column_id,distribution_type,distribution)
select pm.profile_run_id,pm.profile_column_id,upper(pm.metric_key),pm.json_value
from profiling.profile_metrics pm
where pm.metric_key in ('top_values','quantiles','histogram')
  and pm.profile_column_id is not null
  and pm.json_value is not null
on conflict(profile_run_id,profile_column_id,distribution_type) do update set distribution=excluded.distribution;
