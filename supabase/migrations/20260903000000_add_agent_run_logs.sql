...
comment on table agent.agent_run_logs is 'Append-only operational logs for persisted agent runs. Server-side executor writes use the service role; client reads remain denied until a scoped read policy is introduced.';
