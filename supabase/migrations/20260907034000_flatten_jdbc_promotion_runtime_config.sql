-- Ensure promoted JDBC datasets expose runtime connection fields at the execution-config root.
-- Nested connection_metadata is retained for provenance and source context.
...
begin
  ...
  if not found then
    ...
    if exists(select 1 from catalog.datasets where project_id=v_req.project_id and name=v_dataset_name) then
    ...
    insert into catalog.dataset_versions(
      dataset_id,version_number,source_uri,content_hash,schema_hash,column_count,observed_at,status,metadata
    )
    values(
      v_dataset.id,1,v_source_identifier,v_asset.structure_hash,v_asset.structure_hash,
      jsonb_array_length(v_asset.columns),v_asset.last_seen_at,'AVAILABLE',
      jsonb_build_object(
        'discovered_asset_id',v_asset.id,
        'identity_key',v_req.identity_key,
    ...
    v_execution_config := jsonb_build_object(
      'source_id',v_source.id,
      'source_type',v_source.source_type,
      'connection_metadata',v_source.connection_metadata
    );

    if upper(coalesce(v_source.source_type,''))='JDBC' then
      v_execution_config := v_execution_config || jsonb_strip_nulls(jsonb_build_object(
        'jdbc_url',v_source.connection_metadata->>'jdbc_url',
        'credential_ref',v_source.connection_metadata->>'credential_ref',
        'connection_kind',v_source.connection_metadata->>'connection_kind'
      ));
    end if;

    insert into profiling.dataset_execution_sources(
      dataset_version_id,source_type,source_uri,execution_config,active
    )
    values(v_version_id,v_source.source_type,v_source_identifier,v_execution_config,true);
  end if;

  update catalog.asset_promotion_requests
  set status='PROMOTED',dataset_id=v_dataset.id,updated_at=now()
  where id=v_req.id;
