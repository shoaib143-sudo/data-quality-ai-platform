create table if not exists governance.platform_contract_check_runs (
...
create index if not exists platform_contract_check_project_idx on governance.platform_contract_check_runs(project_id,completed_at desc);
alter table governance.platform_contract_check_runs enable row level security;
drop policy if exists platform_contract_check_runs_read on governance.platform_contract_check_runs;
create policy platform_contract_check_runs_read on governance.platform_contract_check_runs
for select to authenticated using(app_private.is_project_member(project_id));
grant select on governance.platform_contract_check_runs to authenticated;
grant all on governance.platform_contract_check_runs to service_role;
...
begin
  ...
  select count(*) into v_dead_jobs from orchestration.job_queue
  where project_id=p_project_id and status='DEAD' and completed_at>=now()-interval '24 hours';
  ...
  v_checks:=jsonb_build_object(
    ...
    'dead_jobs_last_24h',jsonb_build_object('passed',v_dead_jobs=0,'count',v_dead_jobs),
...
$$;
revoke execute on function governance.run_platform_contract_checks(uuid) from public,anon,authenticated;
grant execute on function governance.run_platform_contract_checks(uuid) to service_role;

create or replace function governance.run_all_platform_contract_checks()
returns jsonb
language plpgsql
security definer
...
begin
  for r in select id from app.projects order by id loop
    v_results:=v_results||jsonb_build_array(governance.run_platform_contract_checks(r.id));
  end loop;
  return v_results;
end;
$$;
revoke execute on function governance.run_all_platform_contract_checks() from public,anon,authenticated;
grant execute on function governance.run_all_platform_contract_checks() to service_role;

do $$ declare v_job bigint; begin
  select jobid into v_job from cron.job where jobname='dgp-platform-contract-checks' limit 1;
  if v_job is not null then perform cron.unschedule(v_job); end if;
  perform cron.schedule('dgp-platform-contract-checks','17 * * * *','select governance.run_all_platform_contract_checks();');
end $$;
