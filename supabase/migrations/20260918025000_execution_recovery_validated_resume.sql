-- Queue a governed retry only after an autonomous repair has been independently validated.
-- Heavy processing jobs restart from the beginning of the failed durable job; other jobs
-- may resume at their validated retry stage/checkpoint. Browser roles cannot invoke this primitive.
...
declare
  ...
  v_action_id uuid;
  v_restart_scope text;
  v_effective_retry_checkpoint_id text;
...
begin
  ...
  v_restart_scope := case
    when v_job.job_type in ('PROFILING', 'DISCOVERY', 'DATA_QUALITY') then 'WHOLE_JOB'
    else 'FAILED_JOB'
  end;
  v_effective_retry_checkpoint_id := case
    when v_restart_scope = 'WHOLE_JOB' then null
    else nullif(trim(coalesce(p_retry_checkpoint_id, '')), '')
  end;
  ...
  if v_job.status = 'QUEUED' then
    ...
    return jsonb_build_object(
      ...
      'restart_scope', v_restart_scope,
      'retry_checkpoint_id', v_effective_retry_checkpoint_id
    );
  end if;
