...
begin
  ...
  begin
    ...
    select v_profile_id,md.id,md.metric_key,
      case md.metric_key
        when 'row_count' then 10::numeric
        when 'column_count' then 0::numeric
        when 'duplicate_row_count' then 0::numeric
        when 'duplicate_row_rate' then 0::numeric
        else null
      end,
    ...
    select count(*) into v_count from profiling.profile_metrics pm
