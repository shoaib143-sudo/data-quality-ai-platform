...
begin
  ...
  v_any_review_failure := not v_classification_rpc or not v_classification_trigger
    or not v_cde_rpc or not v_cde_trigger
    or not v_knowledge_rpc or not v_knowledge_trigger;

  v_checks := v_checks || jsonb_build_object(
    'human_review_boundary',
    jsonb_build_object(
      'status',case when not v_any_review_failure then 'PASS' else 'FAIL' end,
      'classification_review_rpc',v_classification_rpc,
      'classification_protection_trigger',v_classification_trigger,
      'cde_review_rpc',v_cde_rpc,
      'cde_protection_trigger',v_cde_trigger,
      'knowledge_document_review_rpc',v_knowledge_rpc,
      'knowledge_document_protection_trigger',v_knowledge_trigger,
      'policy','HUMAN_REVIEW_RPC_AND_PROTECTION_TRIGGER_REQUIRED'
    )
  );

  if v_any_review_failure and not v_existing_review_rpc_failure then
