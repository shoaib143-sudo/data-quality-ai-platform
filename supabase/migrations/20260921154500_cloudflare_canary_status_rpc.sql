-- Read-only operational status for the governed Cloudflare OBSERVABILITY canary.
-- Returns only non-secret state. The RPC is service-role only and is consumed
-- by the protected Infrastructure Admin server component.
...
as $function$
  with vault_state as (
    select
      bool_or(name = 'DGP_CLOUDFLARE_WORKER_URL' and coalesce(decrypted_secret,'') <> '') as worker_url_configured,
      bool_or(name = 'DGP_CLOUDFLARE_WORKER_SECRET' and coalesce(decrypted_secret,'') <> '') as worker_secret_configured,
      coalesce(
