...
begin
  ...
  v_checks := v_checks || jsonb_build_object('governance_control_intelligence',jsonb_build_object(
    'status',case when v_control_tables_ok and v_control_rpcs_ok and v_control_triggers_ok and not v_control_browser_dml and v_control_lifecycle_violations=0 then 'PASS' else 'FAIL' end,
    'mode',v_control_mode,
    ...
    'policy','PROPOSE_FROM_REQUIREMENTS_HUMAN_APPROVE_BEFORE_ACTIVE_DETERMINISTIC_EVIDENCE_EVALUATION'));
  if not v_control_tables_ok or not v_control_rpcs_ok or not v_control_triggers_ok or v_control_browser_dml or v_control_lifecycle_violations>0 then
