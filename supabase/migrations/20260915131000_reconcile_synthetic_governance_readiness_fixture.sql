...
declare
  ...
  v_source text;
  v_patched text;
  v_decl_anchor text := $decl$  v_suffix text := replace(gen_random_uuid()::text,'-','');
$decl$;
  v_decl_new text := $decl$  v_suffix text := replace(gen_random_uuid()::text,'-','');
  ...
  v_manifest_id uuid;
$decl$;
  v_fixture_compact text := $fixture$    insert into catalog.datasets(project_id,name,source_identifier,business_domain,metadata) values(v_project_id,'Synthetic Governed Dataset','profiling_validation.synthetic_customers','TEST',jsonb_build_object('synthetic',true,'profiling_ready',true)) returning id into v_dataset_id;
...
$fixture$;
  v_fixture_new text := $fixture$    insert into catalog.data_sources(project_id,name,source_type,connection_metadata,status)
...
begin
  ...
  if strpos(v_source,'v_data_source_id uuid;')>0
     and strpos(v_source,'Synthetic governed JDBC readiness fixture did not reach READY')>0 then
    return;
  end if;

  if strpos(v_source,v_decl_anchor)=0 then
    raise exception 'Synthetic governance suite declaration anchor changed; refusing live readiness reconciliation';
  end if;
  if strpos(v_source,v_fixture_compact)=0 then
    raise exception 'Synthetic governance suite compact profiling fixture changed; refusing live readiness reconciliation';
  end if;
