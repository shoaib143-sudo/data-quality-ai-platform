...
begin
  if v_url !~ '^https://[A-Za-z0-9.-]+(?::[0-9]+)?/api/jobs/worker$' then
  ...
  if v_id is null then
    perform vault.create_secret(
      ...
      'DataNexus Cloudflare canary worker endpoint',
      null
    );
  else
  ...
  if v_id is null then
    perform vault.create_secret(
      ...
      'DataNexus Cloudflare canary bearer credential',
      null
    );
  else
    perform vault.update_secret(
  ...
  if v_id is null then
    perform vault.create_secret(
      ...
      'DataNexus Cloudflare OBSERVABILITY canary scheduler switch',
      null
    );
  else
    perform vault.update_secret(
