-- V5 reconciliation: keep one canonical governed outcome authority and close the
-- remaining source-agent cross-project boundary at the database layer.

-- A superseded implementation was applied directly to the live database during
-- parallel V5 work. It is not part of the canonical repository history and has
-- no production rows. Clean installations therefore no-op here.
drop table if exists governance.autonomy_action_outcomes;

create or replace function governance.validate_autonomy_action_scope()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, governance, catalog, profiling, agent
as $$
declare
...
begin
  ...
  if new.source_agent_run_id is not null and not exists (
    select 1 from agent.agent_runs ar
    where ar.id = new.source_agent_run_id and ar.project_id = new.project_id
  ) then
    raise exception 'Autonomy source agent run is outside project scope';
  end if;
...
before insert or update of project_id,policy_id,action_key,target_type,target_id,input,source_agent_run_id
