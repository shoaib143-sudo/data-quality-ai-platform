...
const checks = [
  ...
  ['cited evidence survives normal per-domain provenance slicing', investigation.includes('requiredEvidenceIds: ReadonlySet<string>') && investigation.includes('index >= SOURCE_ROWS_PER_DOMAIN && !requiredEvidenceIds.has(id)') && investigation.includes('buildProvenance(input.context, referencedEvidence)')],
