...
const validatedResumeMigration = fs.readFileSync('supabase/migrations/20260918025000_execution_recovery_validated_resume.sql', 'utf8')
...
requireText(closedLoopContract, 'SECURITY_RELEVANT_REPAIR_REQUIRES_REVIEW', 'security repair fail-closed boundary')
...
requireText(closedLoopContract, 'shouldRediagnoseRecoveryFailure', 'bounded repair rediagnosis policy')
...
requireText(recoveryPersistence, 'advanceRecoveryAttemptAfterFailure', 'bounded retry state advancement')
requireText(recoveryPersistence, 'return executePersistedRecovery', 'bounded rediagnosis recursion')
...
requireText(recoveryPersistence, 'executePersistedRecoveryAndQueueResume', 'validated repair resume service')
requireText(recoveryPersistence, "post_repair_validation_result !== 'PASSED'", 'resume validation gate')
requireText(validatedResumeMigration, 'queue_execution_recovery_resume', 'internal validated resume primitive')
requireText(validatedResumeMigration, "v_case.severity not in ('P0', 'P1')", 'resume severity gate')
requireText(validatedResumeMigration, "post_repair_validation_result is distinct from 'PASSED'", 'database repair validation gate')
requireText(validatedResumeMigration, "action_type = 'AUTO_REPAIR'", 'repair evidence requirement')
requireText(validatedResumeMigration, "action_type = 'VALIDATE'", 'validation evidence requirement')
requireText(validatedResumeMigration, "action_type in ('RETRY', 'RESUME')", 'retry and resume terminal reconciliation')
requireText(validatedResumeMigration, "final_outcome = 'RECOVERED'", 'workflow success recovery completion')
requireText(validatedResumeMigration, 'RESUME_FAILED_AFTER_VALIDATED_REPAIR', 'failed resume escalation')
requireText(validatedResumeMigration, "'recoveryResume'", 'resume retry boundary payload')
requireText(validatedResumeMigration, "'retry_stage', p_retry_stage", 'retry stage propagation')
requireText(validatedResumeMigration, "'retry_checkpoint_id'", 'retry checkpoint propagation')
requireText(validatedResumeMigration, 'revoke all on function orchestration.queue_execution_recovery_resume', 'browser resume revocation')
