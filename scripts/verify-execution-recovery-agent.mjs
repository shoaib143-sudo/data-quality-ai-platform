...
const atomicClaimMigration = fs.readFileSync('supabase/migrations/20260918022500_execution_recovery_atomic_claim.sql', 'utf8')
...
requireText(recoveryPersistence, "claimReason ?? ''", 'concurrent repair claim reason normalization')
requireText(recoveryPersistence, "'ATTEMPT_IN_FLIGHT'", 'concurrent fenced repair in-flight replay')
...
requireText(recoveryRouting, "repairClass: 'LEASE_RECONCILIATION'", 'lease routing policy')
...
requireText(validatedResumeMigration, "v_restart_scope := case", 'canonical restart-scope derivation')
requireText(validatedResumeMigration, "when v_restart_scope = 'WHOLE_JOB' then null", 'whole-job checkpoint clearing')
requireText(validatedResumeMigration, "'retry_checkpoint_id', v_effective_retry_checkpoint_id", 'restart evidence uses effective checkpoint')
requireText(validatedResumeMigration, 'retry_checkpoint_id = v_effective_retry_checkpoint_id', 'recovery ledger uses effective checkpoint')
rejectText(validatedResumeMigration, "retry_checkpoint_id = nullif(trim(coalesce(p_retry_checkpoint_id", 'raw checkpoint persistence for whole-job restart')
requireText(recoveryPersistence, "result.retry_checkpoint_id === null", 'explicit null checkpoint preservation')
requireText(recoveryPersistence, 'retry_checkpoint_id: resume.retryCheckpointId', 'post-queue canonical checkpoint reconciliation')
requireText(validatedResumeMigration, "'restart_scope', v_restart_scope", 'idempotent restart-scope response evidence')
