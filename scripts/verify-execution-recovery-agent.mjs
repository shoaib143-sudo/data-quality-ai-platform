...
const connectorRecoveryHandler = fs.readFileSync('lib/orchestration/execution-recovery-connector-handler.ts', 'utf8')
const profilingRecoveryHandler = fs.readFileSync('lib/orchestration/execution-recovery-profiling-handler.ts', 'utf8')
const recoveryRegistry = fs.readFileSync('lib/orchestration/execution-recovery-registry.ts', 'utf8')
const recoveryPersistence = fs.readFileSync('lib/orchestration/execution-recovery-persistence.ts', 'utf8')
const recoveryIdempotencyMigration = fs.readFileSync('supabase/migrations/20260918113000_execution_recovery_action_idempotency.sql', 'utf8')
...
requireText(connectorRecoveryHandler, "failingStage === 'CONNECTOR_ESTABLISHMENT'", 'connector establishment recovery scope')
requireText(connectorRecoveryHandler, "knownRepairClass === 'TRANSIENT_RETRY'", 'connector transient retry boundary')
requireText(profilingRecoveryHandler, "failingStage === 'METRIC_EXECUTION'", 'metric execution recovery scope')
requireText(profilingRecoveryHandler, "profiling-executor:execute_metrics", 'existing profiling executor reuse')
requireText(recoveryRegistry, 'createConnectorRecoveryHandler()', 'connector handler registration')
requireText(recoveryRegistry, 'createSourceReadinessRecoveryHandler()', 'source readiness handler registration')
requireText(recoveryRegistry, 'createMetricExecutionRecoveryHandler()', 'metric handler registration')
requireText(recoveryPersistence, 'persistClosedLoopRecoveryRecord', 'canonical recovery record persistence')
requireText(recoveryPersistence, "actionType: 'AUTO_REPAIR'", 'automatic repair audit evidence')
requireText(recoveryPersistence, "actionType: 'VALIDATE'", 'repair validation audit evidence')
requireText(recoveryPersistence, "actionType: 'RESUME'", 'resume audit evidence')
requireText(recoveryIdempotencyMigration, 'recovery_actions_case_idempotency_idx', 'idempotent recovery action evidence')
