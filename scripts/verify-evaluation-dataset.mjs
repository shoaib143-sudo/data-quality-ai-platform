...
const checks = [
  ...
  [contract.includes("typeof learning.effective !== 'boolean'"), 'recommendation learning must carry an explicit verified positive or negative effectiveness label'],
  [contract.includes("learning.outcome.status !== 'VERIFIED'"), 'remediation outcome must be verified'],
  [contract.includes('hasCompleteVerificationChecks(learning.outcome.checks)'), 'deterministic verification checks must be complete with explicit boolean outcomes'],
  [contract.includes("typeof (value as Record<string, unknown>).passed === 'boolean'"), 'verification checks preserve both passing and failing verified outcomes'],
