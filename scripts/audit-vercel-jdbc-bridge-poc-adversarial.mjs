...
assert.equal(config.services?.jdbc?.runtime, 'container', 'adversarial: PoC must explicitly declare container runtime')
assert.equal(config.services?.jdbc?.entrypoint, 'Dockerfile.vercel', 'adversarial: Dockerfile.vercel must be the declared container entrypoint')
assert.deepEqual(config.rewrites, [{ source: '/(.*)', destination: { service: 'jdbc' } }], 'adversarial: public traffic must not bypass the JDBC service')
