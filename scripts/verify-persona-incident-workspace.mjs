...
assert.ok(routes.includes('governedIncident(issueId: string)'), 'Canonical route builder must define governed incident identity.')
assert.ok(contracts.includes("id: 'governed-incident'"), 'Governed incident dynamic route must be registered as a platform resource contract.')
assert.ok(contracts.includes("routeFile: 'app/issues/[issueId]/page.tsx'"), 'Resource contract must bind the incident detail route.')
assert.ok(issues.includes('Canonical incident journey'), 'Issues workspace must expose the governed incident journey.')
assert.ok(issues.includes('canonicalRoutes.governedIncident(issue.id)'), 'Issues workspace must use the canonical governed incident route builder.')
