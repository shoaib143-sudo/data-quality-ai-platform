...
for (const object of [
  ...
  requireText(repair, `grant select on ${object} to authenticated;`, `authenticated read-only grant for ${object}`)
  requireText(repair, `grant select on ${object} to service_role;`, `service-role read grant for ${object}`)
}

// Attack oracle: the repair must not gain a mutation, anonymous, public, or
// authority-bypass capability while making the legitimate read path operable.
for (const forbidden of [
  ...
  if (forbidden.test(repair)) failures.push(`forbidden grant-boundary weakening: ${forbidden}`)
}
