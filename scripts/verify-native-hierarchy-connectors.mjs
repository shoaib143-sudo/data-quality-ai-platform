...
contains('lib/connectors/native-hierarchy.ts', /excludedNodeIds/, 'Hierarchy selection must support explicit exclusions')
contains('lib/connectors/native-hierarchy.ts', /inheritFutureChildren/, 'Hierarchy selection must preserve dynamic future-child intent')
...
contains('lib/catalog/discovery.ts', /hierarchy_selection[\s\S]*discoverJdbcFromNativeHierarchy/, 'Catalog Discovery must honor saved and frozen native hierarchy scope')
...
contains('lib/catalog/native-jdbc-discovery.ts', /objectFields\(hierarchy\.nodes, object\)/, 'Included physical objects must capture their complete field definitions')
contains('lib/catalog/native-jdbc-discovery.ts', /metadata_discovery_field_scope:\s*'FULL_OBJECT'/, 'Field-level choices must not create intentionally incomplete physical metadata')
contains('lib/catalog/native-jdbc-discovery.ts', /native_identity:\s*identity/, 'Physical discovery must retain provider native identity evidence')
contains('lib/catalog/lineage-enrichment.ts', /discoverJdbcTransformations/, 'Connector lineage discovery must run as a downstream enrichment')
contains('lib/catalog/discovery.ts', /jobType:\s*'LINEAGE_ENRICHMENT'/, 'Physical publication must hand lineage off asynchronously')
