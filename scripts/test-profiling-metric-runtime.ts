...
async function createRun(id: string) {
  ...
  const { error: columnError } = await supabase
    ...
    .insert(columns.map((column) => ({ profile_run_id: id, ...column })))
...
async function metricsFor(runId: string) {
  const { data, error } = await supabase
    ...
    .select('profile_column_id,metric_key,numeric_value,text_value,json_value')
    .eq('profile_run_id', runId)
...
async function findingsFor(runId: string) {
  const { data, error } = await supabase
    ...
    .select('profile_column_id,finding_type,severity,confidence,evidence')
    .eq('profile_run_id', runId)
  assert.equal(error, null, error?.message)
  return data ?? []
}
...
async function assertPrimaryExecution() {
  ...
  assert.equal(result.score.completeness_score, 0.85)
  assert.equal(result.score.uniqueness_score, 0.7)
  assert.equal(result.score.validity_score, 1)
  assert.equal(result.score.overall_score, 0.85)
  ...
  assert.equal(numeric(score?.completeness_score), 0.85)
  assert.equal(numeric(score?.uniqueness_score), 0.7)
  assert.equal(numeric(score?.validity_score), 1)
  assert.equal(numeric(score?.overall_score), 0.85)
  assert.equal(state.status, 'COMPLETED')
  assert.ok(state.completed_at)
  assert.equal(numeric((state.summary as Record<string, any>)?.score?.overall_score), 0.85)
