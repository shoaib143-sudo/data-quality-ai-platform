...
const checks = [
  ...
  ['lib/profiling/remediation-reprofile.ts', /verificationGeneration[\s\S]*verification_generation[\s\S]*profiling:remediation-verification:\$\{outcome\.workflow_instance_id\}:\$\{verificationGeneration\}/, 'verification generations isolate restarted durable jobs'],
  ...
  ['lib/profiling/remediation-reprofile.ts', /verification_job_id:\s*durableJob\.id[\s\S]*JOB_ENQUEUED/, 'idempotent durable job linkage'],
  ...
  ['app/api/profiling/remediation/reprofile/route.ts', /VERIFICATION_CANCELLED[\s\S]*verification_attempt_history[\s\S]*nextGeneration[\s\S]*PROFILING_REMEDIATION_REPROFILE_RESTART_REQUESTED/, 'cancelled verification preserves prior attempt and advances generation'],
  ['app/api/profiling/remediation/reprofile/route.ts', /verification_profile_run_id:\s*null[\s\S]*verification_agent_run_id:\s*null[\s\S]*verification_job_id:\s*null[\s\S]*scheduleRemediationVerificationFromIssue/, 'cancelled restart clears old links before fresh scheduling'],
  ...
  ['lib/orchestration/worker.ts', /recordAutomaticVerificationCancellation[\s\S]*status:\s*'VERIFICATION_CANCELLED'[\s\S]*verification_cancelled:\s*true[\s\S]*PROFILING_REMEDIATION_AUTOMATIC_VERIFICATION_CANCELLED/, 'user cancellation is distinct and restartable'],
  ...
  ['app/api/profiling/remediation/verify/route.ts', /VERIFICATION_CANCELLED_RESTART_REQUIRED/, 'verify API blocks fallback after cancellation'],
  ...
  ['app/workflows/workflow-manager.tsx', /Automatic verification was cancelled[\s\S]*fresh verification generation/, 'workflow explains cancelled evidence preservation'],
  ['app/workflows/workflow-manager.tsx', /verificationRetryable[\s\S]*Automatic verification needs retry/, 'workflow distinguishes technical retry from cancellation'],
