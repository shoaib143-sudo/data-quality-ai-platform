...
const checks = [
  ...
  ['app/workflows/page.tsx', /from\('issues'\)[\s\S]*Profiling remediation:%[\s\S]*issues=/, 'workflow page loads tracked remediation issues'],
  ['app/workflows/workflow-manager.tsx', /Track remediation[\s\S]*Resolve with evidence[\s\S]*Check verification/, 'workflow remediation controls'],
  ...
  ['app/workflows/workflow-manager.tsx', /VERIFICATION_QUEUED[\s\S]*Automatic verification queued[\s\S]*verification_job_id/, 'workflow automatic verification state evidence'],
