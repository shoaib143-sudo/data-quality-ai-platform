...
const checks = [
  ...
  ['app/api/profiling/remediation/reprofile/route.ts', /workflowInstanceId[\s\S]*issues\.manage[\s\S]*remediation_issue_ids[\s\S]*scheduleRemediationVerificationFromIssue/, 'authorized workflow-keyed automatic verification retry'],
  ...
  ['lib/profiling/remediation-verification.ts', /profiling_remediation_outcomes[\s\S]*VERIFIED[\s\S]*VERIFICATION_FAILED[\s\S]*recommendation_effective/, 'verification effectiveness persistence'],
  ...
  ['app/workflows/workflow-manager.tsx', /\/api\/profiling\/remediation\/reprofile[\s\S]*Retry automatic verification/, 'workflow automatic verification retry control'],
  ['app/workflows/workflow-manager.tsx', /allProfileIssuesResolved[\s\S]*ACTION_TRACKED[\s\S]*VERIFICATION_QUEUED[\s\S]*!outcome\.verification_job_id/, 'workflow retry is limited to resolved remediation and missing queue linkage'],
  ...
  ['app/workflows/workflow-manager.tsx', /VERIFICATION_QUEUED/, 'workflow automatic verification queued state'],
