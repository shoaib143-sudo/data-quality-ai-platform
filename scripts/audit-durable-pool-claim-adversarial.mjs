...
assert.match(claimPath, /logClaimRpcEvidence\(\{/, 'pool claims must emit structured RPC timing evidence')
assert.match(claimPath, /first_elapsed_ms: firstElapsedMs/, 'pool evidence must capture initial RPC latency')
assert.match(claimPath, /retry_elapsed_ms: retryElapsedMs/, 'pool evidence must capture replay latency')
assert.match(claimPath, /retry_attempted: firstGatewayTimeout/, 'pool evidence must identify gateway retry attempts')
...
assert.match(outbox, /\[outbox-claim-rpc-evidence\]/, 'outbox claims must emit structured RPC timing evidence')
assert.match(outbox, /retry_attempted: firstGatewayTimeout/, 'outbox evidence must identify gateway retry attempts')
assert.deepEqual(vercelConfig.regions, ['sin1'], 'Vercel functions must remain colocated with the ap-southeast-1 Supabase data plane in Singapore')
