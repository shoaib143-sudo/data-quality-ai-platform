...
contains(recovery, "certification.rollbackStrategy === 'COMPENSATION_TOOL'", 'explicit compensation strategy routing')
contains(recovery, "certification.rollbackStrategy === 'ESCALATE_ONLY'", 'explicit escalation-only strategy routing')
contains(recovery, "'ROLLBACK_ESCALATION_REQUIRED'", 'rollback escalation code')
contains(recovery, "'ROLLBACK_CONTRACT_MISSING'", 'missing rollback contract fail closed')
