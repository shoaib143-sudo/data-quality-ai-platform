...
requireText(workerRoute, 'runAdaptiveWorkerCycle', 'worker adapter delegates adaptive convergence')
requireText(workerService, 'runAdaptiveWorkerCycle', 'job and outbox convergence loop')
requireText(workerService, 'processEvents: processOutboxEvents', 'event-driven outbox processing through isolated lane')
requireText(workerService, 'runOutboxLane', 'isolated outbox lane boundary')
requireText(workerService, 'eventLaneBlocked ? skippedOutboxLane() : await executeOutboxLane(eventWorkerId)', 'stop re-hitting degraded outbox lane within adaptive convergence')
