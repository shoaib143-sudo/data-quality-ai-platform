...
assert.ok(explorer.includes(".eq('verification_profile_run_id', latestRun.id)"), 'verification profile views must recover persisted remediation context')
assert.ok(explorer.includes(".eq('id', verificationOutcomeResult.data.workflow_instance_id)"), 'verification profile views must resolve the originating governance workflow')
assert.ok(explorer.includes(".eq('project_id', projectId)"), 'verification workflow recovery must remain project scoped')
