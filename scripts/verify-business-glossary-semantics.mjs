...
const evidence = fs.readFileSync('supabase/migrations/20260906101500_glossary_evidence_immutability.sql', 'utf8')
...
requireText(evidence, 'glossary_term_versions_append_only', 'semantic versions are DB append-only')
requireText(evidence, 'revoke insert,update,delete on governance.glossary_term_versions from anon,authenticated,service_role', 'service role cannot forge semantic history')
requireText(evidence, 'glossary_mapping_decisions', 'dedicated mapping decision evidence')
requireText(evidence, 'glossary_mapping_decisions_append_only', 'mapping decisions are append-only')
requireText(evidence, 'capture_glossary_mapping_decision', 'mapping decision capture is automatic')
requireText(evidence, 'zz_glossary_term_transactional_audit', 'term audit commits transactionally')
requireText(evidence, 'zz_glossary_mapping_transactional_audit', 'mapping audit commits transactionally')
requireText(evidence, 'verify_glossary_evidence_posture', 'formal evidence posture verification')
requireText(evidence, "last_changed_by=null", 'system invalidation does not inherit user identity')
...
requireText(termsRoute, 'DB triggers capture immutable semantic version + audit evidence', 'term creation relies on transactional evidence')
rejectText(termsRoute, "status:text(b.status)", 'caller-selected approval status')
rejectText(termsRoute, 'writeGovernanceAudit', 'best-effort audit helper used for glossary creation')
...
requireText(termRoute, 'Semantic version capture and audit insertion are DB triggers', 'term lifecycle evidence is transactional')
rejectText(termRoute, 'writeGovernanceAudit', 'best-effort audit helper used for glossary lifecycle')
...
requireText(mappingsRoute, 'last_changed_by: user.id', 'proposal actor is persisted for DB evidence capture')
rejectText(mappingsRoute, 'writeGovernanceAudit', 'best-effort audit helper used for mapping proposal')
...
requireText(mappingReviewRoute, 'last_changed_by: user.id', 'mapping review actor is persisted for DB evidence capture')
requireText(mappingReviewRoute, 'Review decision and audit evidence are inserted by DB triggers', 'mapping review evidence is transactional')
rejectText(mappingReviewRoute, 'writeGovernanceAudit', 'best-effort audit helper used for mapping review')
...
console.log('Business glossary, governed semantics, and immutable evidence contracts verified.')
