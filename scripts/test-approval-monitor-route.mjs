...
assert.ok(inbox.includes('disabled={busy !== null}'), 'execute and text controls must serialize approval mutations')
assert.ok(inbox.includes("disabled={busy !== null || !(comments[key] ?? '').trim()}"), 'approve/reject CTAs must remain disabled until a mandatory comment exists')
