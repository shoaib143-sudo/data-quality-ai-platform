...
begin
  ...
  if not coalesce((v_result->>'queued')::boolean, false) or v_result->>'reason' <> 'QUEUED' then
    raise exception 'Validated repair did not queue resume: %', v_result;
  end if;
  ...
  if v_status <> 'QUEUED'
     ...
     or v_payload#>>'{recoveryResume,retry_checkpoint_id}' <> 'metric-step' then
    raise exception 'Resume boundary was not persisted on durable job: status %, payload %', v_status, v_payload;
  end if;
  ...
  if not coalesce((v_result->>'queued')::boolean, false) or v_result->>'reason' <> 'ALREADY_QUEUED' then
    raise exception 'Resume replay was not idempotent: %', v_result;
  end if;
