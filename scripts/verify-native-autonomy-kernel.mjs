...
contains(runtime, 'certificationsByStepId.set(step.id, loaded.certification)', 'per-step immutable tool certification')
contains(kernel, 'input.certificationsByStepId.get(step.id)', 'per-step certification resolution')
contains(tests, 'stepCertifications = new Map', 'mixed specialist contract regression coverage')
...
contains(runtime, 'step.contractHash !== binding.contractHash', 'post-validation per-step contract drift guard')
contains(runtime, 'step.executorKey !== binding.executorKey', 'post-validation per-step executor drift guard')
contains(tests, 'duplicate step id: steward', 'duplicate step-local certification identity adversarial coverage')
