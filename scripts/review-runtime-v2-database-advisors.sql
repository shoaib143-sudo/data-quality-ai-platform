...
-- Service/control-plane tables surfaced by the current RLS-with-no-policy advisor.
...
-- Exact duplicate index detection by table, uniqueness/primary semantics,
-- full normalized definition and predicate.
with indexes as (
  select
...
normalized as (
  select
    *,
    regexp_replace(index_definition, 'INDEX [^ ]+ ON ', 'INDEX <name> ON ') as normalized_definition
  from indexes
)
select
  a.schema_name,
...
-- Advisor candidates only. idx_scan=0 is not evidence that an index is safe to drop.
