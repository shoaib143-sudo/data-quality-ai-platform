...
requireText(service, "if (!input.reason.trim()) throw new Error('Authority assignment reason is required.')", 'Direct authority assignment must reject a missing reason before any database call.')
requireText(service, "if (!input.projectId.trim()) throw new Error('Direct approval authority assignment must be project scoped.')", 'Direct authority assignment must reject an empty project scope.')
requireText(service, "if (!input.approverUserId.trim()) throw new Error('An approver is required.')", 'Direct authority assignment must reject an empty approver.')
requireText(service, "if (!input.domain.trim()) throw new Error('A governed project domain is required.')", 'Direct authority assignment must reject an empty domain.')
requireText(service, "if (!['BUSINESS','GOVERNANCE'].includes(input.approvalAxis)) throw new Error('Approval axis is invalid.')", 'Direct authority assignment must reject an invalid approval axis.')
requireText(service, "if (!(input.maxRisk in riskRank)) throw new Error('Authority maximum risk is invalid.')", 'Direct authority assignment must reject an invalid risk ceiling.')
...
requireText(service, "if (!actionKeys.length) throw new Error('Authority assignment must include at least one explicitly scoped action.')", 'Direct authority assignment must reject an empty action allowlist.')
requireText(service, 'for (const actionKey of actionKeys) getAgentActionProfile(actionKey)', 'Direct authority assignment must reject unknown governed actions through the canonical action catalog.')
...
requireText(service, "throw new Error('Approval domain is not governed by this project.')", 'Direct authority assignment must reject an unknown project domain.')
requireText(service, "if (!Number.isFinite(startsAt.getTime())) throw new Error('Authority start time is invalid.')", 'Direct authority assignment must reject invalid start timestamps.')
requireText(service, "if (endsAt && !Number.isFinite(endsAt.getTime())) throw new Error('Authority end time is invalid.')", 'Direct authority assignment must reject invalid expiry timestamps.')
requireText(service, "if (endsAt && endsAt <= startsAt) throw new Error('Authority end time must be after its start time.')", 'Direct authority assignment must reject non-positive validity windows.')
...
requireText(authorityMigration, 'Administrator and approver identities are required', 'Database assignment must reject missing actor or subject identities.')
...
requireText(authorityMigration, 'Direct approval authority assignment must be project scoped', 'Database assignment must reject project-less direct authority.')
requireText(authorityMigration, 'Approval domain is required', 'Database assignment must reject a blank domain.')
requireText(authorityMigration, 'Unsupported approval axis', 'Database assignment must reject an invalid approval axis.')
requireText(authorityMigration, 'Unsupported approval risk ceiling', 'Database assignment must reject an invalid risk ceiling.')
requireText(authorityMigration, 'Authority assignment reason is required', 'Database assignment must reject a blank reason.')
requireText(authorityMigration, 'Authority end time must be after its start time', 'Database assignment must reject invalid validity windows.')
...
requireText(authorityMigration, 'Approval project does not exist', 'Database assignment must reject an unknown project.')
