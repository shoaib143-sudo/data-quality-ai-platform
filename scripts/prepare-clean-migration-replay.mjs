...
function addReplayReconstruction(version, name, sql, reason) {
  ...
  fs.writeFileSync(path.join(targetDir, replay), `${sql.trim()}\n`)
  manifest.push({ source: null, replay, normalized: false, reconstructed: true, reason })
}

const reconstructions = [
  {
    version: '20260825235959',
    name: 'reconstruct_dataset_execution_sources',
    ...
    sql: `
      ...
      create unique index if not exists dataset_execution_sources_one_active_per_version
        on profiling.dataset_execution_sources(dataset_version_id) where active = true;
    `,
  },
  {
    version: '20260902021500',
    name: 'reconstruct_data_quality_scores',
    reason: 'Released history normalizes profiling.data_quality_scores before any recorded table creation; reconstruction matches the live table and constraint contract.',
    sql: `
  ...
  {
    version: '20260902041500',
    name: 'reconstruct_persist_profile_execution_result',
    reason: 'Released history revokes the profiling persistence RPC before its creation is represented; reconstruction uses the live signature and behavior so later hardening applies normally.',
    sql: `
      ...
      begin
        ...
        where not exists (
          select 1 from profiling.profile_findings e
          where e.profile_run_id=p_profile_run_id
  ...
  {
    version: '20260903005400',
    name: 'reconstruct_calculate_quality_score',
    reason: 'Released history hardens profiling.calculate_quality_score before its creation is represented; reconstruction uses the live function contract for disposable replay.',
    sql: `
      create or replace function profiling.calculate_quality_score(p_profile_run_id uuid)
      returns void language plpgsql set search_path = pg_catalog, profiling as $$
      declare
        v_score numeric;
        v_dimension_count integer;
      begin
        select
          coalesce(completeness_score,0)+coalesce(uniqueness_score,0)+coalesce(validity_score,0)+coalesce(accuracy_score,0),
        ...
        set overall_score=case when v_dimension_count>0 then round(v_score/v_dimension_count,4) else null end
        where profile_run_id=p_profile_run_id;
      end;
      $$;
    `,
  ...
  {
    version: '20260903005500',
    name: 'reconstruct_run_profile',
    reason: 'Released history hardens profiling.run_profile before its creation is represented; reconstruction uses the live function contract so subsequent migrations can harden and replace it.',
    sql: `
      create or replace function profiling.run_profile(p_dataset_version_id uuid)
      returns uuid language plpgsql set search_path = pg_catalog, profiling as $$
      declare
      ...
      begin
        ...
        values(v_profile_run_id,0,0,0,0) on conflict do nothing;
  ...
  {
    version: '20260903225900',
    name: 'reconstruct_create_file_dataset',
    reason: 'Released history revokes public.create_file_dataset before its creation is represented; reconstruction uses the live signature and behavior in disposable replay.',
    sql: `
      create or replace function public.create_file_dataset(
        p_project_id uuid, p_name text, p_filename text,
        p_description text default null, p_business_domain text default null
      ) returns table(
        dataset_id uuid, dataset_version_id uuid, data_source_id uuid,
        version_number bigint, storage_bucket text, storage_path text
      ) language plpgsql security definer
      set search_path = pg_catalog, public, app, catalog, app_private as $$
      declare
      ...
      begin
        ...
        values(p_project_id,v_source_id,btrim(p_name),nullif(btrim(coalesce(p_description,'')),''),v_filename,v_user_id,nullif(btrim(coalesce(p_business_domain,'')),''),'ACTIVE',jsonb_build_object('ingestion_type','FILE_UPLOAD'))
  ...
  {
    version: '20260903225910',
    name: 'reconstruct_create_organization',
    reason: 'Released history revokes public.create_organization before its creation is represented; reconstruction reproduces the live historical RPC only in disposable replay.',
    sql: `
      create or replace function public.create_organization(p_name text,p_slug text)
      returns table(id uuid,name text,slug text,role app.member_role)
      language plpgsql security definer set search_path=pg_catalog,public,app,app_private as $$
  ...
  {
    version: '20260903225920',
    name: 'reconstruct_create_project',
    reason: 'Released history revokes public.create_project before its creation is represented; reconstruction reproduces the live historical RPC only in disposable replay.',
    sql: `
  ...
  {
    version: '20260903225930',
    name: 'reconstruct_get_dataset_version_for_profiling',
    reason: 'Released history revokes public.get_dataset_version_for_profiling before its creation is represented; reconstruction reproduces the live historical RPC only in disposable replay.',
    sql: `
  ...
  {
    version: '20260904020500',
    name: 'reconstruct_lineage_edges',
    reason: 'Released governance operations history indexes governance.lineage_edges before its creation is represented; reconstruction matches the live pre-transformation table contract for disposable replay.',
    sql: `
  ...
  {
    version: '20260904022510',
    name: 'reconstruct_audit_events',
    reason: 'Released operational audit history enables RLS and writes governance.audit_events before its creation is represented; reconstruction matches the live pre-hash-chain table contract for disposable replay.',
    sql: `
  ...
  {
    version: '20260904044900',
    name: 'reconstruct_dataset_execution_registry',
    reason: 'Released governance write-boundary hardening applies policy to profiling.dataset_execution_registry before its creation is represented; reconstruction matches the live table and constraint contract.',
    sql: `
      create table if not exists profiling.dataset_execution_registry (
        id uuid primary key default gen_random_uuid(),
        dataset_version_id uuid not null unique references catalog.dataset_versions(id) on delete cascade,
        execution_type text not null default 'TABLE',
  ...
  {
    version: '20260904044910',
    name: 'reconstruct_dataset_row_access_registry',
    reason: 'Released governance write-boundary hardening applies policy to profiling.dataset_row_access_registry before its creation is represented; reconstruction matches the live table contract.',
    sql: `
      create table if not exists profiling.dataset_row_access_registry (
        id uuid primary key default gen_random_uuid(),
        dataset_version_id uuid not null,
        relation_name text not null,
        access_type text not null default 'POSTGRES_TABLE',
        created_at timestamptz default now()
      );
    `,
  },
...
for (const reconstruction of reconstructions) {
  addReplayReconstruction(reconstruction.version, reconstruction.name, reconstruction.sql, reconstruction.reason)
}
...
for (const file of files) {
  ...
  if (file === '20260920014000_proactive_governed_case_learning.sql') {
