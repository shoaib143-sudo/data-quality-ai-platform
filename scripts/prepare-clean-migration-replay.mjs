...
function addReplayReconstruction(version, name, sql, reason) {
  if (originalVersions.has(version) || assignedVersions.has(version)) throw new Error(`Clean replay reconstruction version collides with released/assigned migration ${version}`)
...
addReplayReconstruction('20260825235959','reconstruct_dataset_execution_sources',`begin;
create table if not exists profiling.dataset_execution_sources (
  id uuid primary key default gen_random_uuid(), dataset_version_id uuid not null references catalog.dataset_versions(id) on delete cascade,
  source_type text not null, source_uri text, execution_config jsonb not null default '{}'::jsonb, active boolean not null default true,
...
addReplayReconstruction('20260902021500','reconstruct_data_quality_scores',`begin;
create table if not exists profiling.data_quality_scores (
  id uuid primary key default gen_random_uuid(), profile_run_id uuid not null references profiling.profile_runs(id) on delete cascade,
  completeness_score numeric, uniqueness_score numeric, validity_score numeric, accuracy_score numeric, overall_score numeric,
  created_at timestamptz not null default now(), constraint profile_quality_scores_unique_run unique (profile_run_id)
);
commit;
...
addReplayReconstruction('20260902041500','reconstruct_persist_profile_execution_result',`create or replace function profiling.persist_profile_execution_result(
  p_profile_run_id uuid, p_metrics jsonb default '[]'::jsonb, p_findings jsonb default '[]'::jsonb,
  p_score jsonb default '{}'::jsonb, p_status text default 'COMPLETED'
) returns void language plpgsql security definer set search_path = pg_catalog, profiling as $$
begin
  ...
  from jsonb_array_elements(p_metrics) m on conflict (profile_run_id,profile_column_id,metric_key) do update set numeric_value=excluded.numeric_value,text_value=excluded.text_value,boolean_value=excluded.boolean_value,json_value=excluded.json_value;
  ...
  from jsonb_array_elements(p_findings) f where not exists (select 1 from profiling.profile_findings e where e.profile_run_id=p_profile_run_id and e.finding_type=f->>'finding_type' and e.title=f->>'title');
...
addReplayReconstruction('20260903005400','reconstruct_calculate_quality_score',`create or replace function profiling.calculate_quality_score(p_profile_run_id uuid) returns void language plpgsql set search_path = pg_catalog, profiling as $$
declare v_score numeric; v_dimension_count integer;
begin
...
addReplayReconstruction('20260903005500','reconstruct_run_profile',`create or replace function profiling.run_profile(p_dataset_version_id uuid) returns uuid language plpgsql set search_path = pg_catalog, profiling as $$
...
begin
  ...
  perform profiling.execute_metrics(v_profile_run_id); perform profiling.generate_findings(v_profile_run_id);
  ...
  update profiling.profile_runs set status='COMPLETED',completed_at=now() where id=v_profile_run_id; return v_profile_run_id;
exception when others then update profiling.profile_runs set status='FAILED',completed_at=now() where id=v_profile_run_id; raise;
end; $$;
...
addReplayReconstruction('20260903225900','reconstruct_create_file_dataset',`create or replace function public.create_file_dataset(p_project_id uuid,p_name text,p_filename text,p_description text default null,p_business_domain text default null)
...
language plpgsql security definer set search_path = pg_catalog,public,app,catalog,app_private as $$
declare v_user_id uuid:=auth.uid(); v_dataset_id uuid; v_source_id uuid; v_version_id uuid; v_version_number bigint; v_filename text;
begin
  ...
  insert into catalog.data_sources(project_id,name,source_type,connection_metadata,status) values(p_project_id,btrim(p_name)||' upload','FILE_UPLOAD',jsonb_build_object('storage_bucket','dataset-files'),'ACTIVE') returning id into v_source_id;
  insert into catalog.datasets(project_id,data_source_id,name,description,source_identifier,owner_user_id,business_domain,status,metadata) values(p_project_id,v_source_id,btrim(p_name),nullif(btrim(coalesce(p_description,'')),''),v_filename,v_user_id,nullif(btrim(coalesce(p_business_domain,'')),''),'ACTIVE',jsonb_build_object('ingestion_type','FILE_UPLOAD')) returning id into v_dataset_id;
  select coalesce(max(version_number),0)+1 into v_version_number from catalog.dataset_versions where dataset_id=v_dataset_id;
  insert into catalog.dataset_versions(dataset_id,version_number,source_uri,status,metadata) values(v_dataset_id,v_version_number,'supabase://dataset-files/'||p_project_id::text||'/'||v_dataset_id::text||'/'||v_version_number::text||'/'||v_filename,'PROCESSING',jsonb_build_object('storage_bucket','dataset-files','storage_path',p_project_id::text||'/'||v_dataset_id::text||'/'||v_version_number::text||'/'||v_filename,'original_filename',p_filename,'created_by',v_user_id)) returning id into v_version_id;
...
addReplayReconstruction('20260903225910','reconstruct_create_organization',`create or replace function public.create_organization(p_name text,p_slug text) returns table(id uuid,name text,slug text,role app.member_role) language plpgsql security definer set search_path=pg_catalog,public,app,app_private as $$
...
addReplayReconstruction('20260903225920','reconstruct_create_project',`create or replace function public.create_project(p_organization_id uuid,p_name text,p_slug text,p_description text default null) returns table(id uuid,organization_id uuid,name text,slug text,description text) language plpgsql security definer set search_path=pg_catalog,public,app,app_private as $$
...
addReplayReconstruction('20260903225930','reconstruct_get_dataset_version_for_profiling',`create or replace function public.get_dataset_version_for_profiling(dataset_version_id uuid) returns setof catalog.dataset_versions language sql security definer set search_path=public as $$ select * from catalog.dataset_versions where id=dataset_version_id; $$;
...
addReplayReconstruction('20260904020500','reconstruct_lineage_edges',`create table if not exists governance.lineage_edges (
