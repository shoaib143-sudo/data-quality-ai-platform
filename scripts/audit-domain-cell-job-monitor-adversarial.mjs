...
assert.ok(monitor.includes("const visibleRuns = cell.componentRuns"), 'all latest component executions must be represented inside the domain cell')
...
assert.ok(monitor.includes("organicNodePosition(index, visibleRuns.length)"), 'organic layout must deterministically scale placement from actual component count')
...
console.log('Independent adversarial Domain Cell audit passed: no project-as-domain substitution, no cross-project merge, no hidden unassigned mapping, no component truncation, deterministic topology, density adaptation, and latest-state semantics.')
