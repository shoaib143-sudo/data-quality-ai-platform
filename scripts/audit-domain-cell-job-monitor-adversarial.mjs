...
assert.ok(monitor.includes("url.hash = ''"), 'domain drilldown must clear stale log anchors instead of inheriting job-log navigation')
assert.ok(monitor.includes("selectedCell.progressPercent"), 'domain drilldown must expose progress at the domain level')
assert.ok(monitor.includes("inspectorTab === 'DATASETS'"), 'domain drilldown must expose dataset detail without fabricating catalog state')
