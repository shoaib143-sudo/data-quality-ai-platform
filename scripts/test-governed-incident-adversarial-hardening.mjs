...
assert.match(timestampMigration, /revoke insert, update, delete, truncate, references, trigger[\s\S]*governance\.issues[\s\S]*from authenticated/i)
assert.match(timestampMigration, /create policy issues_project_read[\s\S]*for select[\s\S]*to authenticated/i)
assert.match(timestampMigration, /revoke insert, update, delete, truncate, references, trigger[\s\S]*governance\.issue_comments[\s\S]*from authenticated/i)
assert.match(timestampMigration, /create policy issue_comments_project_read[\s\S]*for select[\s\S]*to authenticated/i)
assert.doesNotMatch(timestampMigration, /create policy issues_project_read[\s\S]*for all/i)
assert.doesNotMatch(timestampMigration, /create policy issue_comments_project_read[\s\S]*for all/i)
