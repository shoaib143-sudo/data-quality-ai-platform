...
const checks = [
  ...
  ['app/api/search/route.ts', /createGovernanceRetrievalProvider[\s\S]*modes:\s*\['semantic'\][\s\S]*retrieved\.matches\.map\(semanticResult\)[\s\S]*mergeResults/, 'hybrid lexical and semantic governance search through RetrievalProvider'],
