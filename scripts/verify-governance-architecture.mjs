...
const requiredFiles = [
  ...
  'app/search/page.tsx',
  'app/search/global-search.tsx',
  'app/profiling/explorer/page.tsx',
  'app/profiling/profiling-explorer.tsx',
...
const checks = [
  ...
  ['lib/governance/semantic-search.ts', /EMBEDDING_DIMENSIONS\s*=\s*384[\s\S]*semanticSearchByEmbedding[\s\S]*match_semantic_embeddings[\s\S]*indexSemanticObject/, 'provider-neutral reusable semantic search and indexing'],
  ...
  ['app/api/search/route.ts', /createGovernanceRetrievalProvider[\s\S]*modes:\s*\['semantic'\][\s\S]*retrieved\.matches[\s\S]*\.map\(semanticResult\)[\s\S]*mergeResults/, 'hybrid lexical and semantic governance search through RetrievalProvider'],
  ['app/api/search/route.ts', /'NOT_CONFIGURED'\s*\|\s*'UNAVAILABLE'[\s\S]*EmbeddingProviderNotConfiguredError/, 'semantic search fallback states'],
  ...
  ['app/api/search/semantic/reindex/route.ts', /catalog\.update[\s\S]*reindexProjectSemanticCorpusWithRuntime/, 'authorized semantic reindex endpoint'],
  ['lib/governance/semantic-project-reindex-runtime.ts', /reindexProjectSemanticObjects[\s\S]*governance:\s*reindexProjectSemanticObjects/, 'governed semantic reindex runtime wiring'],
