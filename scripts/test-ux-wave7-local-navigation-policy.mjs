...
assert.ok(profilingPage.includes("canAccessWorkspaceHref(landing.persona, '/monitoring'"), 'Profiling monitoring access must derive server-side from workspace policy')
assert.ok(profilingPage.includes("canAccessWorkspaceHref(landing.persona, '/data-quality'"), 'Profiling Data Quality access must derive server-side from workspace policy')
assert.ok(profilingPage.includes("canAccessWorkspaceHref(landing.persona, '/profiling/explorer'"), 'Profiling Explorer access must derive server-side from workspace policy')
assert.ok(profilingPage.includes('canMonitoring={canMonitoring}'), 'Profiling page must pass monitoring authorization into the client dashboard')
assert.ok(profilingPage.includes('canQuality={canQuality}'), 'Profiling page must pass quality authorization into the client dashboard')
assert.ok(profilingPage.includes('canExplorer={canExplorer}'), 'Profiling page must pass explorer authorization into the client dashboard')
