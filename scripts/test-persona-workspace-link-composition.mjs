...
assert.match(physicalAssetsPage,/homeHref=\{canDashboard \? '\/dashboard' : '\/home'\}/,'Physical Assets Product Shell destination must not send a denied persona to Dashboard.')
...
assert.ok(physicalAssetsPage.includes('<GlobalUtilityBar'),'Physical Assets must use the canonical Product Shell branding.')
