...
assert.match(physicalAssetsPage,/homeHref=\{canDashboard \? '\/dashboard' : '\/home'\}/,'Physical Assets Product Shell home destination must not send a denied persona to Dashboard.')
...
assert.match(physicalAssetsPage,/<GlobalUtilityBar[^>]*roleLabel="Physical Metadata Assets"/,'Physical Assets must use the shared Product Shell with canonical product branding.')
