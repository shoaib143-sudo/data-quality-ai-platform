...
export function GlobalSearch(){
  ...
  const [message,setMessage]=useState('Search across datasets, columns, findings, glossary terms, issues, classifications, policies, contracts and lineage transformations.')
  ...
  async function search(event:FormEvent){
    ...
    try{
      ...
      const suffix=status==='ENABLED'?' Semantic ranking is active.':status==='NOT_CONFIGURED'?' Using lexical search until the embedding service is configured.':status==='UNAVAILABLE'?' Semantic ranking is temporarily unavailable; lexical results are shown.':''
