...
export default async function MonitoringPage({searchParams}: {searchParams: Promise<{run?: string; branch?: string; project?: string}>}) {
  ...
  const canExecute = projectId
    ? await authorizeProject(user.id, projectId, 'agent.execute').then(() => true).catch(() => false)
    : false
  return <main className="min-h-screen p-4 sm:p-8"><div className="mx-auto max-w-[1600px] space-y-5">
    <header className="flex flex-wrap items-center justify-between gap-4"><div><Link href="/dashboard">← Dashboard</Link><h1 className="mt-3 text-3xl font-bold">Job Monitor</h1></div>{canExecute ? <nav className="flex gap-4"><Link href="/recovery">Execution Recovery</Link><Link href="/agents">Run an agent</Link></nav> : null}</header>
