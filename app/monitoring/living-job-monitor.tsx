  ...
  return <section className="living-monitor">
    ...
    {!runId ? <p className="tree-row">{runs.length ? 'Select an execution to inspect its tree.' : 'No executions available for this project and filter.'}</p> : !snapshot ? <p className="tree-row">{error ? 'Use Refresh to retry, or select another execution.' : 'Loading execution hierarchy…'}</p> : <div className="tree-grid">
      ...
      <aside className="tree-panel" aria-label="Selected branch details">
        ...
        <h4>Execution steps</h4>{detailResult.error ? <div role="alert" className="tree-warning"><p>{detailResult.error}</p><button className="tree-action" onClick={refresh}>Retry branch details</button></div> : detail ? (detail.steps.length ? detail.steps.map(s => <div className="tree-row" key={s.id}><strong>{s.step_name}</strong><p>{s.status} · Attempt {s.attempt}</p><p className="tree-subtle">{timestamp(s.started_at)} to {timestamp(s.completed_at)}</p>{s.error_code ? <p>{s.error_code}</p> : null}</div>) : <p>No steps recorded on this page.</p>) : <p role="status">Loading details…</p>}
