...
export default async function IssuesPage() {
  ...
  const projects = await supabase.schema('app').from('projects')
    .select('id,name,organization_id')
    .eq('organization_id', landing.organizationId)
    .order('name')
  if (projects.error) throw new Error(projects.error.message)

  const projectIds = (projects.data ?? []).map(project => String(project.id))
  const empty = { data: [], error: null }
  const [datasets, issues, members] = await Promise.all([
    projectIds.length ? supabase.schema('catalog').from('datasets').select('id,project_id,name').in('project_id', projectIds).order('name') : Promise.resolve(empty),
    projectIds.length ? supabase.schema('governance').from('issues').select('*,issue_comments(*)').in('project_id', projectIds).order('created_at', { ascending: false }) : Promise.resolve(empty),
    supabase.schema('app').from('organization_members').select('organization_id,user_id,role').eq('organization_id', landing.organizationId).order('created_at'),
  ])
  for (const result of [datasets, issues, members]) if (result.error) throw new Error(result.error.message)

  const manageableProjectIds = (await Promise.all((projects.data ?? []).map(async project => [String(project.id), await hasProjectCapability(user.id, String(project.id), 'issues.manage')] as const)))
