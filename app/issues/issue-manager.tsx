...
export function IssueManager({ projects, datasets, members, initialIssues, manageableProjectIds, presentation }: {
  ...
  return <div className={`mt-6 grid gap-6 ${manageableProjects.length ? 'lg:grid-cols-[0.75fr_1.25fr]' : 'lg:grid-cols-1'}`}>
    {manageableProjects.length ? <form onSubmit={create} className="rounded-3xl border border-white/10 bg-[#0a1d33] p-6 shadow-sm"><h2 className="text-xl font-bold text-white">Create remediation issue</h2><div className="mt-5 grid gap-3"><select aria-label="Governed project" value={projectId} onChange={e => { setProjectId(e.target.value); setDatasetId('') }} className="rounded-xl border border-white/10 bg-[#08182b] px-3 py-2.5 text-slate-200">{manageableProjects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><select aria-label="Governed dataset" value={effectiveDatasetId} onChange={e => setDatasetId(e.target.value)} className="rounded-xl border border-white/10 bg-[#08182b] px-3 py-2.5 text-slate-200"><option value="">Project-level issue</option>{projectDatasets.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select><input aria-label="Issue title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Issue title" className="rounded-xl border border-white/10 bg-[#08182b] px-3 py-2.5 text-slate-200"/><textarea aria-label="Description and business impact" value={description} onChange={e => setDescription(e.target.value)} placeholder="Description and business impact" rows={4} className="rounded-xl border border-white/10 bg-[#08182b] px-3 py-2.5 text-slate-200"/><select aria-label="Issue severity" value={severity} onChange={e => setSeverity(e.target.value)} className="rounded-xl border border-white/10 bg-[#08182b] px-3 py-2.5 text-slate-200"><option>INFO</option><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select><button disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-3 font-bold text-white hover:bg-amber-500 disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin"/> : <Plus className="h-4 w-4"/>}Create issue</button>{message ? <p className="text-sm text-slate-400" role="status" aria-live="polite">{message}</p> : null}</div></form> : null}
