...
function severityClass(value:string){const normalized=value.toUpperCase();if(normalized==='CRITICAL'||normalized==='HIGH')return 'border-rose-200 bg-rose-50 text-rose-700';if(normalized==='MEDIUM')return 'border-amber-200 bg-amber-50 text-amber-700';return 'border-blue-200 bg-blue-50 text-blue-700'}
function EmptyState({children}:{children:string}){return <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">{children}</div>}
...
export default async function InboxPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 text-slate-950">
    <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      ...
      <section className="dn-surface p-5 sm:p-6">
      ...
      {summaryCards.length?<section className="mt-4 grid auto-rows-fr gap-3 md:grid-cols-2 2xl:grid-cols-4">{summaryCards.map(({label,value,Icon,href})=><Link key={label} href={href} className="dn-kpi dn-interactive flex min-h-[126px] min-w-0 flex-col p-4 focus-visible:outline-none"><Icon className="h-5 w-5 text-blue-600" aria-hidden="true"/><p className="mt-4 text-3xl font-black">{value}</p><p className="text-sm font-semibold text-slate-500">{label}</p></Link>)}</section>:null}

      <section className="mt-4 grid gap-4 xl:grid-cols-2">
        {canWorkflows?<article className="dn-surface p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><GitBranch className="h-5 w-5 text-violet-600"/><h2 className="text-xl font-black">Approvals and workflows</h2></div><p className="mt-1 text-sm text-slate-500">Pending workflow instances that require continued governance action.</p></div><Link href="/workflows" className="text-sm font-bold text-blue-600">View all</Link></div><div className="mt-4 space-y-2">{workflows.length?workflows.slice(0,8).map(workflow=>{const definition=definitionById.get(workflow.workflow_definition_id);return <Link key={workflow.id} href={`/workflows?instanceId=${encodeURIComponent(workflow.id)}`} className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-blue-200 hover:bg-blue-50"><div className="flex items-center justify-between gap-3"><span className="font-bold">{definition?.name??'Governance workflow'}</span><span className="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-bold text-violet-700">{workflow.status}</span></div><p className="mt-2 text-xs text-slate-500">Current step {workflow.current_step+1} · updated {when(workflow.updated_at??workflow.started_at)}</p></Link>}):<EmptyState>No pending workflow actions are visible.</EmptyState>}</div></article>:null}

        {canIssues?<article className="dn-surface p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Wrench className="h-5 w-5 text-amber-600"/><h2 className="text-xl font-black">Remediation work</h2></div><p className="mt-1 text-sm text-slate-500">Open governed issues, with overdue work surfaced first.</p></div><Link href="/issues" className="text-sm font-bold text-blue-600">View all</Link></div><div className="mt-4 space-y-2">{issues.length?[...issues].sort((a,b)=>Number(Boolean(b.due_at&&new Date(b.due_at).getTime()<Date.now()))-Number(Boolean(a.due_at&&new Date(a.due_at).getTime()<Date.now()))).slice(0,8).map(issue=>{const overdue=Boolean(issue.due_at&&new Date(issue.due_at).getTime()<Date.now());return <Link key={issue.id} href={`/issues/${encodeURIComponent(issue.id)}`} className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-blue-200 hover:bg-blue-50"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-bold">{issue.title}</span><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${severityClass(issue.severity)}`}>{issue.severity}</span></div><p className="mt-2 text-xs text-slate-500">{overdue?'Overdue':issue.status} · updated {when(issue.updated_at)}</p></Link>}):<EmptyState>No open remediation issues are visible.</EmptyState>}</div>{overdueIssues.length?<p className="mt-4 text-xs font-semibold text-rose-700">{overdueIssues.length} issue{overdueIssues.length===1?'':'s'} currently overdue.</p>:null}</article>:null}

        {canObservability?<article className="dn-surface p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-rose-600"/><h2 className="text-xl font-black">Risk signals</h2></div><p className="mt-1 text-sm text-slate-500">Persisted observability alerts requiring review.</p></div><Link href="/observability#alerts" className="text-sm font-bold text-blue-600">View all</Link></div><div className="mt-4 space-y-2">{alerts.length?alerts.slice(0,8).map(alert=><Link key={alert.id} href="/observability#alerts" className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-blue-200 hover:bg-blue-50"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-bold">{alert.title}</span><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${severityClass(alert.severity)}`}>{alert.severity}</span></div><p className="mt-2 text-xs text-slate-500">{alert.category.replaceAll('_',' ')} · observed {when(alert.last_observed_at)}</p></Link>):<EmptyState>No unresolved observability alerts are visible.</EmptyState>}</div></article>:null}

        {canMonitoring?<article className="dn-surface p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Activity className="h-5 w-5 text-blue-600"/><h2 className="text-xl font-black">Execution attention</h2></div><p className="mt-1 text-sm text-slate-500">Failed and active governed agent runs.</p></div><Link href="/monitoring" className="text-sm font-bold text-blue-600">Job monitor</Link></div><div className="mt-4 space-y-2">{jobs.length?jobs.slice(0,8).map(job=><Link key={job.id} href={`/monitoring?run=${encodeURIComponent(job.id)}`} className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-blue-200 hover:bg-blue-50"><div className="flex items-center justify-between gap-3"><span className="font-bold">{job.status==='FAILED'?'Agent run failed':'Agent run in progress'}</span><ExecutionStatusBadge status={job.status}/></div><p className="mt-2 text-xs text-slate-500">{job.error_code?`Error ${job.error_code} · `:''}{when(job.started_at??job.created_at)}</p></Link>):<EmptyState>No failed or active agent runs are visible.</EmptyState>}</div>{!failedJobs.length&&!activeJobs.length?<div className="mt-4 flex items-center gap-2 text-xs font-semibold text-emerald-700"><CheckCircle2 className="h-4 w-4"/>Execution queue is clear.</div>:null}</article>:null}
      </section>

      <footer className="mt-4 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-3 text-xs text-slate-500"><Clock3 className="h-4 w-4" aria-hidden="true"/>Inbox contents are derived from persisted governance evidence and current access scope. No synthetic notifications are created.</footer>
    </div>
  </main>
}
