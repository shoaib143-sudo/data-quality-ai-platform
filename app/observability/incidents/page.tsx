...
function slaState(incident:Incident,now:number){if(incident.status==='RESOLVED'||!incident.response_due_at)return null;const due=Date.parse(incident.response_due_at);if(!Number.isFinite(due))return{overdue:false,minutes:null,due:null,invalid:true};const minutes=Math.round((due-now)/60000);return{overdue:minutes<0,minutes:Math.abs(minutes),due,invalid:false}}
...
export default async function IncidentsPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="AI Operations Center" contextLabel="Correlated observability incidents" homeHref="/home" />
    <nav className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-white px-5 py-3 shadow-sm"><Link href="/observability" className="font-bold text-blue-700">← Observability</Link><div className="flex gap-2">{canLineage?<Link href="/lineage/impact" className="rounded-xl border px-3 py-2 text-sm font-semibold">Impact analysis</Link>:null}{canWorkflows?<Link href="/workflows" className="rounded-xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Response approvals</Link>:null}</div></nav>
    ...
    <section className="mt-6 space-y-4">{typed.length?typed.map(incident=>{const causes=Array.isArray(incident.probable_root_causes)?incident.probable_root_causes:[];const recommendations=Array.isArray(incident.recommendations)?incident.recommendations:[];const impacts=(impactsByIncident.get(incident.id)??[]).slice(0,8);const correlations=(correlationsByIncident.get(incident.id)??[]).slice(0,6);const sla=slaState(incident,now);return <article key={incident.id} className="rounded-3xl border bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">{datasetById.get(incident.dataset_id)?.name??incident.dataset_id.slice(0,8)}</p><h2 className="mt-1 text-xl font-bold">{incident.title}</h2><p className="mt-2 max-w-4xl text-sm text-slate-600">{incident.summary}</p></div><div className="flex flex-wrap gap-2"><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${tone(incident.severity)}`}>{incident.severity}</span><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${tone(incident.status)}`}>{incident.status}</span>{sla?<span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${sla.invalid?'border-amber-200 bg-amber-50 text-amber-700':sla.overdue?'border-red-200 bg-red-50 text-red-700':'border-blue-200 bg-blue-50 text-blue-700'}`}>{sla.invalid?'Invalid SLA due date':sla.overdue?`${sla.minutes}m overdue`:`${sla.minutes}m to SLA`}</span>:null}{incident.escalation_level>0?<span className="rounded-full border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-bold text-orange-700">Escalation L{incident.escalation_level}</span>:null}</div></div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 border-t pt-4 text-sm">{incident.approval_required?<><ShieldCheck className="h-4 w-4 text-amber-600"/><span className="font-semibold text-amber-700">Governed approval required</span>{canWorkflows?<Link href="/workflows" className="font-bold text-violet-700">Open workflow →</Link>:null}</>:<><CheckCircle2 className="h-4 w-4 text-emerald-600"/><span className="text-slate-500">No critical response approval required</span></>}<span className="ml-auto text-xs text-slate-400">{incident.response_due_at?`Due ${Number.isFinite(Date.parse(incident.response_due_at))?new Date(incident.response_due_at).toLocaleString():'Invalid timestamp'} · `:''}Confidence {incident.confidence===null?'n/a':`${Math.round(incident.confidence*100)}%`}</span></div>
