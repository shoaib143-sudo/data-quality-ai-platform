...
import { Activity, Bot, BrainCircuit, CheckCircle2, CircleDashed, DatabaseZap, ShieldCheck } from 'lucide-react'
...
export default async function AICapabilitiesPage({ searchParams }: { searchParams: Promise<{ projectId?: string; status?: string }> }) {
  ...
  const domainCoverage = [...matrix.reduce((domains, row) => {
    const current = domains.get(row.evidence_domain) ?? { total: 0, evidenced: 0 }
    current.total += 1
    if (row.status === 'EVIDENCED') current.evidenced += 1
    domains.set(row.evidence_domain, current)
    return domains
  }, new Map<string,{total:number;evidenced:number}>()).entries()]
    ...
    <main id="main-content" tabIndex={-1} className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/50 p-5 sm:p-8">
      <div className="mx-auto max-w-7xl space-y-7">
        ...
        <nav aria-label="AI governance workspace" className="flex items-center justify-between gap-3 overflow-x-auto rounded-2xl border bg-white p-2 shadow-sm">
          <span className="shrink-0 px-2 text-xs font-black uppercase tracking-[.14em] text-slate-400">AI governance</span>
          <div className="flex shrink-0 gap-1">{canAiInsights ? <Link href="/ai-insights" className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Ask & investigate</Link> : null}<Link href="/agents" className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Agent registry</Link><Link href="/ai-capabilities" aria-current="page" className="rounded-xl bg-violet-50 px-3 py-2 text-sm font-bold text-violet-700">AI posture</Link><Link href="/monitoring" className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Activity</Link></div>
        </nav>

        <header className="rounded-3xl border border-blue-100 bg-white p-7 shadow-sm"><div className="flex items-start gap-4"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-blue-600 text-white"><BrainCircuit className="h-6 w-6" /></span><div><p className="text-xs font-black uppercase tracking-[.14em] text-violet-600">AI posture & controls</p><h1 className="mt-1 text-3xl font-black tracking-tight">AI Capability Control Center</h1><p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">Evidence coverage, governance knowledge readiness and operational capability posture for DataNexus AI. A green capability means qualifying project evidence exists. It does not mean the capability executes on every dataset or that AI has governance authority.</p></div></div></header>
        ...
        <section className="grid gap-4 lg:grid-cols-3"><article className="rounded-2xl border bg-white p-6"><h2 className="text-lg font-black">Governance knowledge readiness</h2><p className="mt-2 text-sm leading-6 text-slate-600">{enterpriseKnowledgeCount > 0 ? `${enterpriseKnowledgeCount} approved non-synthetic governance documents are available for policy-aware intelligence.` : 'No approved non-synthetic governance corpus is available. Policy, standards and regulatory capabilities therefore remain bootstrap-only rather than pretending synthetic knowledge is enterprise authority.'}</p><div className="mt-4 flex gap-4 text-sm"><span><b>{totalKnowledgeCount}</b> active documents</span><span><b>{enterpriseKnowledgeCount}</b> enterprise-approved</span></div></article><article className="rounded-2xl border bg-white p-6"><Bot className="h-5 w-5 text-violet-600"/><h2 className="mt-3 text-lg font-black">Operational AI control plane</h2><p className="mt-2 text-sm leading-6 text-slate-600">Agents, executions and approvals remain separately observable. Use the registry for what agents can do, Activity for what they did, and this posture view for whether the supporting capabilities are evidenced.</p><div className="mt-4 flex flex-wrap gap-2"><Link href="/agents" className="rounded-xl border px-3 py-2 text-xs font-bold text-violet-700">Agent registry</Link><Link href="/monitoring" className="rounded-xl border px-3 py-2 text-xs font-bold text-blue-700">Execution activity</Link></div></article><article className="rounded-2xl border bg-white p-6"><h2 className="text-lg font-black">How to read the matrix</h2><div className="mt-3 space-y-2 text-sm text-slate-600">{statusOrder.map((status) => <p key={status}><span className={`mr-2 inline-flex rounded-full border px-2 py-0.5 text-xs font-bold ${statusTone(status)}`}>{status}</span>{statusDescription(status)}</p>)}</div></article></section>

        <section className="rounded-2xl border bg-white p-6">
          <div><p className="text-xs font-black uppercase tracking-[.14em] text-violet-600">Coverage by evidence domain</p><h2 className="mt-1 text-xl font-black">Where AI is actually evidenced</h2><p className="mt-1 text-sm text-slate-500">Coverage is grouped by the persisted evidence domain behind each strategic capability.</p></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{domainCoverage.map(([domain,coverage]) => <div key={domain} className="rounded-2xl border bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><p className="truncate text-sm font-bold">{domain}</p><span className="text-xs font-black text-violet-700">{coverage.evidenced}/{coverage.total}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-violet-600" style={{width:`${coverage.total ? Math.round((coverage.evidenced/coverage.total)*100) : 0}%`}}/></div><p className="mt-2 text-xs text-slate-500">{coverage.total ? Math.round((coverage.evidenced/coverage.total)*100) : 0}% evidenced</p></div>)}</div>
        </section>
