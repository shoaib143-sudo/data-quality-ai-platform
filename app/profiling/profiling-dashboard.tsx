...
function MiniHistogram({ values }: { values: number[] }) {
  const fallback = [2, 4, 7, 12, 18, 12, 8, 5, 3]
  const bars = values.length ? values.slice(0, 14) : fallback
  const max = Math.max(...bars, 1)
  return <div className="flex h-9 items-end gap-[2px]" aria-label="Distribution histogram">
...
export default function ProfilingDashboard({ run, datasetName, datasetSubtitle, columns, metrics, distributions, findings, samples }: Props) {
  ...
  const columnRows = columns.map((column) => {
    ...
    return { column, columnMetrics, nullRate, uniqueRate, sensitiveRate, outlierRate }
  })
  ...
  const visibleColumns = columnRows.slice(0, 8)
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    <div className="mx-auto max-w-[1540px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-3">
        <div><h2 className="text-2xl font-black">Data Profiling</h2><p className="text-sm text-slate-500">Comprehensive analysis of structure, content and statistical properties. Select any card or chart to drill down.</p></div>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-3">
        <Card className="p-5">
          ...
          <div className="mt-4 grid grid-cols-[1.2fr_0.8fr] gap-4">
            ...
            <dl className="space-y-3 text-sm"><div><dt className="text-xs text-slate-500">Rows Processed</dt><dd className="text-lg font-black">{compact(run.row_count)}</dd></div><div><dt className="text-xs text-slate-500">Columns Analyzed</dt><dd className="text-lg font-black">{compact(run.column_count)}</dd></div><div><dt className="text-xs text-slate-500">Findings</dt><dd className="text-lg font-black">{findings.length}</dd></div></dl>
          </div>
        </Card>

        <Card className="p-5">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2 font-black"><BarChart3 className="h-5 w-5 text-blue-600" /> Column Statistics</div><span className="text-xs text-slate-400">Null % / Unique %</span></div>
          <div className="mt-5 space-y-3">
            {visibleColumns.slice(0, 6).map((row) => <button key={row.column.id} type="button" onClick={() => setDrilldown({ kind: 'column', columnId: row.column.id })} className="grid w-full grid-cols-[minmax(80px,1fr)_2fr] items-center gap-3 text-left hover:bg-slate-50">
        ...
        <Card className="p-5" onClick={() => setDrilldown({ kind: 'duplicates' })}>
          ...
          <div className="mt-5 divide-y divide-slate-100 text-sm"><div className="flex justify-between py-2"><span className="text-slate-500">Total Rows</span><strong>{compact(run.row_count)}</strong></div><div className="flex justify-between py-2"><span className="text-slate-500">Duplicate Rows</span><strong>{compact(duplicateCount)}</strong></div><div className="flex justify-between py-2"><span className="text-slate-500">Duplicate Percentage</span><strong>{percent(duplicateRate)}</strong></div></div>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-3">
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4"><div className="flex items-center gap-2 font-black"><Database className="h-5 w-5 text-blue-600" /> Schema Overview</div><Link href={`/profiling/explorer?runId=${encodeURIComponent(run.id)}`} className="text-xs font-bold text-blue-700">View all {columns.length} columns →</Link></div>
          <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-100 text-slate-500"><tr><th className="px-4 py-2">Column Name</th><th className="px-4 py-2">Data Type</th><th className="px-4 py-2">Null %</th><th className="px-4 py-2">Unique %</th></tr></thead><tbody>{visibleColumns.slice(0, 5).map((row) => <tr key={row.column.id} className="cursor-pointer border-t border-slate-100 hover:bg-blue-50" onClick={() => setDrilldown({ kind: 'column', columnId: row.column.id })}><td className="px-4 py-2 font-semibold">{row.column.column_name}</td><td className="px-4 py-2">{row.column.inferred_type ?? row.column.source_type ?? 'unknown'}</td><td className="px-4 py-2">{percent(row.nullRate)}</td><td className="px-4 py-2">{percent(row.uniqueRate)}</td></tr>)}</tbody></table></div>
        </Card>

        <Card className="overflow-hidden" onClick={() => setDrilldown({ kind: 'sensitive' })}>
          <div className="flex items-center justify-between px-5 py-4"><div className="flex items-center gap-2 font-black"><ShieldAlert className="h-5 w-5 text-blue-600" /> Sensitive Data Detection</div><span className="rounded-full bg-red-50 px-2 py-1 text-xs font-bold text-red-600">{sensitiveColumns.length} detected</span></div>
          <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-100 text-slate-500"><tr><th className="px-4 py-2">Column Name</th><th className="px-4 py-2">Detected Type</th><th className="px-4 py-2">Confidence</th></tr></thead><tbody>{sensitiveColumns.slice(0, 5).map((row) => <tr key={row.column.id} className="border-t border-slate-100"><td className="px-4 py-2 font-semibold">{row.column.column_name}</td><td className="px-4 py-2">{row.column.semantic_type ?? 'Sensitive pattern'}</td><td className="px-4 py-2">{percent(row.sensitiveRate, 0)}</td></tr>)}{!sensitiveColumns.length ? <tr><td colSpan={3} className="px-4 py-5 text-center text-slate-500">No sensitive columns detected.</td></tr> : null}</tbody></table></div>
        </Card>

        <Card className="overflow-hidden" onClick={() => setDrilldown({ kind: 'outliers' })}>
          <div className="flex items-center justify-between px-5 py-4"><div className="flex items-center gap-2 font-black"><Activity className="h-5 w-5 text-blue-600" /> Outlier Detection</div><span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">{outlierColumns.length} columns</span></div>
          <div className="divide-y divide-slate-100">{outlierColumns.slice(0, 4).map((row) => { const histogram = distributionsByColumn.get(row.column.id)?.find((item) => item.distribution_type.toUpperCase() === 'HISTOGRAM'); return <div key={row.column.id} className="grid grid-cols-[1fr_auto_120px] items-center gap-3 px-5 py-3 text-xs"><strong className="truncate">{row.column.column_name}</strong><span>{percent(row.outlierRate)}</span><MiniHistogram values={histogramValues(histogram?.distribution)} /></div> })}{!outlierColumns.length ? <div className="px-5 py-6 text-center text-sm text-slate-500">No outlier evidence detected.</div> : null}</div>
        </Card>
      </section>

      <Card className="mt-4 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div><div className="flex items-center gap-2 font-black"><Table2 className="h-5 w-5 text-blue-600" /> Sample Data Preview</div><p className="mt-1 text-xs text-slate-500">Governed sample evidence persisted with this profiling run.</p></div><button type="button" onClick={() => setDrilldown({ kind: 'sample' })} className="inline-flex items-center gap-1 text-xs font-bold text-blue-700">View full preview <Eye className="h-4 w-4" /></button></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-slate-100 text-slate-500"><tr><th className="px-4 py-2">#</th><th className="px-4 py-2">Persisted sample content</th><th className="px-4 py-2">Characters</th></tr></thead><tbody>{samples.slice(0, 7).map((sample) => <tr key={sample.index} className="border-t border-slate-100"><td className="px-4 py-2 font-semibold">{sample.index}</td><td className="max-w-[1000px] truncate px-4 py-2">{sample.content}</td><td className="px-4 py-2">{compact(sample.character_count)}</td></tr>)}{!samples.length ? <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-500">No governed sample rows were persisted for this source type. Profiling metrics remain available above.</td></tr> : null}</tbody></table></div>
      </Card>
    </div>
    ...
    {drilldown ? <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Profiling drill-down">
      ...
      <aside className="relative h-full w-full max-w-2xl overflow-y-auto border-l border-slate-200 bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Profiling drill-down</div><h3 className="mt-1 text-2xl font-black">{drilldown.kind === 'column' ? selectedColumn?.column.column_name ?? 'Column' : drilldown.kind === 'duplicates' ? 'Duplicate Analysis' : drilldown.kind === 'sensitive' ? 'Sensitive Data Detection' : drilldown.kind === 'outliers' ? 'Outlier Detection' : 'Sample Data Preview'}</h3></div><button type="button" onClick={() => setDrilldown(null)} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-50"><X className="h-5 w-5" /></button></div>
        ...
        {selectedColumn ? <div className="mt-6 space-y-5">
          ...
          <Link href={`/profiling/explorer?runId=${encodeURIComponent(run.id)}&columnId=${encodeURIComponent(selectedColumn.column.id)}`} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700">Open full column explorer <ArrowRight className="h-4 w-4" /></Link>
        </div> : null}

        {drilldown.kind === 'duplicates' ? <div className="mt-6 space-y-4"><div className="grid grid-cols-3 gap-3">{[['Total rows', compact(run.row_count)], ['Duplicate rows', compact(duplicateCount)], ['Duplicate rate', percent(duplicateRate)]].map(([name, value]) => <div key={name} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs text-slate-500">{name}</div><div className="mt-1 text-xl font-black">{value}</div></div>)}</div><p className="text-sm leading-6 text-slate-600">This panel uses the persisted dataset-level duplicate metrics from this exact profiling run. It does not infer duplicate evidence from the UI.</p></div> : null}

        {drilldown.kind === 'sensitive' ? <div className="mt-6 space-y-3">{sensitiveColumns.map((row) => <button key={row.column.id} type="button" onClick={() => setDrilldown({ kind: 'column', columnId: row.column.id })} className="flex w-full items-center justify-between rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:bg-blue-50"><div><div className="font-black">{row.column.column_name}</div><div className="text-xs text-slate-500">{row.column.semantic_type ?? row.column.inferred_type ?? 'Sensitive pattern'}</div></div><div className="font-black text-red-600">{percent(row.sensitiveRate)}</div></button>)}{!sensitiveColumns.length ? <p className="text-sm text-slate-500">No sensitive column evidence is present in this run.</p> : null}</div> : null}

        {drilldown.kind === 'outliers' ? <div className="mt-6 space-y-3">{outlierColumns.map((row) => { const histogram = distributionsByColumn.get(row.column.id)?.find((item) => item.distribution_type.toUpperCase() === 'HISTOGRAM'); return <button key={row.column.id} type="button" onClick={() => setDrilldown({ kind: 'column', columnId: row.column.id })} className="grid w-full grid-cols-[1fr_auto_150px] items-center gap-4 rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 hover:bg-blue-50"><div><div className="font-black">{row.column.column_name}</div><div className="text-xs text-slate-500">{row.column.inferred_type ?? row.column.source_type ?? 'unknown'}</div></div><strong>{percent(row.outlierRate)}</strong><MiniHistogram values={histogramValues(histogram?.distribution)} /></button> })}{!outlierColumns.length ? <p className="text-sm text-slate-500">No outlier evidence is present in this run.</p> : null}</div> : null}

        {drilldown.kind === 'sample' ? <div className="mt-6 space-y-3">{samples.map((sample) => <article key={sample.index} className="rounded-xl border border-slate-200 p-4"><div className="flex justify-between text-xs text-slate-500"><span>Sample {sample.index}</span><span>{compact(sample.character_count)} characters</span></div><pre className="mt-3 whitespace-pre-wrap break-words font-sans text-sm leading-6 text-slate-700">{sample.content}</pre></article>)}{!samples.length ? <p className="text-sm text-slate-500">No governed sample evidence was persisted for this source type.</p> : null}</div> : null}
      </aside>
    </div> : null}
  </main>
}
