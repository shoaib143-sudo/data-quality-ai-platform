...
export function WorkflowManager({projects,definitions,instances,outcomes,learning,issues}:{projects:Project[];definitions:Definition[];instances:Instance[];outcomes:Outcome[];learning:Learning[];issues:Issue[]}){
  ...
  async function retryReprofile(instanceId:string){
    ...
    try{
      ...
      if(payload.status==='QUEUED')setMessage(payload.restart
        ? `Cancelled verification restarted as generation ${payload.verificationGeneration??'next'} using profile ${shortId(payload.profilingRunId)}.`
        : `Automatic verification profile ${shortId(payload.profilingRunId)} queued.`)
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 space-y-3">{instances.filter(instance=>instance.project_id===projectId).length?instances.filter(instance=>instance.project_id===projectId).map(instance=>{
          ...
          const canRetryAutomaticVerification=Boolean(outcome&&allProfileIssuesResolved&&(outcome.status==='ACTION_TRACKED'||verificationCancelled||verificationRetryable||(outcome.status==='VERIFICATION_QUEUED'&&!outcome.verification_job_id)))
          return <article key={instance.id} className="rounded-2xl border p-4">
            ...
            {isProfiling&&instance.status==='APPROVED'?<div className="mt-3 flex flex-wrap gap-2">{!outcome?<button disabled={busy===`remediate:${instance.id}`} onClick={()=>void remediate(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{busy===`remediate:${instance.id}`?<Loader2 className="h-3.5 w-3.5 animate-spin"/>:<Wrench className="h-3.5 w-3.5"/>}Track remediation</button>:null}{canRetryAutomaticVerification?<button disabled={busy===`reprofile:${instance.id}`} onClick={()=>void retryReprofile(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 px-3 py-2 text-xs font-bold text-blue-700 disabled:opacity-50">{busy===`reprofile:${instance.id}`?<Loader2 className="h-3.5 w-3.5 animate-spin"/>:<PlayCircle className="h-3.5 w-3.5"/>}{verificationCancelled?'Restart automatic verification':'Retry automatic verification'}</button>:null}{outcome&&!['ACTION_TRACKED','VERIFICATION_CANCELLED'].includes(outcome.status)&&outcome.verification_profile_run_id?<button disabled={busy===`verify:${instance.id}`} onClick={()=>void verify(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 px-3 py-2 text-xs font-bold text-emerald-700 disabled:opacity-50">{busy===`verify:${instance.id}`?<Loader2 className="h-3.5 w-3.5 animate-spin"/>:<ShieldCheck className="h-3.5 w-3.5"/>}Check verification</button>:null}</div>:null}
          </article>
