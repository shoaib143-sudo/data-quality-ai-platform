...
import { CheckCircle2, GitBranch, Loader2, PlayCircle, Plus, ShieldCheck, Wrench, XCircle } from 'lucide-react'
...
export function WorkflowManager({projects,definitions,instances,outcomes}:{projects:Project[];definitions:Definition[];instances:Instance[];outcomes:Outcome[]}){
  ...
  async function remediate(instanceId:string){
    ...
    try{
      ...
      setMessage(`Remediation tracked. ${payload.created?.length??0} issue(s) created, ${payload.reused?.length??0} reused.`)
      router.refresh()
  ...
  async function verify(instanceId:string){
    ...
    try{
      ...
      setMessage(payload.verificationPassed?'Remediation verification passed.':'Remediation verification did not pass.')
      router.refresh()
