      ...
      <header className="rounded-3xl border bg-white p-7 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 max-w-4xl text-sm text-slate-600">Read-only visibility from the canonical effective provider/model pricing authority. This page does not calculate execution cost, perform FX conversion, publish prices, or enable runtime USD budget enforcement.</p>
          </div>
        </div>
      </header>
      ...
      {!state ? <section className="rounded-2xl border bg-white p-6 text-sm text-slate-600">No authorized project is available for this account.</section> : <>
        ...
        <section className="rounded-2xl border bg-white p-6">
          ...
          <div className="mt-5 overflow-x-auto">
            ...
            </table> : <p className="rounded-xl border border-dashed p-4 text-sm text-slate-600">No canonical current model pricing authority is recorded for this project. No price is inferred, scraped, or fabricated. Runtime cost enforcement remains disabled.</p>}
          </div>
        </section>
        ...
        <section className="rounded-2xl border bg-slate-950 p-6 text-white">
          ...
          <p className="mt-3 max-w-4xl text-sm text-slate-300">Pricing authority records provide reviewed provider/model price evidence only. They do not approve a model, authorize deployment, calculate observed execution cost, convert currencies, or activate max_cost_usd_per_day. Pricing mutation and runtime cost enforcement remain disabled in the Command Center.</p>
        </section>
      </>}
    </div>
  </main>
