...
export default async function AdminPage() {
  ...
  if(!organizationIds.length) {
    return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-4xl"><GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Administration" contextLabel="Organization access control" homeHref="/home" /><div className="mt-6 rounded-3xl border border-amber-200 bg-white p-8 shadow-sm"><ShieldCheck className="h-8 w-8 text-amber-600"/><h1 className="mt-4 text-2xl font-black">Organization administration</h1><p className="mt-2 text-slate-600">OWNER or ADMIN membership is required to manage organization access.</p><Link href="/home" className="mt-6 inline-block text-sm font-bold text-blue-600">Return home</Link></div></div></main>
  }
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen px-4 py-5 text-slate-100 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-7xl">
      ...
      {canAdminWorkspace ? <div className="dn-glass-rail sticky top-[4.5rem] z-30 mb-3 mt-3 flex flex-wrap justify-end gap-1 rounded-2xl p-2 text-sm"><Link href="/admin" aria-current="page" className="rounded-xl px-3 py-2 font-semibold text-slate-700 hover:bg-blue-50">Organization Access</Link><Link href="/admin/cleanup" className="rounded-xl px-3 py-2 font-semibold text-rose-700 hover:bg-rose-50">Super Admin Cleanup</Link><Link href="/admin/project-roles" className="rounded-xl px-3 py-2 font-semibold text-slate-700 hover:bg-blue-50">Project Roles</Link><Link href="/admin/landing-pages" className="rounded-xl px-3 py-2 font-semibold text-blue-700 hover:bg-blue-50">Landing Pages</Link><Link href="/admin/identity" className="rounded-xl px-3 py-2 font-semibold text-blue-700 hover:bg-blue-50">Enterprise Identity</Link><Link href="/admin/infrastructure" className="rounded-xl px-3 py-2 font-semibold text-cyan-700 hover:bg-cyan-50">Infrastructure</Link><Link href="/admin/ai-command-center" className="rounded-xl px-3 py-2 font-semibold text-violet-700 hover:bg-violet-50">AI Command Center</Link><Link href="/admin/ai-command-center/resource-controls" className="rounded-xl px-3 py-2 font-semibold text-violet-700 hover:bg-violet-50">AI Resource Controls</Link><Link href="/admin/ai-command-center/audit" className="rounded-xl px-3 py-2 font-semibold text-violet-700 hover:bg-violet-50">AI Audit Evidence</Link></div> : null}
      <header className="dn-surface mb-4 p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-600/80 text-white"><ShieldCheck className="h-5 w-5"/></span><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Administration</p><h1 className="text-3xl font-black">Organization access control</h1></div></div><p className="mt-4 max-w-3xl text-sm leading-6 text-slate-600">Manage membership, governance personas and role-based landing experiences with last-owner protection and an auditable change trail. Organization administration remains separate from governance workspace access.</p></header>
