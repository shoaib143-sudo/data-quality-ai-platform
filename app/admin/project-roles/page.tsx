...
import { governancePersonaRoleKeys } from '@/lib/governance/persona-role-keys'
...
export default async function ProjectRolesPage(){
  const user=await requireUser();const landing=await resolveLandingAccess(user.id);const canAdminWorkspace=canAccessWorkspaceHref(landing.persona,'/admin',landing.organizationRole);const admin=createAdminClient()
  ...
  if(!orgIds.length)return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-4xl"><GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Project Roles" contextLabel="Fine grained authorization" homeHref="/home"/><div className="mt-6 rounded-3xl border bg-white p-8 shadow-sm"><ShieldCheck className="h-8 w-8 text-amber-600"/><h1 className="mt-4 text-2xl font-black">Project governance roles</h1><p className="mt-2 text-slate-600">Organization OWNER or ADMIN access is required.</p><Link href="/home" className="mt-5 inline-block font-bold text-blue-600">Return home</Link></div></div></main>
  ...
  const [projectsResult,membersResult,rolesResult,bindingsResult,usersResult]=await Promise.all([
    ...
    admin.schema('governance').from('access_roles').select('role_key,name,description,capabilities').in('role_key',[...governancePersonaRoleKeys]).order('name'),
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8"><GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Project Roles" contextLabel="Fine grained authorization" homeHref="/home"/>{canAdminWorkspace?<div className="mb-6 mt-4 flex flex-wrap justify-end gap-2 text-sm"><Link href="/admin" className="rounded-xl px-3 py-2 font-semibold text-slate-600 hover:bg-blue-50">Organization Access</Link><Link href="/admin/landing-pages" className="rounded-xl px-3 py-2 font-semibold text-slate-600 hover:bg-blue-50">Landing Pages</Link><Link href="/admin/identity" className="rounded-xl px-3 py-2 font-semibold text-slate-600 hover:bg-blue-50">Enterprise Identity</Link></div>:null}<header className="rounded-3xl border border-blue-100 bg-white p-7 shadow-sm"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-blue-600 text-white"><ShieldCheck className="h-6 w-6"/></span><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Fine grained authorization</p><h1 className="text-3xl font-black">Project governance roles</h1></div></div><p className="mt-4 max-w-4xl text-sm leading-6 text-slate-600">Assign one of the {governancePersonaRoleKeys.length} approved governance personas by project without granting organization-wide administration. Changes are enforced by centralized server authorization and written to the immutable audit trail.</p></header><div className="mt-6"><ProjectRoleManager projects={projects} members={members} roles={roles} bindings={bindings}/></div></div></main>
}
