        ...
        <table className="w-full min-w-[760px] text-left text-sm">
          ...
          <tbody>{visibleMembers.map((member)=>{
            ...
            return <tr key={member.userId} className="border-b border-slate-100 align-top">
              ...
              <td className="px-3 py-3"><select aria-label={`Role for ${member.email||member.userId}`} disabled={Boolean(busy)||lockedOwner} value={member.role} onChange={(event)=>void call('PATCH',{organizationId,userId:member.userId,role:event.target.value},member.userId)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs font-bold">
