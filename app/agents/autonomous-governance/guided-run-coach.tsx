            ...
            <tbody className="divide-y">
              {readiness.tables.map(table => <tr key={table.sourceId + ':' + table.qualifiedName}>
                ...
                <td className="p-3 text-muted-foreground">
                  ...
                  {(table.blockerCodes?.length ?? 0) > 0 && <span className="mt-1 block break-words text-amber-700 dark:text-amber-300">Readiness blockers: {table.blockerCodes?.join(', ')}</span>}
