          ...
          {datasets.length === 0 ? <p className="mt-5 text-sm text-muted-foreground">No datasets are registered yet.</p> :
            <div className="mt-5 space-y-3">{datasets.map(dataset => {
              ...
              return <div key={dataset.id} className="rounded-lg border p-4">
                ...
                <div className="mt-3 grid gap-2 text-xs text-muted-foreground md:grid-cols-3"><span>Source: {source?.name ?? 'Unbound'}</span><span>Version: {latest ? `v${latest.version_number}` : 'None'}</span><span>Status: {statusLabel(latest?.status)}</span></div>
