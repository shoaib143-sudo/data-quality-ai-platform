...
export function RegisterDatasetForm({ projects, organizations, sources }: { projects: ProjectOption[]; organizations: OrganizationOption[]; sources: SourceOption[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!submittedProjectId || !submittedSourceId || !submittedName || !submittedSourceIdentifier) { setStatus('Project, source, dataset name, and source identifier are required.'); return }
    setRunning(true)
    try {
  ...
  return <section className="rounded-xl border p-6">
    ...
    {availableProjects.length === 0 ? <p className="text-sm text-muted-foreground">No projects are available for dataset registration.</p> :
      <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2"><button type="submit" disabled={running || profiling || projectMode !== 'existing'} className="rounded-md border px-4 py-2 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50">{running ? 'Registering…' : 'Register dataset'}</button></div>
      </form>}
