...
export default async function EditSourcePage({ params }: { params: Promise<{ sourceId: string }> }) {
  ...
  if (String(source.source_type).toUpperCase() !== 'JDBC') {
    return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-6"><div className="mx-auto max-w-4xl"><GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Edit Connection" contextLabel="Governed source configuration" homeHref="/home" />{canDatasets ? <div className="mt-4"><Link href="/datasets" className="text-sm font-semibold text-blue-600">Back to connections</Link></div> : null}<h1 className="my-5 text-3xl font-bold">Edit connection</h1><p className="rounded-xl border bg-white p-5 text-sm text-slate-600">This source is not a database/JDBC connection. Manage its source file or application-specific configuration from the corresponding connector.</p></div></main>
  }
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-6"><div className="mx-auto max-w-4xl">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Edit Connection" contextLabel="Governed source configuration" homeHref="/home" />
    {canDatasets ? <div className="mt-4"><Link href="/datasets" className="text-sm font-semibold text-blue-600">Back to connections</Link></div> : null}
