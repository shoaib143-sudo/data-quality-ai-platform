...
const CONNECTIONS: ConnectionOption[] = [
  { id: 'csv', label: 'CSV File', description: 'HTTPS CSV or Supabase Storage object', placeholder: 'https://host/path/data.csv or bucket/path/data.csv', tips: ['Use a reachable HTTPS CSV URL or Supabase Storage bucket/path.', 'The file is checked immediately and its columns are discovered before registration.', 'No database username, password, or JDBC setup is needed for CSV.'] },
  { id: 'file', label: 'Unstructured File', description: 'Text, JSON, JSONL, documents, images, or other file metadata', placeholder: 'https://host/path/document.txt or bucket/path/document.pdf', tips: ['Text, Markdown, HTML, XML, JSON and JSONL content is scanned directly.', 'Binary files such as PDF, Office documents and images are scanned for metadata until a document extraction runtime is enabled.', 'Files can be read from HTTPS URLs or Supabase Storage bucket/path locations.'] },
  { id: 'postgresql', label: 'PostgreSQL', description: 'PostgreSQL or Supabase database', placeholder: 'jdbc:postgresql://host:5432/database', tips: ['Connect to the database first; DataNexus reads PostgreSQL native schemas, objects, and columns.', 'The hierarchy is reported by PostgreSQL metadata rather than invented by DataNexus.', 'After discovery, select any combination of schemas, objects, or fields.'] },
  { id: 'mssql', label: 'Microsoft SQL Server', description: 'SQL Server or Azure SQL', placeholder: 'jdbc:sqlserver://host:1433;databaseName=database', tips: ['Connect to the target database using SQL Server connection semantics.', 'DataNexus preserves the database → schema → native object hierarchy returned by the driver.', 'After discovery, select any combination of native hierarchy nodes.'] },
  { id: 'mysql', label: 'MySQL', description: 'MySQL compatible database', placeholder: 'jdbc:mysql://host:3306/database', tips: ['MySQL database/schema semantics are preserved as reported by the driver.', 'DataNexus does not create an extra schema layer when the product does not have one.', 'After discovery, select databases, objects, or fields from the native tree.'] },
  { id: 'databricks', label: 'Databricks Unity Catalog', description: 'Databricks SQL warehouse and Unity Catalog', placeholder: 'jdbc:databricks://host:443/default;transportMode=http;ssl=1;AuthMech=3;httpPath=/sql/1.0/warehouses/...', tips: ['Enter only the workspace connection details; do not preselect a catalog or schema.', 'DataNexus discovers accessible Unity Catalog catalogs, schemas, tables/views, and fields after connecting.', 'Select multiple catalogs, schemas, objects, or fields directly from the discovered Unity Catalog hierarchy.'] },
  { id: 'jdbc', label: 'Generic JDBC', description: 'Supported JDBC driver endpoint', placeholder: 'jdbc:<driver>://host:port/database', tips: ['Use a JDBC URL without embedded credentials.', 'DataNexus asks the JDBC driver for its native catalog/schema terminology and hierarchy.', 'The current bridge includes PostgreSQL, SQL Server, MySQL, MariaDB, Databricks, Snowflake, Redshift, and Oracle drivers.'] },
]
...
export function JdbcSourceForm({ projects, organizations, initialSource }: { projects: JdbcProjectOption[]; organizations: JdbcOrganizationOption[]; initialSource?: SavedDatabricksConnection }) {
  ...
  async function discover() {
    ...
    try {
      if (isFile) {
        ...
        setStatus(`${isCsv ? 'CSV validated' : 'File scanned'}. ${payload.columns?.length ?? 0} profile fields discovered.${warning}`)
        return
      }
      ...
      setStatus(`Connected to ${payload.hierarchy.databaseProduct}. ${payload.hierarchy.nodes.length} native hierarchy nodes discovered, including ${objectCount} objects and ${fieldCount} fields.`)
  ...
  async function register() {
    ...
    if (!isFile && selectionMode === 'SELECTED' && selectedNodeIds.length === 0) { setError(true); setStatus('Select at least one node from the discovered hierarchy.'); return }
    setBusy(true)
    try {
      ...
      const selectedQualifiedNames = hierarchy
        ? selectedNodeIds.map(id => hierarchy.nodes.find(node => node.id === id)?.qualifiedName).filter((value): value is string => Boolean(value))
        : []
      const response = await fetch('/api/datasets/source/register', {
        ...
        body: JSON.stringify({
          ...
          hierarchySelection: !isFile ? { mode: selectionMode, nodeIds: selectedNodeIds, qualifiedNames: selectedQualifiedNames } : undefined,
        }),
      })
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.validation?.errors?.join(' ') || payload.error || 'Source registration failed.')
      setStatus(isFile ? `Connection is ready. ${isCsv ? 'CSV' : 'FILE'} source registered successfully.` : 'Database connection and native discovery scope saved. Open Catalog Discovery to ingest the selected hierarchy, metadata, and available lineage.')
  ...
  return <section className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      {columns.length > 0 && <div className="md:col-span-2 rounded-xl border border-emerald-100 bg-emerald-50/40 p-3 text-xs text-slate-600"><strong className="text-slate-800">Validated source:</strong> {columns.length} fields{typeof rowCount === 'number' ? ` · ${rowCount} rows` : ''}</div>}
