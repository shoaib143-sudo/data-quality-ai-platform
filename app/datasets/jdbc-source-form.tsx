...
export function JdbcSourceForm({ projects, organizations }: { projects: JdbcProjectOption[]; organizations: JdbcOrganizationOption[] }) {
  ...
  async function register() {
    ...
    if (!isFile && (!schema || !table)) { setError(true); setStatus('Select a schema and table/view after discovery.'); return }
    setBusy(true)
    try { const ref = isFile ? '' : (credentialRef || await provisionCredentials()); const fileSourceType = isCsv ? 'CSV' : 'FILE'; const response = await fetch('/api/datasets/source/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, name, sourceType: isFile ? fileSourceType : 'JDBC', jdbcUrl: isFile ? undefined : url, sourceUri: isFile ? url : undefined, connectionKind, schema: isFile ? (isCsv ? 'CSV' : 'FILE') : schema, table: isFile ? undefined : table, credentialRef: ref }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.validation?.errors?.join(' ') || payload.error || 'Source registration failed.'); setColumns(payload.validation?.details?.columns ?? payload.validation?.columns ?? []); setRowCount(typeof payload.validation?.rowCount === 'number' ? payload.validation.rowCount : null); setStatus(isFile ? `Connection is ready. ${isCsv ? 'CSV' : 'FILE'} source registered successfully.` : `Connection is ready. ${schema}.${table} is available for profiling.`); if (payload.source) window.dispatchEvent(new CustomEvent('dgp:source-created', { detail: { id: payload.source.id, projectId: payload.source.project_id, name: payload.source.name, sourceType: payload.source.source_type, status: payload.source.status } })) } catch (e) { setError(true); setStatus(e instanceof Error ? e.message : 'Source registration failed.') } finally { setBusy(false) }
  }
