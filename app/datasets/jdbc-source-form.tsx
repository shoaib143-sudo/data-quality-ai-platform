...
export function JdbcSourceForm({ projects }: { projects: JdbcProjectOption[] }) {
  ...
  async function discover() {
    ...
    try {
      if (isCsv) {
        ...
        setSchemas(['CSV']); setSchema('CSV'); setTables(payload.tables ?? []); setStatus(`CSV source available. Found ${payload.columns?.length ?? 0} columns${typeof payload.rowCount === 'number' ? ` and ${payload.rowCount} rows` : ''}.`); return
      }
  ...
  async function inspectTable(value: string) {
    setTable(value); setColumns([]); setRowCount(null); setStatus(null); if (isCsv || !schema || !value) return
    setBusy(true)
    try {
  ...
  async function register() {
    ...
    try {
      ...
      setColumns(payload.validation?.details?.columns ?? payload.validation?.columns ?? []); setRowCount(typeof payload.validation?.rowCount === 'number' ? payload.validation.rowCount : null)
      setStatus(isCsv ? `CSV source saved and is profiling-ready: ${jdbcUrl}.` : `Connection saved and source is profiling-ready: ${schema}.${table}${typeof payload.validation?.rowCount === 'number' ? ` · ${payload.validation.rowCount} rows` : ''}.`); setName('')
