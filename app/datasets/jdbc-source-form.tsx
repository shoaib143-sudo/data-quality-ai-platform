...
export function JdbcSourceForm({ projects }: { projects: JdbcProjectOption[] }) {
  ...
  function selectConnection(value: ConnectionKind) {
    ...
    setJdbcUrl(''); setSchema(''); setTable(''); setSchemas([]); setTables([]); setColumns([]); setRowCount(null); setStatus(null)
  }
  ...
  async function discover() {
    ...
    try {
      if (isCsv) {
        ...
        setSchemas(['CSV']); setSchema('CSV'); setTables(payload.tables ?? [])
      ...
      const response = await fetch('/api/datasets/source/discover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, jdbcUrl, schema: schema || undefined }) })
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error ?? 'Connection discovery failed.')
      setSchemas(payload.schemas ?? []); setTables(payload.tables ?? [])
  ...
  async function inspectTable(value: string) {
    ...
    setBusy(true)
    try {
      const response = await fetch('/api/datasets/source/discover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, jdbcUrl, schema }) })
      const payload = await response.json()
  ...
  async function register() {
    ...
    setBusy(true)
    try {
      const response = await fetch('/api/datasets/source/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, name, sourceType: isCsv ? 'CSV' : 'JDBC', jdbcUrl: isCsv ? undefined : jdbcUrl, sourceUri: isCsv ? jdbcUrl : undefined, schema: isCsv ? 'CSV' : schema, table: isCsv ? undefined : table }) })
      const payload = await response.json()
  ...
  return <section className="rounded-xl border p-6">
    <div className="mb-6"><h2 className="text-lg font-semibold">Connect a data source</h2><p className="mt-1 text-sm text-muted-foreground">Choose the source type from one dropdown. Database sources can test connectivity and discover schemas and tables. CSV sources can be validated directly.</p></div>
