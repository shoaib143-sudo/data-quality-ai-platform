...
export function JdbcSourceForm({ projects, organizations }: { projects: JdbcProjectOption[]; organizations: JdbcOrganizationOption[] }) {
  ...
  async function discover() {
    ...
    try {
      ...
      const ref = await provisionCredentials(); const response = await fetch('/api/datasets/source/discover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, jdbcUrl: url, connectionKind, schema: schema || undefined, catalog: connectionKind === 'databricks' ? catalog || undefined : undefined, credentialRef: ref || credentialRef }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error ?? 'Connection discovery failed.'); setJdbcUrl(url); setSchemas(payload.schemas ?? []); setTables(payload.tables ?? []); setStatus(`Connection successful. ${payload.schemas?.length ?? 0} schemas discovered${payload.tables?.length ? ` · ${payload.tables.length} tables/views returned` : ''}.`)
  ...
  async function register() {
    ...
    setBusy(true)
    try { const ref = isFile ? '' : (credentialRef || await provisionCredentials()); const fileSourceType = isCsv ? 'CSV' : 'FILE'; const connectionOnly = connectionKind === 'databricks'; const response = await fetch('/api/datasets/source/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId, name, sourceType: isFile ? fileSourceType : 'JDBC', jdbcUrl: isFile ? undefined : url, sourceUri: isFile ? url : undefined, connectionKind, catalog: connectionOnly ? catalog : undefined, schema: isFile ? (isCsv ? 'CSV' : 'FILE') : schema || undefined, table: connectionOnly ? undefined : (isFile ? undefined : table), credentialRef: ref, connectionOnly }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.validation?.errors?.join(' ') || payload.error || 'Source registration failed.'); setColumns(payload.validation?.details?.columns ?? payload.validation?.columns ?? []); setRowCount(typeof payload.validation?.rowCount === 'number' ? payload.validation.rowCount : null); setStatus(connectionOnly ? 'Databricks connection saved. Open Catalog → Discovery to scan and persist its real tables, views, columns, and lineage.' : isFile ? `Connection is ready. ${isCsv ? 'CSV' : 'FILE'} source registered successfully.` : `Connection is ready. ${schema}.${table} is available for profiling.`); if (payload.source) window.dispatchEvent(new CustomEvent('dgp:source-created', { detail: { id: payload.source.id, projectId: payload.source.project_id, name: payload.source.name, sourceType: payload.source.source_type, status: payload.source.status } })) } catch (e) { setError(true); setStatus(e instanceof Error ? e.message : 'Source registration failed.') } finally { setBusy(false) }
  }
  ...
  return <section className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      {!isFile && <>{field('Schema', schema, setSchema, schemas.length ? 'Select from discovered schemas' : 'Discover schemas first')}{field('Table / view', table, setTable, tables.length ? 'Select from discovered tables/views' : 'Discover tables after schema')}</>}
