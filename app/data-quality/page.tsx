  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-[#061426] text-slate-100">
    <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Data quality workspace" className={`${surface} mb-6 mt-4 flex items-center justify-between gap-4 overflow-x-auto px-4 py-3 sm:px-5`}>
        <span className="shrink-0 text-xs font-black uppercase tracking-[0.14em] text-slate-500">Quality workspace</span>
        <div className="flex shrink-0 gap-2"><Link href="/data-quality" aria-current="page" className={`rounded-xl bg-blue-600/20 px-3 py-2 text-sm font-semibold text-blue-200 ring-1 ring-blue-400/20 ${focus}`}>Overview</Link>{canProfiling ? <Link href="/profiling/explorer" className={`rounded-xl px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white ${focus}`}>Profiling Explorer</Link> : null}<Link href="/data-quality/rules" className={`rounded-xl px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white ${focus}`}>Quality Rules</Link>{canObservability ? <Link href="/observability" className={`rounded-xl px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white ${focus}`}>Observability</Link> : null}</div>
      </nav>
      ...
      <section className="mt-5 grid auto-rows-fr gap-4 md:grid-cols-2 2xl:grid-cols-4">
        <Link href="/profiling/explorer" className={`${surface} ${interactive} flex min-h-[148px] min-w-0 flex-col p-5`}><Gauge className={`h-5 w-5 ${scoreTone(averageScore)}`} /><p className="mt-3 text-3xl font-black text-white">{formatScore(averageScore)}</p><p className="mt-1 text-sm font-bold text-slate-200">Average quality</p><p className="mt-1 text-xs text-slate-500">Across scored completed runs</p></Link>
        <Link href="/issues" className={`${surface} ${interactive} flex min-h-[148px] min-w-0 flex-col p-5`}><AlertTriangle className="h-5 w-5 text-rose-300"/><p className="mt-3 text-3xl font-black text-white">{criticalFindings.length}</p><p className="mt-1 text-sm font-bold text-slate-200">High-priority findings</p><p className="mt-1 text-xs text-slate-500">Critical and high persisted findings</p></Link>
        <Link href="/data-quality/rules" className={`${surface} ${interactive} flex min-h-[148px] min-w-0 flex-col p-5`}><ShieldCheck className="h-5 w-5 text-cyan-300"/><p className="mt-3 text-3xl font-black text-white">{enabledQualityRules.length}</p><p className="mt-1 text-sm font-bold text-slate-200">Enabled controls</p><p className="mt-1 text-xs text-slate-500">Governed deterministic quality rules</p></Link>
        <Link href={controlRunHref} className={`${surface} ${interactive} flex min-h-[148px] min-w-0 flex-col p-5`}><CheckCircle2 className="h-5 w-5 text-amber-300"/><p className="mt-3 text-3xl font-black text-white">{failedQualityRules.length}</p><p className="mt-1 text-sm font-bold text-slate-200">Current control failures</p><p className="mt-1 text-xs text-slate-500">Latest failed or non-passing executions</p></Link>
      </section>
      ...
      <section className="mt-5 space-y-4">{runs.slice(0,8).map(run => {
        ...
        return <article key={run.id} className={`${surface} overflow-hidden`}><div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/[0.07] p-5"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-black text-white">{dataset?.name??'Unknown dataset'}</h2>{version?<span className="rounded-lg bg-blue-400/10 px-2 py-1 text-xs font-bold text-blue-300">v{version.version_number}</span>:null}<span className="rounded-lg bg-white/[0.05] px-2 py-1 text-xs font-bold text-slate-400">{run.status}</span></div><Link href={`/profiling/explorer?runId=${encodeURIComponent(run.id)}`} className={`mt-2 inline-flex items-center gap-1 text-xs font-bold text-cyan-300 ${focus}`}>Open run evidence <ArrowRight className="h-3 w-3" /></Link></div><p className={`text-3xl font-black ${scoreTone(score?.overall_score)}`}>{formatScore(score?.overall_score)}</p></div>
          <div className="p-5"><div className="grid auto-rows-fr gap-2 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">{[['Completeness',score?.completeness_score],['Validity',score?.validity_score],['Uniqueness',score?.uniqueness_score],['Accuracy',score?.accuracy_score]].map(([name,value])=><Link key={String(name)} href={`/profiling/explorer?runId=${encodeURIComponent(run.id)}`} className={`${inset} ${interactive} min-w-0 overflow-hidden p-3`}><p className="text-[10px] font-bold uppercase tracking-wide text-slate-600">{name}</p><p className="mt-1 text-lg font-black text-slate-200">{formatScore(typeof value==='number'?value:null)}</p></Link>)}<Link href={`/profiling/explorer?runId=${encodeURIComponent(run.id)}`} className={`${inset} ${interactive} min-w-0 overflow-hidden p-3`}><p className="text-[10px] font-bold uppercase tracking-wide text-slate-600">Rows / columns</p><p className="mt-1 text-lg font-black text-slate-200">{run.row_count??'N/A'} / {run.column_count??'N/A'}</p></Link></div>
