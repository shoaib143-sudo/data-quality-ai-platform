...
export default async function DataQualityRulesPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-[#061426] text-slate-100"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Data Quality Rules" contextLabel="Governed controls and rule execution" homeHref="/home" />
    <nav className="mb-6 mt-4 flex flex-wrap items-center justify-end gap-3 rounded-2xl border border-white/10 bg-[#0a1d33] px-5 py-3 shadow-sm"><div className="flex gap-2 text-sm"><Link href="/data-quality" className="rounded-xl px-3 py-2 font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white">Data Quality</Link>{canMonitoring?<Link href="/monitoring" className="rounded-xl px-3 py-2 font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white">Job Monitor</Link>:null}</div></nav>
