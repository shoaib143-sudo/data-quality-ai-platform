...
export function CatalogManager({datasets,versions,catalog:initialCatalog,projects,members,initialQuery=''}:{datasets:Dataset[];versions:Version[];catalog:Catalog[];projects:Project[];members:Member[];initialQuery?:string}){
  ...
  return <section className="mt-6 rounded-3xl border bg-white p-6 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Governed assets</h2><p className="mt-1 text-sm text-slate-500">{datasets.length} registered datasets · {filtered.length} shown</p></div><label className="relative min-w-[280px]"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search name, domain, tag, source…" className="w-full rounded-xl border py-2.5 pl-9 pr-3 text-sm"/></label></div>
    ...
    <div className="mt-5 space-y-3">{filtered.map(dataset=>{
      ...
      return <article key={dataset.id} className="rounded-2xl border border-slate-200 p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link href={`/catalog/dataset/${dataset.id}`} className="group inline-flex items-center gap-1 text-lg font-bold hover:text-cyan-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">{dataset.name}<ArrowRight className="h-4 w-4 opacity-40 transition group-hover:translate-x-0.5 group-hover:opacity-100"/></Link><span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">{c?.certification_status??'UNCERTIFIED'}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{c?.lifecycle_status??'ACTIVE'}</span><span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">{c?.criticality??'MEDIUM'} criticality</span></div><p className="mt-1 text-sm text-slate-500">{c?.business_description||dataset.description||'No business description.'}</p><div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500"><span>{project?.name??'Project'}</span><span>·</span><span>{dataset.business_domain??'Unassigned domain'}</span><span>·</span><span>{dataset.source_identifier??'No source identifier'}</span><span>·</span><span>v{v?.version_number??'—'} {v?.status??''}</span>{v?.row_count!=null?<><span>·</span><span>{v.row_count} rows</span></>:null}</div><div className="mt-3 flex flex-wrap gap-2">{(c?.tags??[]).map(tag=><button key={tag} type="button" onClick={()=>setQuery(tag)} className="rounded-full border bg-slate-50 px-2.5 py-1 text-xs font-semibold hover:border-cyan-400/30">#{tag}</button>)}</div></div><div className="flex flex-wrap gap-2"><Link href={`/catalog/dataset/${dataset.id}`} className="rounded-xl border px-4 py-2 text-sm font-bold hover:border-cyan-400/30">Open dataset</Link><button type="button" onClick={()=>setEditing(editing===dataset.id?null:dataset.id)} className="rounded-xl border px-4 py-2 text-sm font-bold">{editing===dataset.id?'Close':'Edit governance metadata'}</button></div></div>
    ...
    })}{filtered.length===0?<div className="rounded-2xl border border-dashed p-8 text-center"><p className="font-semibold">No governed data matches “{query}”.</p><button type="button" onClick={()=>setQuery('')} className="mt-3 rounded-xl border px-4 py-2 text-sm font-bold">Clear search</button></div>:null}</div>
  </section>
}
