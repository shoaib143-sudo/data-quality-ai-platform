...
type DatasetContext={lineageAssets:number;openIssues:number;dataContracts:number;openAlerts:number}

export function CatalogManager({datasets,versions,catalog:initialCatalog,projects,members,editableProjectIds,contextByDataset,initialQuery=''}:{datasets:Dataset[];versions:Version[];catalog:Catalog[];projects:Project[];members:Member[];editableProjectIds:string[];contextByDataset:Record<string,DatasetContext>;initialQuery?:string}){
  ...
  return <section className="mt-6 rounded-3xl border border-white/10 bg-[#102036] p-6 shadow-sm">
    ...
    <div className="mt-5 space-y-3">{filtered.map(dataset=>{
      ...
      return <article key={dataset.id} className="rounded-2xl border border-white/[0.08] bg-[#0d1c30] p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link href={detailHref} className="group inline-flex items-center gap-1 text-lg font-bold text-white hover:text-cyan-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">{dataset.name}<ArrowRight className="h-4 w-4 opacity-40 transition group-hover:translate-x-0.5 group-hover:opacity-100"/></Link><span className="rounded-full bg-blue-400/10 px-2.5 py-1 text-xs font-bold text-blue-300">{c?.certification_status??'UNCERTIFIED'}</span><span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-xs font-bold text-slate-400">{c?.lifecycle_status??'ACTIVE'}</span><span className="rounded-full bg-amber-400/10 px-2.5 py-1 text-xs font-bold text-amber-300">{c?.criticality??'MEDIUM'} criticality</span></div><p className="mt-1 text-sm text-slate-400">{c?.business_description||dataset.description||'No business description.'}</p><div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500"><span>{project?.name??'Project'}</span><span>·</span><span>{dataset.business_domain??'Unassigned domain'}</span><span>·</span><span>{dataset.source_identifier??'No source identifier'}</span><span>·</span><span>v{v?.version_number??'—'} {v?.status??''}</span>{v?.row_count!=null?<><span>·</span><span>{v.row_count} rows</span></>:null}</div><div className="mt-3 flex flex-wrap gap-2 text-[11px] font-semibold"><span className="rounded-full bg-violet-400/10 px-2.5 py-1 text-violet-300">Lineage {context.lineageAssets}</span><span className="rounded-full bg-rose-400/10 px-2.5 py-1 text-rose-300">Open issues {context.openIssues}</span><span className="rounded-full bg-cyan-400/10 px-2.5 py-1 text-cyan-300">Contracts {context.dataContracts}</span><span className="rounded-full bg-amber-400/10 px-2.5 py-1 text-amber-300">Alerts {context.openAlerts}</span></div><div className="mt-3 flex flex-wrap gap-2">{(c?.tags??[]).map(tag=><button key={tag} type="button" onClick={()=>setQuery(tag)} className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs font-semibold text-slate-300 hover:border-cyan-400/30">#{tag}</button>)}</div></div><div className="flex flex-wrap gap-2"><Link href={detailHref} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-500">Open Data 360</Link><Link href={lineageHref} className="rounded-xl border border-violet-400/20 px-4 py-2 text-sm font-bold text-violet-200 hover:bg-violet-400/10">Lineage</Link>{canEdit?<button type="button" onClick={()=>setEditing(editing===dataset.id?null:dataset.id)} className="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold text-slate-200 hover:border-cyan-400/30">{editing===dataset.id?'Close':'Edit governance metadata'}</button>:null}</div></div>
      {editing===dataset.id&&canEdit?<form onSubmit={e=>{e.preventDefault();void save(dataset,e.currentTarget)}} className="mt-5 grid gap-3 rounded-2xl border border-white/[0.06] bg-[#0b1422] p-4 lg:grid-cols-2"><label className="text-xs font-bold text-slate-300">Business description<textarea name="businessDescription" defaultValue={c?.business_description??''} rows={3} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-sm text-slate-200"/></label><label className="text-xs font-bold text-slate-300">Tags<input name="tags" defaultValue={(c?.tags??[]).join(', ')} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-sm text-slate-200" placeholder="customer, finance, gold"/></label><label className="text-xs font-bold text-slate-300">Lifecycle<select name="lifecycleStatus" defaultValue={c?.lifecycle_status??'ACTIVE'} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-slate-200"><option>ACTIVE</option><option>DEPRECATED</option><option>RETIRED</option></select></label><div className="text-xs font-bold text-slate-300">Certification<div className="mt-1 rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-sm font-medium text-slate-300">{c?.certification_status??'UNCERTIFIED'} <span className="font-normal text-slate-500">· managed through certification workflow</span></div></div><label className="text-xs font-bold text-slate-300">Criticality<select name="criticality" defaultValue={c?.criticality??'MEDIUM'} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-slate-200"><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select></label><label className="text-xs font-bold text-slate-300">Retention days<input type="number" min="0" name="retentionDays" defaultValue={c?.retention_days??''} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-slate-200"/></label>{[['technicalOwnerUserId','Technical owner',c?.technical_owner_user_id],['businessOwnerUserId','Business owner',c?.business_owner_user_id],['stewardUserId','Data steward',c?.steward_user_id]].map(([name,label,value])=><label key={String(name)} className="text-xs font-bold text-slate-300">{label}<select name={String(name)} defaultValue={String(value??'')} className="mt-1 w-full rounded-lg border border-white/10 bg-[#0d1c30] px-3 py-2 text-slate-200"><option value="">Unassigned</option>{orgMembers.map(member=><option key={member.user_id+member.role} value={member.user_id}>{member.user_id.slice(0,8)} · {member.role}</option>)}</select></label>)}<div className="lg:col-span-2"><button disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-500 disabled:opacity-50">{busy?<Loader2 className="h-4 w-4 animate-spin"/>:<Save className="h-4 w-4"/>}Save metadata</button></div></form>:null}
      </article>
