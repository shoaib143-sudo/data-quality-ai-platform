...
export function DiscoveryManager({ sources, runs, jobs }: { sources: Source[]; runs: Run[]; jobs: Job[] }) {
  ...
  async function discover(sourceId: string) {
    ...
    try {
      ...
      setMessage(payload.alreadyActive ? 'A metadata discovery job is already active for this source. Monitoring the existing durable worker job.' : 'Metadata discovery queued. The monitor will refresh while the durable worker scans and persists source assets.')
      router.refresh()
  ...
  const groups = useMemo(() => {
    ...
    const rows = sources.flatMap(source => {
      ...
      const searchable = [source.name, source.source_type, ...allRuns.flatMap(run => [run.id, run.error_message ?? '']), ...allJobs.flatMap(job => [job.id, job.last_error ?? '', job.lease_owner ?? '', discoveryJobHealth(job).title, discoveryJobHealth(job).message])].join(' ').toLowerCase()
      ...
      return [{ source, runs: filteredRuns, jobs: filteredJobs, allRuns, allJobs, latest, assets, latestStatus: upper(allJobs[0]?.status ?? allRuns[0]?.status ?? source.status) }]
    })
    rows.sort((a, b) => sortKey === 'SOURCE' ? direction * a.source.name.localeCompare(b.source.name) : sortKey === 'ASSETS' ? direction * (a.assets - b.assets) : sortKey === 'STATUS' ? direction * a.latestStatus.localeCompare(b.latestStatus) : direction * (a.latest - b.latest))
    return rows
  ...
  const attentionJobCount = jobs.filter(job => discoveryJobHealth(job).severity === 'warning' || discoveryJobHealth(job).severity === 'error').length
  ...
  return <div className="mt-6 space-y-6">
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{sources.map(source => {
      ...
      return <article key={source.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        {run ? <div className="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-3 text-xs text-slate-600"><div className="flex flex-wrap items-center justify-between gap-2"><StatusBadge status={activeJob?.status ?? run.status} /><span className="font-semibold">{run.assets_discovered} assets</span></div><div className="mt-2 flex items-center gap-1.5 text-slate-500"><Clock3 className="h-3.5 w-3.5" />{formatDate(run.started_at)}</div>{activeJob ? <div className="mt-1 text-slate-500">Worker attempt {activeJob.attempts}/{activeJob.max_attempts}</div> : null}</div> : <p className="mt-4 text-xs text-slate-400">{active ? 'Waiting for durable discovery worker…' : 'No discovery run yet.'}</p>}
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 bg-gradient-to-r from-violet-50 via-white to-blue-50 px-6 py-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><RefreshCw className={`h-5 w-5 text-violet-600 ${hasActiveJobs ? 'animate-spin' : ''}`} /><h2 className="text-2xl font-black">Discovery activity</h2></div><p className="mt-1 text-sm text-slate-500">Hierarchical view: source → discovery runs and durable worker execution.</p></div><div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 shadow-sm"><span className={`h-2 w-2 rounded-full ${hasActiveJobs ? 'animate-pulse bg-blue-500' : 'bg-emerald-500'}`} />{hasActiveJobs ? 'Auto-refreshing every 5s' : 'No active worker jobs'}</div></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border border-blue-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Active jobs</p><p className="mt-1 text-2xl font-black text-blue-700">{activeJobCount}</p></div><div className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Completed runs</p><p className="mt-1 text-2xl font-black text-emerald-700">{completedRunCount}</p></div><div className="rounded-2xl border border-violet-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Assets discovered</p><p className="mt-1 text-2xl font-black text-violet-700">{totalAssets}</p></div><div className="rounded-2xl border border-amber-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Needs attention</p><p className="mt-1 text-2xl font-black text-amber-700">{attentionJobCount}</p></div></div>
      </div>

      <div className="border-b border-slate-100 px-6 py-4"><div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-slate-400"><SlidersHorizontal className="h-4 w-4" />Filter & sort</div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(240px,1.5fr)_minmax(170px,1fr)_minmax(150px,0.8fr)_minmax(170px,0.9fr)_auto]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search source, run ID, job ID, worker or error…" className="w-full rounded-xl border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" /></label><select value={sourceFilter} onChange={event => setSourceFilter(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="ALL">All sources</option>{sources.map(source => <option key={source.id} value={source.id}>{source.name}</option>)}</select><select value={statusFilter} onChange={event => setStatusFilter(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="ALL">All statuses</option>{statusOptions.map(status => <option key={status} value={status}>{status}</option>)}</select><select value={sortKey} onChange={event => setSortKey(event.target.value as SortKey)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="LATEST">Latest activity</option><option value="SOURCE">Source name</option><option value="ASSETS">Assets discovered</option><option value="STATUS">Status</option></select><button type="button" onClick={() => setSortDirection(current => current === 'ASC' ? 'DESC' : 'ASC')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"><ArrowUpDown className="h-4 w-4" />{sortDirection === 'ASC' ? 'Ascending' : 'Descending'}</button></div></div>
      ...
      <div className="divide-y divide-slate-100">{groups.length ? groups.map(group => {
        ...
        {open ? <div className="border-t border-slate-100 bg-slate-50/60 px-6 py-6"><div className="ml-4 border-l-2 border-violet-100 pl-6"><div className="grid gap-6 xl:grid-cols-2">
          <section><div className="mb-3 flex items-center gap-2"><GitBranch className="h-4 w-4 text-violet-600" /><h3 className="text-sm font-black text-slate-800">Discovery runs</h3><span className="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-black text-violet-700">{group.runs.length}</span></div><div className="space-y-3">{group.runs.length ? group.runs.map(run => <div key={run.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><StatusBadge status={run.status} /><span className="font-mono text-[11px] text-slate-400">run {shortId(run.id)}</span></div><span className="inline-flex items-center gap-1 text-sm font-black text-slate-800"><Boxes className="h-4 w-4 text-violet-500" />{run.assets_discovered} assets</span></div><dl className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><div><dt className="font-bold text-slate-400">Started</dt><dd className="mt-0.5">{formatDate(run.started_at)}</dd></div><div><dt className="font-bold text-slate-400">Completed / duration</dt><dd className="mt-0.5">{run.completed_at ? `${formatDate(run.completed_at)} · ${duration(run.started_at, run.completed_at)}` : 'In progress'}</dd></div></dl>{run.error_message ? <div className="mt-3 rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-xs text-red-700">{run.error_message}</div> : null}</div>) : <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-5 text-sm text-slate-400">No discovery runs match the current filters.</div>}</div></section>
