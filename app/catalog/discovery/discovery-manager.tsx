...
export function DiscoveryManager({ sources, runs, jobs, currentAssetCounts }: { sources: Source[]; runs: Run[]; jobs: Job[]; currentAssetCounts: Record<string, number> }) {
  ...
  useEffect(() => {
    ...
    if (matchingRun && (matchingRun.completed_at || matchingRun.error_message)) {
      setPollTarget(null)
      setMessage(matchingRun.error_message ? `Metadata discovery failed: ${matchingRun.error_message}` : `Metadata discovery completed. ${matchingRun.assets_discovered} objects observed in this scan.`)
    }
  }, [pollTarget, runs])
  ...
  const attentionJobCount = jobs.filter(job => ['warning', 'error'].includes(discoveryJobHealth(job).severity)).length
  ...
  return <div className="mt-6 space-y-6">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 bg-gradient-to-r from-violet-50 via-white to-blue-50 px-6 py-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><RefreshCw className={`h-5 w-5 text-violet-600 ${hasActiveJobs ? 'animate-spin' : ''}`} /><h2 className="text-2xl font-black">Discovery activity</h2></div><p className="mt-1 text-sm text-slate-500">Current catalog state is separated from historical scan observations and worker execution.</p></div><div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 shadow-sm"><span className={`h-2 w-2 rounded-full ${hasActiveJobs ? 'animate-pulse bg-blue-500' : 'bg-emerald-500'}`} />{hasActiveJobs ? 'Auto-refreshing every 5s' : 'No active worker jobs'}</div></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border border-blue-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Active jobs</p><p className="mt-1 text-2xl font-black text-blue-700">{activeJobCount}</p></div><div className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Completed scans</p><p className="mt-1 text-2xl font-black text-emerald-700">{completedRunCount}</p></div><div className="rounded-2xl border border-violet-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Current assets</p><p className="mt-1 text-2xl font-black text-violet-700">{totalCurrentAssets}</p></div><div className="rounded-2xl border border-amber-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Needs attention</p><p className="mt-1 text-2xl font-black text-amber-700">{attentionJobCount}</p></div></div>
      </div>
      ...
      <div className="divide-y divide-slate-100">{groups.length ? groups.map(group => {
        ...
        {open ? <div className="border-t border-slate-100 bg-slate-50/60 px-6 py-6"><div className="ml-4 border-l-2 border-violet-100 pl-6"><div className="grid gap-6 xl:grid-cols-2">
          <section><div className="mb-3 flex items-center gap-2"><GitBranch className="h-4 w-4 text-violet-600" /><h3 className="text-sm font-black text-slate-800">Discovery scans</h3><span className="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-black text-violet-700">{group.runs.length}</span></div><div className="space-y-3">{group.runs.length ? group.runs.map(run => <div key={run.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><StatusBadge status={run.status} /><span className="font-mono text-[11px] text-slate-400">run {shortId(run.id)}</span></div><span className="inline-flex items-center gap-1 text-sm font-black text-slate-800"><Boxes className="h-4 w-4 text-violet-500" />{run.assets_discovered} objects observed</span></div><dl className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><div><dt className="font-bold text-slate-400">Started</dt><dd className="mt-0.5">{formatDate(run.started_at)}</dd></div><div><dt className="font-bold text-slate-400">Completed / duration</dt><dd className="mt-0.5">{run.completed_at ? `${formatDate(run.completed_at)} · ${duration(run.started_at, run.completed_at)}` : 'In progress'}</dd></div></dl>{run.error_message ? <div className="mt-3 rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-xs text-red-700">{run.error_message}</div> : null}</div>) : <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-5 text-sm text-slate-400">No discovery scans match the current filters.</div>}</div></section>
