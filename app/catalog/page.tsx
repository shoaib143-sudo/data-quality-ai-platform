...
export default async function CatalogPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  const canDiscover=capabilityRows.some(([, ,discover])=>discover)
  ...
  return <main className="min-h-screen bg-[#061426] text-slate-100"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <nav className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-[#0a1d33] px-5 py-3 shadow-sm"><Link href="/home" className="flex items-center gap-3 font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"><span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 text-white"><Layers3 className="h-5 w-5"/></span>DataNexus AI</Link><div className="flex flex-wrap gap-2 text-sm">{canDiscover?<Link href="/catalog/discovery" className="rounded-xl px-3 py-2 font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white">Discovery</Link>:null}<Link href="/catalog/physical-assets" className="rounded-xl px-3 py-2 font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white">Physical Assets</Link>{canGlossary?<Link href="/glossary" className="rounded-xl px-3 py-2 font-semibold text-slate-300 hover:bg-white/[0.05] hover:text-white">Glossary</Link>:null}</div></nav>
    ...
    <CatalogManager datasets={datasetsResult.data??[]} versions={versionsResult.data??[]} catalog={catalogResult.data??[]} projects={projectsResult.data??[]} members={membersResult.data??[]} editableProjectIds={editableProjectIds} initialQuery={(params.q??'').trim().slice(0,120)}/>
  </div></main>
}