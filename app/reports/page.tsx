...
export default async function ReportsPage() {
  ...
  const {data:datasets,error:datasetsError}=projectIds.length?await supabase.schema('catalog').from('datasets').select('id,project_id').in('project_id',projectIds):{data:[],error:null}
  if(datasetsError) throw new Error(`Unable to load report datasets: ${datasetsError.message}`)
  const datasetIds=(datasets??[]).map((dataset)=>dataset.id)
  const [{data:issues,error:issuesError},{data:alerts,error:alertsError}]=await Promise.all([
