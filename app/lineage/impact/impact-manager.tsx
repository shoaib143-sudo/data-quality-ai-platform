...
type Result={analysisId:string;projectId:string;rootAssetType:string;rootAssetId:string;direction:string;maxDepth:number;affectedCount:number;criticalAffectedCount:number;riskScore:number;confidence:number;summary:string;nodes:ImpactNode[]}
...
export function ImpactManager({projects,datasets}:{projects:Project[];datasets:Dataset[]}){
  ...
  async function analyze(event:FormEvent){
    ...
    try{
      ...
      const response=await fetch('/api/lineage/impact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,rootAssetType:'DATASET',rootAssetId:selectedId,rootAssetName:selected?.name,direction,maxDepth,triggerType:'USER_REQUEST'})})
  ...
  return <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
    <form onSubmit={analyze} className="h-fit rounded-3xl border bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">Traverse persisted lineage evidence without modifying source systems.</p>
      <div className="mt-5 grid gap-4">
        ...
        <label className="text-sm font-semibold">Maximum depth<input type="number" min={1} max={20} value={maxDepth} onChange={event=>setMaxDepth(Math.max(1,Math.min(20,Number(event.target.value)||5)))} className="mt-1 w-full rounded-xl border px-3 py-2.5"/></label>
