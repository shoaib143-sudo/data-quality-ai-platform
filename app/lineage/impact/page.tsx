...
export default async function LineageImpactPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <nav className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-white px-5 py-3 shadow-sm"><Link href="/lineage" className="font-bold text-blue-700">← Lineage</Link><div className="flex gap-2">{canObservability?<Link href="/observability/incidents" className="rounded-xl border px-3 py-2 text-sm font-semibold">AI Operations Center</Link>:null}<Link href="/lineage/ingest" className="rounded-xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Lineage ingestion</Link></div></nav>
