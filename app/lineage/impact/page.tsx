...
export default async function LineageImpactPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen text-slate-100"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Lineage Impact" contextLabel="Blast radius and business exposure" homeHref="/home" />
    <nav className="dn-glass-rail sticky top-[4.5rem] z-30 mt-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl px-3 py-2"><Link href="/lineage" className="font-bold text-blue-700">← Lineage</Link><div className="flex gap-2">{canObservability?<Link href="/observability/incidents" className="rounded-xl border px-3 py-2 text-sm font-semibold">AI Operations Center</Link>:null}<Link href="/lineage/ingest" className="rounded-xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Lineage ingestion</Link></div></nav>
    <section className="dn-surface mt-4 p-5 sm:p-6"><div className="inline-flex items-center gap-2 rounded-full bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-700"><Network className="h-4 w-4"/>Lineage Impact Intelligence</div><h1 className="mt-3 text-3xl font-black">Blast radius, business exposure and confidence from persisted lineage evidence</h1><p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">Analyze downstream or upstream dependencies across governed and externally discovered assets. Risk combines incident severity, path distance, dataset criticality and certification evidence while confidence reflects lineage provenance and transformation evidence.</p><div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-slate-600"><span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5"><GitBranch className="h-3.5 w-3.5"/>Cycle-safe multi-hop traversal</span><span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5"><ShieldCheck className="h-3.5 w-3.5"/>Auditable deterministic scoring</span></div></section>
