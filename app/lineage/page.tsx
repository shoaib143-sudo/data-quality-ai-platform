...
export default async function LineagePage(){
  ...
  const [edgesResult,datasetsResult,sourcesResult,versionsResult,runsResult,externalAssetsResult]=await Promise.all([
    ...
    supabase.schema('governance').from('lineage_assets').select('id,namespace,name,asset_type,dataset_id').order('last_seen_at',{ascending:false}).limit(1000),
  ])
  for(const result of [edgesResult,datasetsResult,sourcesResult,versionsResult,runsResult,externalAssetsResult]) if(result.error) throw new Error(result.error.message)
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <header className="rounded-3xl border border-violet-100 bg-white p-7 shadow-sm"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-50 text-violet-600"><GitBranch className="h-6 w-6"/></span><div><h1 className="text-3xl font-black">Data Lineage & Impact</h1><p className="mt-1 text-sm text-slate-500">Automatically discovered and externally ingested source, dataset, version, profiling and execution relationships.</p></div></div></header>
    <section className="mt-6 grid gap-4 sm:grid-cols-4"><div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Edges</p><p className="mt-1 text-3xl font-black">{edges.length}</p></div><div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Datasets</p><p className="mt-1 text-3xl font-black">{datasets.size}</p></div><div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">External assets</p><p className="mt-1 text-3xl font-black">{externalAssets.size}</p></div><div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Auto discovered</p><p className="mt-1 text-3xl font-black">{edges.filter(e=>e.metadata?.auto_discovered).length}</p></div></section>
