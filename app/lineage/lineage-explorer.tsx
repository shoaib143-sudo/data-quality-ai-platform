...
export function LineageExplorer({fields,mappings,edges,stats}:Props){
  ...
  const visibleMappings=useMemo(()=>{
    ...
    return mappings.filter(mapping=>{
      ...
      if(governedOnly&&!([source,target].some(field=>field.terms.length||field.classifications.length||field.stakeholders.length||field.contracts.length)))return false
      return true
    })
  ...
  return <>
    <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      <Stat label="Edges" value={stats.edges}/><Stat label="Datasets" value={stats.datasets}/><Stat label="Lineage assets" value={stats.assets}/><Stat label="Transformations" value={stats.transformations}/><Stat label="Mapped fields" value={stats.mappedColumns}/>
    </section>
    ...
    <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 border-t pt-4">
        ...
        <span className="text-xs font-semibold text-slate-400">{visibleMappings.length} field mappings visible</span>
      </div>
    </section>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 space-y-4">{visibleMappings.length?visibleMappings.map(mapping=>{
          const source=fieldByKey.get(mapping.sourceFieldKey)!
          const target=fieldByKey.get(mapping.targetFieldKey)!
        ...
        }):<div className="rounded-2xl border border-dashed p-10 text-center"><GitBranch className="mx-auto h-10 w-10 text-slate-300"/><h3 className="mt-3 font-bold text-slate-700">No mapped fields match this view</h3><p className="mt-1 text-sm text-slate-500">Column mappings appear here as soon as lineage ingestion supplies source and target field mappings. Dataset-level lineage remains available below.</p></div>}</div>
      ...
      <aside className="self-start rounded-3xl border border-slate-200 bg-white shadow-sm xl:sticky xl:top-6">
        {selected?<FieldDrawer field={selected} overlays={activeOverlays} onClose={()=>setSelectedKey(null)}/>:<div className="grid min-h-[420px] place-items-center p-8 text-center"><div><Database className="mx-auto h-11 w-11 text-slate-300"/><h3 className="mt-4 text-lg font-black">Field governance context</h3><p className="mt-2 text-sm text-slate-500">Choose a source or target field to see quality, glossary, stakeholder, classification, contract and operational evidence together.</p></div></div>}
      </aside>
    </div>
  </>
}
