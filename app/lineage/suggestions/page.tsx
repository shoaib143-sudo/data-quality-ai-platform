...
export default async function AiLineageSuggestionsPage(){
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Lineage Suggestions" contextLabel="Governed AI dependency candidates" homeHref="/home" />
    <nav className="mb-6 mt-4 flex flex-wrap items-center justify-end gap-3 rounded-2xl border bg-white px-5 py-3 shadow-sm"><div className="flex flex-wrap gap-2 text-sm"><Link href="/lineage" className="rounded-xl px-3 py-2 font-semibold text-violet-600 hover:bg-violet-50">Lineage explorer</Link><Link href="/lineage/impact" className="rounded-xl px-3 py-2 font-semibold text-violet-600 hover:bg-violet-50">Impact analysis</Link><Link href="/lineage/ingest" className="rounded-xl px-3 py-2 font-semibold text-violet-600 hover:bg-violet-50">Ingest observed lineage</Link></div></nav>
