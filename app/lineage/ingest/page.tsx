...
export default async function LineageIngestPage() {
  ...
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
    <GlobalUtilityBar persona={landing.persona} organizationRole={landing.organizationRole} roleLabel="Lineage Ingestion" contextLabel="Observed lineage integrations" homeHref="/home" />
    <nav className="mb-6 mt-4 flex flex-wrap items-center justify-end gap-3 rounded-2xl border bg-white px-5 py-3 shadow-sm"><div className="flex gap-2 text-sm"><Link href="/lineage" className="rounded-xl px-3 py-2 font-semibold text-violet-700 hover:bg-violet-50">Lineage graph</Link>{canDiscovery ? <Link href="/catalog/discovery" className="rounded-xl px-3 py-2 font-semibold text-slate-600 hover:bg-slate-100">Discovery</Link> : null}</div></nav>
