...
export async function GET(request: Request) {
  try {
    ...
    if (error) throw new Error(`Unable to load quality rules: ${error.message}`)
    return NextResponse.json({ rules: data ?? [] })
  } catch (error) {
    const authorization = authorizationErrorResponse(error)
    if (authorization) return NextResponse.json({ error: authorization.error }, { status: authorization.status })
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to load quality rules.' }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const { data: dataset, error: datasetError } = await admin
      ...
      .eq('project_id', projectId)
      .maybeSingle()
    if (datasetError) throw new Error(`Unable to verify dataset: ${datasetError.message}`)
    ...
    const authorization = authorizationErrorResponse(error)
    if (authorization) return NextResponse.json({ error: authorization.error }, { status: authorization.status })
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to create quality rule.' }, { status: 500 })
  }
}
