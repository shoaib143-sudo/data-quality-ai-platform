...
async function reconcileSourceBoundDatasets(admin: ReturnType<typeof createAdminClient>, source: { id: string; project_id: string; source_type: string; connection_metadata: unknown }) {
  ...
  const latestByDataset = new Map<string, DatasetVersionForReconciliation>(); for (const version of typedVersions) if (!latestByDataset.has(version.dataset_id)) latestByDataset.set(version.dataset_id, version)
  ...
  for (const dataset of datasets) {
    ...
    await admin.schema('profiling').from('dataset_execution_sources').update({ execution_config: { ...executionConfig, ...connectionMetadata, source_id: source.id, source_type: source.source_type, connection_metadata: connectionMetadata, validation }, active: true, updated_at: now }).eq('id', executionSource.id)
  }
}

export async function POST(request: Request) {
  try {
    const user = await requireUser(); const body = await request.json(); const projectId = text(body.projectId); const name = text(body.name); const sourceType = text(body.sourceType).toUpperCase() || 'JDBC'; const sourceUri = text(body.sourceUri) || text(body.jdbcUrl); const jdbcUrl = text(body.jdbcUrl); const connectionKind = text(body.connectionKind) || 'jdbc'; const schema = text(body.schema); const table = text(body.table); const connectionOnly = body.connectionOnly === true; const requestedCredentialRef = text(body.credentialRef)
    ...
    if (sourceType === 'JDBC' && connectionOnly) {
      const connectionMetadata: Record<string, unknown> = { jdbc_url: jdbcUrl, connection_kind: connectionKind, credential_ref: credentialRef }; if (schema) connectionMetadata.schema = schema; if (table) connectionMetadata.table = table
      ...
      if (existing) { const { data: source, error } = await admin.schema('catalog').from('data_sources').update({ source_type: 'JDBC', connection_metadata: connectionMetadata, status: 'CONFIGURED', updated_at: new Date().toISOString() }).eq('id', existing.id).select('id, project_id, name, source_type, connection_metadata, status, created_at, updated_at').single(); if (error || !source) return NextResponse.json({ error: `Unable to save connection: ${error?.message ?? 'unknown error'}` }, { status: 500 }); return NextResponse.json({ source, profiling_ready: false, connection_saved: true }) }
      const { data: source, error } = await admin.schema('catalog').from('data_sources').insert({ project_id: projectId, name, source_type: 'JDBC', connection_metadata: connectionMetadata, status: 'CONFIGURED' }).select('id, project_id, name, source_type, connection_metadata, status, created_at, updated_at').single(); if (error || !source) return NextResponse.json({ error: `Unable to save connection: ${error?.message ?? 'unknown error'}` }, { status: 500 }); return NextResponse.json({ source, profiling_ready: false, connection_saved: true })
    }
