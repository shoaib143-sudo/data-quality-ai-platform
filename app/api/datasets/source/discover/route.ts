...
export async function POST(request: Request) {
  try {
    ...
    if (!projectId || !jdbcUrl) return NextResponse.json({ error: 'projectId and connection string are required.' }, { status: 400 })
    if (!credentialRef) return NextResponse.json({ error: 'No server-managed JDBC credentials are configured for this connection type.' }, { status: 503 })
