  ...
  try {
    ...
    if (!projectId || !sourceId || !name || !sourceIdentifier) {
      return NextResponse.json(
        { error: 'projectId, sourceId, name, and sourceIdentifier are required.' },
        { status: 400 },
      )
    }
    ...
    if (sourceError || !source) {
      return NextResponse.json({ error: 'The selected data source is unavailable.' }, { status: 404 })
    }
    ...
    if (existingDataset) {
      return NextResponse.json({ error: 'A dataset with this name already exists in the project.' }, { status: 409 })
    }
    ...
    const { data: dataset, error: datasetError } = await admin
      ...
      .select('id, project_id, data_source_id, name, description, source_identifier, business_domain, status, created_at')
      .single()

    if (datasetError || !dataset) {
      throw new Error(`Unable to register dataset: ${datasetError?.message ?? 'unknown error'}`)
    }

    const { data: latestVersion, error: versionLookupError } = await admin
      .schema('catalog')
    ...
    const { data: version, error: versionError } = await admin
      ...
      .select('id, dataset_id, version_number, source_uri, status, observed_at, created_at')
      .single()
    ...
    const { error: executionSourceError } = await admin
      .schema('profiling')
