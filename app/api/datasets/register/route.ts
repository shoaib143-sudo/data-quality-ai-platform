...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: source, error: sourceError } = await admin.schema('catalog').from('data_sources').select('id, project_id, name, source_type, connection_metadata, status').eq('id', sourceId).eq('project_id', projectId).eq('status', 'ACTIVE').maybeSingle()
