...
export async function POST(request: Request) {
  try {
    ...
    const { data: activeJob, error: activeJobError } = await admin
      ...
      .from('job_queue')
      .select('id,status,attempts,max_attempts,created_at,started_at')
      ...
      .limit(1)
      .maybeSingle()
    if (activeJobError) throw new Error(`Unable to check active discovery jobs: ${activeJobError.message}`)
    if (activeJob) {
      return NextResponse.json({
        accepted: true,
        alreadyActive: true,
        durableJobId: activeJob.id,
    ...
    await writeGovernanceAudit({
      projectId: source.project_id,
      actorUserId: user.id,
      eventType: 'METADATA_DISCOVERY_QUEUED',
      entityType: 'DATA_SOURCE',
      entityId: source.id,
      metadata: { durable_job_id: durable.id, source_type: source.source_type },
    })
