...
export function ScheduleManager({projects,datasets,versions,initialSchedules}:{projects:Project[];datasets:Dataset[];versions:Version[];initialSchedules:Schedule[]}){
  ...
  const visibleSchedules=schedules.filter(schedule=>schedule.project_id===projectId)
  ...
  async function refresh(targetProjectId=projectId){
    if(!targetProjectId){setSchedules([]);return}
    const response=await fetch(`/api/schedules?projectId=${encodeURIComponent(targetProjectId)}`)
    const payload=await response.json()
    if(!response.ok) throw new Error(payload.error??'Unable to load schedules.')
    setSchedules(current=>[...current.filter(schedule=>schedule.project_id!==targetProjectId),...(payload.schedules??[])])
  }
  async function create(event:FormEvent){
    ...
    try{
      ...
      const response=await fetch('/api/schedules',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,datasetVersionId:version,jobType,name,cadence,runHour:Number(runHour),runMinute:Number(runMinute),intervalMinutes:Number(intervalMinutes),dayOfWeek:Number(dayOfWeek),misfirePolicy,maxAttempts:3,backoffMinutes:5})})
      const payload=await response.json()
  ...
  return <div className="mt-6 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
    <form onSubmit={create} className="rounded-3xl border bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-4">
        <label className="text-sm font-semibold">Project<select value={projectId} onChange={e=>{setProjectId(e.target.value);setDatasetVersionId('');setMessage('');void refresh(e.target.value)}} className="mt-1 w-full rounded-xl border px-3 py-2.5">{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
        ...
        <button disabled={busy||!projectId} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 font-bold text-white disabled:opacity-50">{busy?<Loader2 className="h-4 w-4 animate-spin"/>:<Plus className="h-4 w-4"/>}Create schedule</button>
        {message?<p className="text-sm text-slate-600" role="status" aria-live="polite">{message}</p>:null}
      </div>
    </form>

    <section className="rounded-3xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">Run calendar</h2><p className="mt-1 text-sm text-slate-500">Only schedules for the selected project are shown. Durable schedules survive browser sessions and retry according to the configured policy.</p><div className="mt-5 space-y-3">{visibleSchedules.length?visibleSchedules.map(s=><article key={s.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="font-bold">{s.name}</span><span className="rounded-full border px-2 py-0.5 text-xs font-bold">{s.job_type}</span><span className={`rounded-full px-2 py-0.5 text-xs font-bold ${s.enabled?'bg-emerald-50 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{s.enabled?'ENABLED':'PAUSED'}</span></div><p className="mt-1 text-sm text-slate-500">{versionLabels.get(s.dataset_version_id)??s.dataset_version_id}</p><p className="mt-2 text-xs text-slate-400">{s.cadence} · next {new Date(s.next_run_at).toLocaleString()} · misfire {s.misfire_policy} · retries {String(s.retry_policy?.max_attempts??3)}</p></div><div className="flex gap-2"><button type="button" onClick={()=>void patch(s.id,{enabled:!s.enabled})} className="rounded-lg border p-2" title={s.enabled?'Pause':'Resume'} aria-label={`${s.enabled?'Pause':'Resume'} schedule ${s.name}`}>{s.enabled?<Pause className="h-4 w-4" aria-hidden="true"/>:<Play className="h-4 w-4" aria-hidden="true"/>}</button><button type="button" onClick={()=>void remove(s.id)} className="rounded-lg border border-red-200 p-2 text-red-600" aria-label={`Delete schedule ${s.name}`}><Trash2 className="h-4 w-4" aria-hidden="true"/></button></div></div></article>):<div className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">No recurring jobs configured for this project.</div>}</div></section>
  </div>
}