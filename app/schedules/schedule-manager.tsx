...
export function ScheduleManager({projects,datasets,versions,initialSchedules}:{projects:Project[];datasets:Dataset[];versions:Version[];initialSchedules:Schedule[]}){
  ...
  return <div className="mt-6 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
    ...
    <section className="rounded-3xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">Run calendar</h2><p className="mt-1 text-sm text-slate-500">Only schedules for the selected project are shown. Durable schedules survive browser sessions and retry according to the configured policy.</p><div className="mt-5 space-y-3">{visibleSchedules.length?visibleSchedules.map(s=><article key={s.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="font-bold">{s.name}</span><span className="rounded-full border px-2 py-0.5 text-xs font-bold">{s.job_type}</span><span className={`rounded-full px-2 py-0.5 text-xs font-bold ${s.enabled?'bg-emerald-50 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{s.enabled?'ENABLED':'PAUSED'}</span></div><p className="mt-1 text-sm text-slate-500">{versionLabels.get(s.dataset_version_id)??s.dataset_version_id}</p><p className="mt-2 text-xs text-slate-400">{s.cadence} · next {new Date(s.next_run_at).toLocaleString()} · misfire {s.misfire_policy} · retries {String(s.retry_policy?.max_attempts??3)}</p></div><div className="flex gap-2"><button onClick={()=>void patch(s.id,{enabled:!s.enabled})} className="rounded-lg border p-2" title={s.enabled?'Pause':'Resume'}>{s.enabled?<Pause className="h-4 w-4"/>:<Play className="h-4 w-4"/>}</button><button onClick={()=>void remove(s.id)} className="rounded-lg border border-red-200 p-2 text-red-600"><Trash2 className="h-4 w-4"/></button></div></div></article>):<div className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">No recurring jobs configured for this project.</div>}</div></section>
  </div>
}