...
function personaView(slug: PersonaSlug, data: RoleLandingData): PersonaView {
  const risk = String(data.highFindings), pressure = String(data.failedControls + data.openAlerts), coverage = `${data.coverage}%`
...
function icon(kind: MetricCard['icon']) { if (kind === 'database') return <Database className="h-5 w-5" />; if (kind === 'alert') return <AlertTriangle className="h-5 w-5" />; if (kind === 'check') return <CheckCircle2 className="h-5 w-5" />; return <Gauge className="h-5 w-5" /> }
...
export function RoleLandingPage({ persona, data, userLabel, canAdmin = false }: { persona: PersonaDefinition; data: RoleLandingData; userLabel: string; canAdmin?: boolean }) {
  const current = status(data.confidence), view = personaView(persona.slug, data), visibleNav = persona.nav.filter(item => canAdmin || !item.href.startsWith('/admin')), findings = data.topFindings.slice(0, 3)
  return <main className="min-h-screen bg-[#eef3f8] text-slate-900"><div className="mx-auto grid max-w-[1480px] gap-6 px-4 py-5 lg:grid-cols-[220px_1fr] lg:px-6">
    <aside className={`${surface} hidden min-h-[calc(100vh-40px)] p-4 lg:flex lg:flex-col`}><Link href={`/home/${persona.slug}`} className="flex items-center gap-3 rounded-2xl px-2 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-600 text-white shadow-sm"><ShieldCheck className="h-5 w-5" /></span><span><span className="block text-lg font-black tracking-tight">DataNexus</span><span className="block text-[11px] text-slate-500">Trusted data</span></span></Link><nav className="mt-7 space-y-1.5">{visibleNav.map((item,index)=><Link key={item.href} href={item.href} className={`block rounded-2xl px-4 py-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${index===0?'bg-white/65 text-blue-700 shadow-sm':'text-slate-600 hover:bg-white/55 hover:text-slate-900'}`}>{item.label}</Link>)}</nav>{canAdmin?<div className="mt-auto pt-5"><Link href="/admin" className={`${button} w-full bg-white/55 text-slate-700 shadow-sm`}><Settings className="h-4 w-4" />Administration</Link></div>:null}</aside>
    <section className="min-w-0 py-1"><header className="flex flex-wrap items-start justify-between gap-4 px-1"><div><p className="text-sm font-semibold text-blue-600">{persona.title}</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">Good day, {userLabel}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{persona.primaryQuestion}</p></div><div className="flex flex-wrap gap-2 lg:hidden">{visibleNav.slice(0,3).map(item=><Link key={item.href} href={item.href} className={`${button} ${inset} text-slate-700`}>{item.label}</Link>)}{canAdmin?<Link href="/admin" className={`${button} ${inset} text-slate-700`}><Settings className="h-4 w-4" />Admin</Link>:null}</div></header>
      <section className={`${surface} mt-6 p-6 sm:p-7`}><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-blue-600">{persona.labels.confidence}</p><div className="mt-2 flex items-end gap-3"><span className={`text-5xl font-black tracking-tight ${current.tone}`}>{pct(data.confidence)}</span><span className={`mb-1 rounded-full px-3 py-1 text-xs font-bold ${current.badge}`}>{current.label}</span></div></div><p className="max-w-xl text-sm leading-6 text-slate-600">{persona.strapline}</p></div></section>
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><div className={`${surface} p-5 sm:p-6`}><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-black">{view.attentionTitle}</h2><Link href="/issues" className="rounded-xl px-3 py-2 text-sm font-bold text-blue-600 hover:bg-white/55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">View all</Link></div><div className="mt-4 space-y-2.5">{findings.length?findings.map(finding=><Link key={finding.id} href="/issues" className={`${inset} flex items-center gap-3 p-4 transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${['CRITICAL','HIGH'].includes(finding.severity.toUpperCase())?'bg-red-50 text-red-600':'bg-amber-50 text-amber-600'}`}><AlertTriangle className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{finding.title}</span><span className="mt-0.5 block truncate text-xs text-slate-500">{finding.description}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-slate-400" /></Link>):<div className={`${inset} p-4 text-sm text-slate-500`}>No material findings need attention.</div>}</div></div>
        <div className={`${surface} p-5 sm:p-6`}><h2 className="text-lg font-black">{view.contextTitle}</h2><div className="mt-4 space-y-3">{data.domains.slice(0,5).map(domain=><div key={domain.name} className="flex items-center justify-between gap-4 rounded-xl px-2 py-1.5 text-sm"><span className="truncate font-semibold text-slate-600">{domain.name}</span><span className="font-black tabular-nums">{domain.assets}</span></div>)}{!data.domains.length&&<p className="text-sm leading-6 text-slate-500">Business context will appear as governed assets are classified.</p>}</div></div></section>
...
function Kpi({icon,label,value}:{icon:React.ReactNode;label:string;value:string}){return <div className={`${surface} p-5`}><div className="flex items-center gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-white/55 text-blue-600 shadow-sm">{icon}</span><div><p className="text-2xl font-black tracking-tight tabular-nums">{value}</p><p className="mt-0.5 text-sm font-semibold text-slate-600">{label}</p></div></div></div>}
