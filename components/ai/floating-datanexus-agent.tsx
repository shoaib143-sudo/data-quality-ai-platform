  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-5 left-4 z-[120] grid h-12 w-12 place-items-center rounded-2xl border border-white/10 bg-gradient-to-br from-violet-700 to-blue-700 text-white shadow-[8px_8px_22px_rgba(2,8,18,.30),-3px_-3px_12px_rgba(39,64,91,.055)] hover:-translate-y-0.5 hover:border-violet-300/25 ${focus}`}
      >
      ...
      {open ? (
        <section
          ...
          className={`fixed bottom-20 left-4 z-[119] flex max-h-[min(70vh,720px)] flex-col overflow-hidden rounded-[22px] border border-white/10 bg-[#102036]/97 shadow-[14px_14px_36px_rgba(2,8,18,.38),-6px_-6px_18px_rgba(39,64,91,.055)] backdrop-blur-xl transition-all ${expanded ? 'h-[min(78vh,780px)] w-[min(720px,calc(100vw-2rem))]' : 'h-[min(620px,68vh)] w-[min(410px,calc(100vw-2rem))]'}`}
        >
          <header className="flex items-center justify-between gap-3 border-b border-white/[0.08] bg-gradient-to-r from-violet-500/[0.08] via-blue-500/[0.05] to-transparent px-4 py-3">
          ...
          <div ref={transcriptRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4" aria-live="polite">
            {!messages.length ? (
              <div className="rounded-2xl border border-white/[0.07] bg-[#14283f] p-4">
            ...
            {messages.map((message, index) => (
              <div key={index} className={message.role === 'user' ? 'ml-8' : 'mr-8'}>
                <div className={`rounded-2xl px-3.5 py-3 text-sm leading-6 ${message.role === 'user' ? 'bg-blue-600 text-white' : 'border border-white/[0.07] bg-[#14283f] text-slate-200'}`}>
                  {message.content}
                </div>
              </div>
            ))}
            {busy ? <div className="mr-8 rounded-2xl border border-white/[0.07] bg-[#14283f] px-3.5 py-3 text-sm text-slate-400">Reviewing governed evidence…</div> : null}
          </div>
          ...
          <form onSubmit={submit} className="flex items-end gap-2 border-t border-white/[0.08] bg-[#0b1422]/80 p-3">
            ...
            <textarea
              ...
              className={`min-h-[46px] flex-1 resize-none rounded-xl border border-white/10 bg-[#0d1c30] px-3 py-2.5 text-sm text-white outline-none placeholder:text-slate-600 ${focus}`}
            />
