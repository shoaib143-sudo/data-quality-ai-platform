...
const workflow = readFileSync('.github/workflows/real-field-lineage-revalidation.yml', 'utf8')
...
test('live lineage mutation never runs for pull_request', () => {
  assert.match(workflow, /live-development:\n\s+if: github\.event_name != 'pull_request'/)
})
...
test('live lineage proof uses the existing protected Supabase service-role secret', () => {
  ...
  assert.doesNotMatch(workflow, /VERCEL|CLOUDFLARE|R2/i)
})
