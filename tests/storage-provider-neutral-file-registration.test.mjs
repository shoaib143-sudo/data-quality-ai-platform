...
test('provider-neutral FILE parsing accepts R2, Supabase, and storage URI schemes without regex backtracking', () => {
  assert.match(providerNeutral, /value\.indexOf\('\:\/\/'\)/)
  assert.match(providerNeutral, /scheme !== 'r2' && scheme !== 'supabase' && scheme !== 'storage'/)
  assert.match(providerNeutral, /const provider = scheme === 'r2' \? 'r2' : 'supabase'/)
  assert.match(providerNeutral, /trimBoundarySlashes/)
  assert.doesNotMatch(providerNeutral, /replace\(\/\^\\\/+\|\\\/+\$\/g/)
})
