...
test('canary activation is separately owner-approved and remains reversible', () => {
  assert.match(workflow, /cloudflare-worker-canary-enable/)
  assert.match(workflow, /confirm_worker_execution/)
  assert.match(workflow, /inputs\.confirm_worker_execution == true/)
  assert.match(workflow, /DATANEXUS_WORKER_EXECUTION_ENABLED:true/)
  assert.match(workflow, /DATANEXUS_WORKER_CANARY_JOB_TYPE:OBSERVABILITY/)
...
test('enabled canary requires privileged secrets and verifies unauthorized and over-broad modes fail closed', () => {
  assert.match(workflow, /secrets\.DATANEXUS_WORKER_SECRET/)
  assert.match(workflow, /secrets\.SUPABASE_SERVICE_ROLE_KEY/)
  assert.match(workflow, /test "\$unauthorized_code" = "403"/)
  assert.match(workflow, /test "\$forbidden_code" = "403"/)
  assert.match(workflow, /body\.jobType !== 'OBSERVABILITY'/)
  assert.match(workflow, /body\.claimed < 0 \|\| body\.claimed > 1/)
})
