...
test('post-deploy verification fails closed on identity and critical health contracts', () => {
  ...
  assert.match(deploy, /Vercel remote build did not emit deployed artifact provenance/)
  assert.match(deploy, /\/tmp\/vercel-deploy\.log/)
  assert.match(deploy, /https:\/\/api\.vercel\.com\/v13\/deployments\/\$DEPLOYMENT_HOST/)
