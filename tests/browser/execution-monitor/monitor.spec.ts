...
test('collapse and expand are read-only and retain the root',async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'Governance: running',exact:true})).toBeVisible()
  await page.screenshot({path:'test-results/execution-monitor/living-tree-collapsed.png',fullPage:true})
...
test('reduced motion and mobile layout retain readable controls',async({page})=>{
  ...
  const canvas=page.locator('.tree-scroll')
  const fitted=await canvas.evaluate(el=>({scroll:el.scrollWidth,width:el.clientWidth}))
  expect(fitted.scroll).toBeLessThanOrEqual(fitted.width)
  const rootBounds=await page.getByRole('button',{name:'Governance: running',exact:true}).evaluate(el=>{const r=el.getBoundingClientRect();return {left:r.left,right:r.right}})
  expect(rootBounds.left).toBeGreaterThanOrEqual(0)
  expect(rootBounds.right).toBeLessThanOrEqual(width.window)
...
test('feature flag uses the same safe list and evidence semantics',async({page})=>{
  ...
  await page.screenshot({path:'test-results/execution-monitor/living-tree-list-fallback.png',fullPage:true})
})
