...
# stale, or non-authoritative canonical envelope intentionally produces no
# decision; the application provider treats that as DENY.
decision := result if {
	canonical := input.canonical
	request := input.request

	canonical.authority_status in {"SYSTEM_BASELINE", "APPROVED"}
