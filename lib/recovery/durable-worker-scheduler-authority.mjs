...
const REQUIRED_URL_SECRET = 'DGP_DURABLE_WORKER_URL'
...
export function verifyDurableWorkerSchedulerAuthority({ migrationSql, vercelConfig }) {
  ...
  requireMatch(migrationSql, new RegExp(REQUIRED_URL_SECRET), 'DURABLE_WORKER_URL_SECRET_NAME_MISSING')
  ...
  requireMatch(migrationSql, /url\s*:=\s*worker_url/i, 'DURABLE_WORKER_VAULT_URL_NOT_USED')
  requireMatch(migrationSql, /worker_url\s*!~\s*'\^https:\/\//i, 'DURABLE_WORKER_HTTPS_URL_GUARD_MISSING')
  requireMatch(migrationSql, /\/api\/jobs\/worker\$/i, 'DURABLE_WORKER_CANONICAL_PATH_GUARD_MISSING')
  if (/https:\/\/data-quality-ai-platform\.vercel\.app\/api\/jobs\/worker/i.test(migrationSql)) {
    throw new Error('DURABLE_WORKER_PROVIDER_SPECIFIC_URL_PRESENT')
  }
  ...
  return {
    ...
    workerUrlSecretName: REQUIRED_URL_SECRET,
  }
}
