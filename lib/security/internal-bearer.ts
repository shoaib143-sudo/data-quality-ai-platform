...
const GITHUB_OIDC_SUBJECT = 'repo:shoaib143-sudo/data-quality-ai-platform:environment:production'
...
async function githubOidcKeys(forceRefresh = false) {
  const now = Date.now()
...
async function verifyGitHubActionsOidcBearer(request: Request) {
  ...
  const matchesSigningKey = (candidate: JsonWebKeyWithKid) =>
    candidate.kid === header.kid
    && candidate.kty === 'RSA'
    && (!candidate.alg || candidate.alg === 'RS256')
    && (!candidate.use || candidate.use === 'sig')

  let jwk = (await githubOidcKeys()).find(matchesSigningKey)
  if (!jwk) jwk = (await githubOidcKeys(true)).find(matchesSigningKey)
  if (!jwk) return false
