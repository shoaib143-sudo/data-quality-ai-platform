...
function containsSubstantiveCommand(block, command) {
  if (!command || typeof command !== 'string') return false
  return block.includes(command)
}
...
export function validateExactMainCertificationWorkflow(source, {
  ...
  if (postMergeBlock) {
    for (const command of expectedPostMergeCommands) {
      if (!containsSubstantiveCommand(postMergeBlock, command)) {
        failures.push(`full post-merge certification job ${requiredPostMergeJob} must retain substantive command: ${command}`)
      }
    }
  }
  if (!/(?:^|\n)\s*contents:\s*read\s*(?:$|\n)/.test(`permissions:\n${permissionsBlock}`)) {
