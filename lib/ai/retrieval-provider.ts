...
/**
 * Classifies retrieval authority only from explicit governance evidence already
 * stored with the projection. Similarity, object type, source table and model
 * output are never governance authority and cannot upgrade a result.
 */
export function classifyRetrievalAuthority(metadata: Record<string, unknown>): {
  ...
  if (referenceKeys.length && (statusKeys.length || explicitGoverned || humanReviewed)) {
    const evidenceKeys = [...new Set([
      ...referenceKeys,
      ...statusKeys,
      ...explicitEvidenceKeys,
      ...(humanReviewed ? ['human_reviewed'] : []),
    ])]
    return { authority: 'GOVERNED', evidenceKeys }
  }

  const observationalKeys = ['observed_at', 'source_provider', 'source_revision_id', 'source_table']
    .filter((key) => nonEmpty(metadata[key]))
...
function normalizeAuthority(metadata: Record<string, unknown>) {
  const classified = classifyRetrievalAuthority(metadata)
  if (classified.authority === 'GOVERNED') {
    return {
      authorityClass: 'GOVERNED_DECISION' as const,
      authoritySource: classified.evidenceKeys[0] ?? null,
    }
  }
  if (classified.authority === 'OBSERVATION') {
    return {
      authorityClass: 'OBSERVED_EVIDENCE' as const,
      authoritySource: classified.evidenceKeys[0] ?? null,
    }
  }
  ...
  for (const key of AUTHORITY_KEYS) {
    ...
    if (!token) continue
    const authorityClass = AUTHORITY_ALIASES[token]
    if (authorityClass) return { authorityClass, authoritySource: key }
  }
 ...
 * timestamp evidence is excluded by temporal mode rather than being assumed current.
 */
export function resolveRetrievalTemporalEvidence(
  metadata: Record<string, unknown>,
  asOf: string,
): RetrievalTemporalEvidence | null {
  ...
  const validFrom = starts.length
    ? starts.reduce((latest, entry) => entry.timestamp > latest.timestamp ? entry : latest)
    : null
  const validTo = ends.length
    ? ends.reduce((earliest, entry) => entry.timestamp < earliest.timestamp ? entry : earliest)
    : null
...
export class SemanticProjectionRetrievalProvider implements RetrievalProvider {
  ...
  async retrieve(request: RetrievalRequest): Promise<RetrievalResponse> {
    ...
    if (requiresTemporal && !Number.isFinite(Date.parse(request.asOf!))) {
      throw new Error('Temporal retrieval requires a valid asOf timestamp')
    }
    ...
    const evidenceAwareLimit = requiresAuthority || requiresTemporal
      ? Math.min(100, perProjectLimit * 3)
      : perProjectLimit
    ...
    const normalizationNow = requiresTemporal ? new Date(request.asOf!) : new Date()
    const matches = groups
      .flatMap(({ projectId, matches: projectMatches }) => projectMatches.flatMap((match): RetrievalMatch[] => {
        ...
        return [{
          ...
          metadata: withNormalizedRetrievalMetadata(metadata, normalizationNow),
