...
function candidateEvidence(entry: RegisteredModelVersion, policy: RoutingPolicy | null, now: Date): CandidateEvidence {
  ...
  if (!enforcedPolicy) return { averageScore: null, scoredCount: 0, passRate: null, evidenceResultIds: [], lastObservedAt: null, fallbackReason: 'NO_ACTIVE_POLICY' }
  if (!enforcedPolicy.evaluationType || !enforcedPolicy.evaluationMetricName) return { averageScore: null, scoredCount: 0, passRate: null, evidenceResultIds: [], lastObservedAt: null, fallbackReason: 'EVALUATION_RANKING_NOT_CONFIGURED' }
  ...
  if (metrics.length !== 1) return { averageScore: null, scoredCount: 0, passRate: null, evidenceResultIds: [], lastObservedAt: null, fallbackReason: 'INCOMPLETE_COMPARABLE_EVIDENCE' }

  const metric = metrics[0]
  const passFailCount = metric.passCount + metric.failCount
  const passRate = passFailCount > 0 ? metric.passCount / passFailCount : null
  const evidenceResultIds = metric.evidenceResultIds ?? []
  ...
  if (enforcedPolicy.evaluationMaxAgeSeconds !== null) {
    ...
    if (!Number.isFinite(observedMs) || observedMs > now.getTime() || now.getTime() - observedMs > maxAgeMs) return { averageScore: null, scoredCount: metric.scoredCount, passRate, evidenceResultIds, lastObservedAt, fallbackReason: 'STALE_EVALUATION_EVIDENCE' }
  }
  if (metric.averageScore === null || metric.scoredCount < enforcedPolicy.minScoredCount) return { averageScore: null, scoredCount: metric.scoredCount, passRate, evidenceResultIds, lastObservedAt, fallbackReason: 'INSUFFICIENT_EVALUATION_EVIDENCE' }
  if (enforcedPolicy.minEvaluationScore !== null && metric.averageScore < enforcedPolicy.minEvaluationScore) return { averageScore: null, scoredCount: metric.scoredCount, passRate, evidenceResultIds, lastObservedAt, fallbackReason: 'EVALUATION_SCORE_BELOW_POLICY_THRESHOLD' }
  return { averageScore: metric.averageScore, scoredCount: metric.scoredCount, passRate, evidenceResultIds, lastObservedAt, fallbackReason: null }
}

function deterministicCompare(left: RankedCandidate, right: RankedCandidate) { return left.entry.systemKey.localeCompare(right.entry.systemKey) }
function evidenceCompare(left: RankedCandidate, right: RankedCandidate) {
  if (left.evidence.averageScore !== right.evidence.averageScore) return (right.evidence.averageScore ?? 0) - (left.evidence.averageScore ?? 0)
...
export class EvaluationAwareIntelligentRouter implements IntelligentModelRouter {
  ...
  constructor(dependencies: IntelligentRouterDependencies) { this.dependencies = dependencies }
  ...
  async route(context: IntelligentRouteContext): Promise<IntelligentRouteDecision> {
    ...
    try { policy = await this.dependencies.routingPolicy.resolve({ projectId, task: context.task, sensitivity: context.sensitivity, risk: context.risk }) }
    catch { return { source: 'UNAVAILABLE', reason: 'ROUTING_POLICY_UNAVAILABLE', provider: null, evidence: null } }

    let candidates: RegisteredModelVersion[]
    try { candidates = await this.dependencies.registry.listCurrent({ projectId, capability: context.task, routingEligibleOnly: true }) }
    catch { return { source: 'UNAVAILABLE', reason: 'REGISTRY_UNAVAILABLE', provider: null, evidence: null } }
    ...
    const eligible = candidates.map((entry) => {
      ...
      return { entry, evidence: candidateEvidence(entry, policy, now), policyReason: policyEvaluation.reason, policyAllowed: policyEvaluation.allowed }
