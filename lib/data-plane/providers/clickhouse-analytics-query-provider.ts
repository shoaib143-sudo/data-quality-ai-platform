...
const metricSpecs: Record<string, MetricSpec> = {
  'profiling.run_history': {
    ...
    select: [
      ...
      'toString(occurred_at) AS observedAt',
      'profile_run_id AS profileRunId',
  ...
  'profiling.metric_history': {
    ...
    select: [
      "'PROFILING.METRIC_BATCH_CAPTURED' AS eventType",
      'toString(occurred_at) AS observedAt',
      'profile_run_id AS profileRunId',
      'dataset_id AS datasetId',
      'dataset_version_id AS datasetVersionId',
      'profile_column_id AS profileColumnId',
      'metric_key AS metricKey',
      "if(JSONHas(metric_json, 'numericValue'), toFloat64OrNull(JSONExtractRaw(metric_json, 'numericValue')), NULL) AS numericValue",
      "nullIf(JSONExtractString(metric_json, 'textValue'), '') AS textValue",
      "if(JSONHas(metric_json, 'booleanValue'), JSONExtractBool(metric_json, 'booleanValue'), NULL) AS booleanValue",
      'metric_json AS metricJson',
    ].join(', '),
    filters: {
      datasetId: 'dataset_id',
      datasetVersionId: 'dataset_version_id',
  ...
  'dq.score_history': {
    ...
    select: [
      ...
      'validity_score AS validityScore',
...
function parseJsonEachRow(text: string): AnalyticsQueryRow[] {
  return text
    ...
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => JSON.parse(line) as AnalyticsQueryRow)
}
...
export class ClickHouseAnalyticsQueryProvider implements AnalyticsQueryProvider {
  ...
  async query(request: AnalyticsQueryRequest): Promise<AnalyticsQueryRow[]> {
    ...
    if (request.from) {
      clauses.push("occurred_at >= parseDateTime64BestEffort({from:String}, 3, 'UTC')")
      parameters.set('from', request.from)
    }
    if (request.to) {
      clauses.push("occurred_at <= parseDateTime64BestEffort({to:String}, 3, 'UTC')")
