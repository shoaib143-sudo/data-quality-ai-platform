export const resourceRouteContracts = [
  ...
  },
  {
    ...
    authenticationEvidence: ['const user = await requireUser()'],
    resolverEvidence: [".eq('id', runId)", 'canViewExecutionRun(user.id, run as AgentRun)'],
    missingEvidence: ['if (!run) notFound()', 'if (!await canViewExecutionRun(user.id, run as AgentRun)) notFound()'],
    sourceConsumers: [
      {
        file: 'app/agents/page.tsx',
  ...
  {
    id: 'governance-run',
    routeFile: 'app/journeys/[projectId]/page.tsx',
    routeBuilder: 'governanceRun',
    identityParams: ['projectId'],
    authenticationEvidence: ['const user = await requireUser()'],
    resolverEvidence: [".eq('id', projectId)", ".eq('organization_id', landing.organizationId)"],
    missingEvidence: ['if (!projectResult.data) notFound()'],
    sourceConsumers: [
      {
        file: 'app/journeys/page.tsx',
        evidence: ['canonicalRoutes.governanceRun(project.id)'],
      },
    ],
    readOnly: true,
  },
  ...
  },
  {
    ...
    identityParams: ['datasetId'],
    authenticationEvidence: ['const user = await requireUser()'],
    resolverEvidence: [".eq('id', datasetId)", "authorizeDataset(user.id, datasetId, 'catalog.update')"],
    missingEvidence: ["try { await authorizeDataset(user.id, datasetId, 'catalog.update') } catch { notFound() }", 'if (!dataset) notFound()', 'if (!project) notFound()'],
    sourceConsumers: [
      {
  ...
  },
  {
    ...
    authenticationEvidence: ['const user = await requireUser()'],
    resolverEvidence: [".eq('id', sourceId)", "authorizeProject(user.id, source.project_id, 'source.manage')"],
    missingEvidence: ['if (!source) notFound()', 'if (!project) notFound()', "try { await authorizeProject(user.id, source.project_id, 'source.manage') } catch { notFound() }"],
    sourceConsumers: [
      {
