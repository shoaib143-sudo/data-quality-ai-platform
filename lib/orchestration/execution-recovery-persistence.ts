...
import { executeAuthorizedRecovery, type ExecutionRecoveryHandlerRegistry } from './execution-recovery-runtime.ts'
import { initialRecoveryRecord, type CanonicalRecoveryRecord, type RecoveryFailureContext } from './execution-recovery-contract.ts'
...
async function claimAutoRepair(input: {
  ...
  const { data, error } = await admin.schema('orchestration').rpc('claim_execution_recovery_auto_repair', {
    ...
    p_replay_contract_version: input.replayContractVersion,
    p_replay_safe: input.replaySafe,
    p_lease_seconds: 300,
  })
  ...
  return {
    ...
    executionToken: typeof result.execution_token === 'string' ? result.execution_token : null,
  }
}
...
export async function executePersistedRecovery(input: {
  ...
  const result = await executeAuthorizedRecovery({
    ...
    beforeApply: async ({ repair }) => {
      const claim = await claimAutoRepair({
        ...
        replayContractVersion: repair.replayContractVersion,
        replaySafe: repair.replaySafe,
      })
  ...
  if (!repairClaimed && ['ATTEMPT_IN_FLIGHT'].includes(claimReason ?? '')) {
    return { replayed: true, replayState: 'IN_FLIGHT' as const, record: initial }
  }

  await persistCanonicalRecoveryRecord(result.record)
  if (repairClaimed) {
    if (!executionToken) throw new Error('Autonomous recovery claim did not return an execution fence token.')
...
export async function queuePersistedRecoveryResume(input: {
  ...
  return {
    ...
    restartScope: result.restart_scope === 'WHOLE_JOB' || result.restart_scope === 'FAILED_JOB'
      ? result.restart_scope
      : null,
    retryCheckpointId: result.retry_checkpoint_id === null
      ? null
      : typeof result.retry_checkpoint_id === 'string'
        ? result.retry_checkpoint_id
        : input.retryCheckpointId ?? null,
  }
}
...
export async function executePersistedRecoveryAndQueueResume(input: {
  ...
  return {
    ...
    record: {
      ...recovery.record,
      retry_stage: resume.retryStage,
      retry_checkpoint_id: resume.retryCheckpointId,
    },
    resume,
  }
}
