...
import {
  ...
  validateBlastRadius,
  type AutonomyPolicy,
  type CanonicalCapabilityResultRow,
...
function asPolicy(row: Record<string, unknown> | null): AutonomyPolicy {
  ...
  return {
    mode: String(row.mode ?? 'OFF') as AutonomyPolicy['mode'], enabled: row.enabled === true,
    policyVersion: String(row.policy_version ?? 'unknown'), maximumRiskTier: String(row.maximum_risk_tier ?? 'NONE') as AutonomyPolicy['maximumRiskTier'],
    ...
    autoRemediationEnabled: row.auto_remediation_enabled === true, autoRollbackEnabled: row.auto_rollback_enabled === true,
    maxExecutionBudget: Number(row.max_execution_budget ?? 0), maxModelBudget: Number(row.max_model_budget ?? 0),
    maxRuntimeMs: Number(row.max_runtime_ms ?? 300000), maxDatasetsChangedPerRun: Number(row.max_datasets_changed_per_run ?? 0),
    maxProjectsAffectedPerRun: Number(row.max_projects_affected_per_run ?? 1), maxRemediationActionsPerHour: Number(row.max_remediation_actions_per_hour ?? 0),
    maxConcurrentModelCalls: Number(row.max_concurrent_model_calls ?? 0), emergencyStop: row.emergency_stop === true,
  }
}
...
export async function upsertProjectAutonomyPolicy(projectId: string, actorUserId: string, policy: AutonomyPolicy) {
  ...
  const { error } = await admin.schema('governance').from(ORCHESTRATOR_POLICY_TABLE).upsert({
    ...
    approval_required_actions: policy.approvalRequiredActions, auto_remediation_enabled: policy.autoRemediationEnabled,
    auto_rollback_enabled: policy.autoRollbackEnabled, max_execution_budget: policy.maxExecutionBudget, max_model_budget: policy.maxModelBudget,
    max_runtime_ms: policy.maxRuntimeMs, max_datasets_changed_per_run: policy.maxDatasetsChangedPerRun,
    max_projects_affected_per_run: policy.maxProjectsAffectedPerRun, max_remediation_actions_per_hour: policy.maxRemediationActionsPerHour,
    max_concurrent_model_calls: policy.maxConcurrentModelCalls, emergency_stop: policy.emergencyStop,
    updated_by: actorUserId, updated_at: new Date().toISOString(),
...
function agentPolicyFailure(policy: AutonomyPolicy) {
...
async function insertRun(input: { projectId: string; actorUserId: string; policy: AutonomyPolicy; goal: string; status: string; trace: Record<string, unknown> }) {
  const admin = createAdminClient()
  const { data, error } = await admin.schema('governance').from('governance_orchestrator_runs').insert({
    project_id: input.projectId, actor_user_id: input.actorUserId, policy_version: input.policy.policyVersion, mode: input.policy.mode,
    goal_hash: createHash('sha256').update(input.goal).digest('hex'), status: input.status, decision_trace: input.trace,
...
async function updateRun(id: string, patch: Record<string, unknown>) {
  const admin = createAdminClient()
  const { error } = await admin.schema('governance').from('governance_orchestrator_runs').update(patch).eq('id', id)
  if (error) throw new Error(`Unable to update orchestrator run: ${error.message}`)
}

async function createCapabilityRun(projectId: string, orchestratorRunId: string, policy: AutonomyPolicy) {
  const admin = createAdminClient()
  const { data, error } = await admin.schema('governance').rpc('create_ai_capability_e2e_run', {
    p_project_id: projectId,
    p_metadata: { orchestrator_run_id: orchestratorRunId, policy_version: policy.policyVersion, autonomy_mode: policy.mode, source: 'datanexus_governance_orchestrator' },
  })
...
async function loadCoverage(capabilityRunId: string) {
  const admin = createAdminClient()
  const { data, error } = await admin.schema('governance').from('ai_capability_e2e_results')
    .select('capability_sr_no,module,capability,execution_state,verification_state,blocker_code').eq('run_id', capabilityRunId).order('capability_sr_no')
...
export async function runGovernanceOrchestrator(input: { projectId: string; actorUserId: string; goal: string; sourceScopeVersionId?: string }) {
  ...
  const failures = [
    ...
    ...validateBlastRadius(policy, { datasetsChanged: 0, projectsAffected: 1, remediationActionsThisHour: 0 }),
    ...(policy.mode === 'OFF' || !agentPolicyFailure(policy) ? [] : [agentPolicyFailure(policy)!]),
  ]
  if (failures.length) {
  ...
  try {
    ...
    const supervisor = await runNativeSpecialistSupervisor({
      projectId: input.projectId, actorUserId: input.actorUserId, goal: input.goal,
      workers: [
...
export async function getLatestCoverageRun(projectId: string) {
  const admin = createAdminClient()
  const { data, error } = await admin.schema('governance').from('governance_orchestrator_runs').select('*').eq('project_id', projectId).order('created_at', { ascending: false }).limit(1).maybeSingle()
  if (error) throw new Error(`Unable to load latest orchestrator run: ${error.message}`)
  if (!data) return null
  const capabilityRunId = data.ai_capability_e2e_run_id ? String(data.ai_capability_e2e_run_id) : null
  return { ...data, orchestratorRunId: String(data.id), capabilityRunId, summary: capabilityRunId ? await loadCoverage(capabilityRunId) : summarizeCanonicalCapabilityLedger([]) }
}

export async function finalizeGovernanceOrchestratorCertification(input: { projectId: string; orchestratorRunId: string }) {
  const admin = createAdminClient()
  const { data: row, error } = await admin.schema('governance').from('governance_orchestrator_runs')
  ...
  const preflight = await loadCoverage(capabilityRunId)
  if (!preflight.certificationEligible) {
    throw new Error(`Canonical capability evidence is not certification-ready: ${preflight.executed}/75 executed, ${preflight.verified}/75 verified, ${preflight.blocked} blocked, ${preflight.failed} failed.`)
  }
  const { data: finalized, error: finalizeError } = await admin.schema('governance').rpc('finalize_ai_capability_e2e_run', { p_run_id: capabilityRunId })
  ...
  const summary = await loadCoverage(capabilityRunId)
  await updateRun(input.orchestratorRunId, { status: assessment === 'PASS' ? 'SUCCEEDED' : 'FAILED', completed_at: new Date().toISOString(), decision_trace: { certification_authority: 'governance.finalize_ai_capability_e2e_run', assessment_state: assessment, self_certification: false } })
