...
async function withRetry<T>(
  ...
  for (
    let attempt = 1;
    attempt <= PERSISTENCE_RETRY_LIMIT;
    attempt++
  ) {
    try {
...
export async function executeProfilingExecutor(
  ...
  switch (operation) {
    case "profile_dataset": {
      const definitions =
        (input.metricDefinitions ?? []) as MetricDefinition[]

      const rows =
        (input.rows ?? []) as Record<string, unknown>[]

      const profileRunId =
        input.profileRunId as string | undefined
            ...
            results
              ...
              .map((result) => ({
                ...
                severity:
                  "MEDIUM",
                title:
                  `Metric execution failed: ${result.metric_name}`,
                description:
                  result.error ??
          ...
          const { error } =
            await withRetry(() =>
              supabase
                .schema("profiling")
                .rpc(
                  "persist_profile_execution_result",
                  {
                    p_profile_run_id:
                      profileRunId,
                    p_metrics:
